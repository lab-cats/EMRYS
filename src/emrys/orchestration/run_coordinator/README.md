...
This owner exposes the fixed run-coordinator admission, onboarding, and lifecycle
...
contract. It expects CSU Slurm metadata and the generated run-coordinator wrapper's
...
.venv/bin/python -m pytest -q \
  tests/orchestration/run_coordinator/test_execution_profile.py \
  tests/orchestration/run_coordinator/test_slurm_submission.py \
  tests/orchestration/run_coordinator/test_onboarding.py \
  tests/orchestration/run_coordinator/test_dashboard.py \
...
inventory without depending on the run-coordinator application owner.
...
emrys.orchestration.run_coordinator.task --dispatch ...` module is the one-owner
