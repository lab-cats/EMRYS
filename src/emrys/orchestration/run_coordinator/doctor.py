    ...
    *,
    ...
    if runtime_inventory is None and not os.path.lexists(profile_path):
        ...
        if any(item.check.check_id in fixed_ids - PYTHON_CHECK_IDS for item in failed):
            if selected_shared is not None and runtime_root_for_seal(
                selected_shared.seal_path
        ...
        try:
            ...
            if owner is None or owner == onboarding.runtime_profile_path(project).parent:
                remediations.append(
...
class _RepairPlan:
    ...
    def runtime_work(self) -> str:
        ...
        action = (
            ...
            and self.runtime.source_seal is None
...
def _build_repair_plan(result: DoctorResult) -> _RepairPlan:
    ...
    try:
        selection = (
            ...
            else shared_runtime_selection(profile_bytes)
        )
        ...
        if source_seal is not None:
            ...
            admitted_seal = admit_runtime_seal_bytes(
                source_seal, source_seal_bytes
            )
            replacement_seal = (
    ...
    *,
    ...
    if (
        ...
        or (
            ...
            and runtime_root != plan.runtime.profile.parent
        )
    ):
...
def _print_repair_plan(plan: _RepairPlan) -> None:
    ...
    if plan.runtime is not None:
        ...
        _stderr(
            f"  Maintenance claim: {runtime.profile.parent / 'maintenance.lock'}"
        )
