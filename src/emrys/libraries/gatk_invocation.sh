...
admit_gatk_helper_python() {
    ...
    [[ -n "$python_bin" ]] || {
        printf 'ERROR: EMRYS_SHA256_PYTHON must bind an absolute Python 3.11+ launcher for controlled GATK execution.\n' >&2
        return 2
    }
    [[ "$python_bin" == /* ]] || {
        printf 'ERROR: EMRYS_SHA256_PYTHON must be an absolute path: %s\n' "$python_bin" >&2
        return 2
    }
    [[ -x "$python_bin" ]] || {
        printf 'ERROR: EMRYS_SHA256_PYTHON is not executable: %s\n' "$python_bin" >&2
        return 2
    }
    "$python_bin" -X pycache_prefix=/dev/null -I -c \
        'import sys; raise SystemExit(0 if sys.version_info >= (3, 11) else 2)' || {
        printf 'ERROR: EMRYS_SHA256_PYTHON must run Python 3.11 or newer: %s\n' "$python_bin" >&2
        return 2
    }
...
invoke_gatk_with_selected_java() {
    ...
    python_bin="$EMRYS_SHA256_PYTHON"
