...
from emrys.libraries.validation.inputs import Snapshot, require_unchanged
...
def run(
    build: Callable[[], tuple[bytes, dict[Path, Snapshot]]],
    *,
    step_id: str,
    scope_id: str,
    check_ids: set[str],
    ...
    try:
        return finish(
            Runtime(step_id, scope_id, check_ids, output, execute, published_label),
            *build(),
