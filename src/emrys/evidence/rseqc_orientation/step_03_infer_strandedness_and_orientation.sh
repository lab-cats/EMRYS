...
Options:
  ...
  --no-clobber             Accepted for existing callers; publication always
                           requires an absent final report.
...
cleanup_no_clobber() {
    cleanup_no_clobber_outputs "$1" "Step 03" \
...
# Capture native stdout in staging before publishing the complete report.
"${infer_command[@]}" >"$tmp_output_file"

[[ -s "$tmp_output_file" ]] || die "infer_experiment.py output is missing or empty: $tmp_output_file"
