...
| Reference contig parsing | `libraries/references/contigs.py` | Reference provenance, local-pilot onboarding, Step `00a` through the STAR parser, Step `00c`/`05` validators, and Step `07` through the mpileup validation seam. Agreement policy, reporting, and publication remain local. |
