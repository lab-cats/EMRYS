...
| Step `07` mpileup formats and input parsing | `libraries/validation/mpileup.py` | The Step `07` producer and validator share strict sample/partition manifest admission, FAI and selector mechanics, and fixed receipt/VCF vocabulary. The Step `08` producer and artifact-index consumers reuse only the fixed format vocabulary. Region-file readers remain separate: Project admission accepts Unicode decimal digits in BED coordinates, while the Step `07` producer accepts ASCII digits. Prove equivalent accepted inputs before sharing these readers. Scientific commands, output validation, reconciliation, and reporting policy remain with their owners; the runner owns scientific execution and publication. |
...
| Scientific-worker file checks | `libraries/file_checks.sh` | Bash scientific workers check input files and runner-selected absolute executables and hash through the bound Python. Runtime admission chooses tools; the runner owns execution and publication. |
...
| Selected-Java GATK bridge | `libraries/gatk_invocation.sh` | Step `00c` and Step `05` share only the bound Python handoff into the controlled Java/GATK environment. The runner supplies admitted tools and owns publication and recovery. Each stage retains exact scientific arguments and native validation. |
