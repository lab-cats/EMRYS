...
| Application logging | `libraries/application_logging/` | The binding logging contract authorizes this neutral, stage-independent owner for the two-sink foundation as its slices land. It owns resolved controls, attempt records, protected persistence, projection, and redaction primitives; each adopted operation remains the single semantic owner of its attempt, computation, publication, rollback, recovery, streams, and exit. |
