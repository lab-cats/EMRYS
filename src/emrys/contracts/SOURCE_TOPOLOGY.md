...
| `SRC-TRANS-013` | `analyses/paired_cmh_candidate_ranking/__init__.py` → `emrys.reporting._run_report.context` | The built-in module preserves the exact retained paired-CMH scientific HTML while the collaborator boundary is introduced; this grants no neighboring reporting access. | `AC-SLICE-04` / `ANALYSIS-02`: move the paired-specific renderer and its presentation assets into the built-in module with output parity, bring the separately tracked sibling Step 10 implementation/assets under the declared implementation package/hash, then remove this edge. Until then the module-owner migration remains In progress. |
