...
| Application logging | `libraries/application_logging/` | This neutral, stage-independent two-sink foundation owns resolved controls, attempt records, protected persistence, projection, and redaction primitives. Grouped local-pilot `run`/`resume` control is the first production adopter: an execute owns exactly one compute-side application attempt, while scheduler submission transport and valid dry-run own none. Application logs default to `<project-root>/logs/application`; scheduler OUT/ERR remain separate under `<project-root>/logs`. The owner records analysis start, publication readiness before receipt-last publication, and receipt commitment afterward while retaining computation, rollback, recovery, streams, and exit authority. The packaged-Python production-import roster is mechanically guarded. |
