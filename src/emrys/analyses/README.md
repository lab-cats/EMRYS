...
- [`paired_cmh_candidate_ranking/`](paired_cmh_candidate_ranking/README.md)
  ...
  ranking analysis for historical Step `09`, plus the nested
  [`scientific_context_projection/`](paired_cmh_candidate_ranking/scientific_context_projection/README.md)
  owner of semantic `project_candidate_scientific_context`, the bounded
  post-Step09 reference, known-motif, logo-value, and enrichment projection for
  historical Step `10`.
...
ambiguous ID. Distribution/version, entry-point value, configuration-schema
hash, runtime requirements, and the selected implementation-package identity
are persisted or bound to the Run. Built-in and external providers cross the
same admission boundary; unselected installed modules do not affect identity.
...
- required check IDs from the admitted EMRYS runtime profile.

EMRYS derives the profile edges, canonical artifact IDs, adapter registry,
and evidence records. Every declared task has one standard validation report
and uses the existing atomic task publication and failure/recovery semantics.

Scientific rendering is deliberately outside this Run-hashed descriptor. An
installed distribution may expose the same module ID through a separate
...
Reporting loads that exact provider only after admitting the completed Run.
It receives immutable report-only contracts from `emrys.reporting` plus the
module's exact admitted result roster. Its distribution, entry point, package
content, returned inputs, and output are bound to the report receipt, never to
Analysis or Run identity. The provider owns bespoke scientific presentation;
fixed EMRYS reporting owns automatic invocation, opt-out, independent
regeneration, the evidence-and-operations view, generic safety/accessibility
validation, locking, rollback, receipt, and publication. There is no generic
report schema, section DSL, or second artifact authority.
