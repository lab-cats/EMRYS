...
def run(build: Callable[[], tuple[bytes, dict[Path, Snapshot]]], *, step_id: str, scope_id: str, check_ids: set[str], output: Path, execute: bool, published_label: str, before_report: Callable[[], None] | None = None, caught_errors: tuple[type[BaseException], ...] = (OSError, UnicodeError, csv.Error, ValidationError)) -> int:
    try:
