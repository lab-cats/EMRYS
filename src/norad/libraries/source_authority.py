"""Fail-closed filesystem authorities shared by NORAD functional owners.
...
def controlled_python_argv(
    ...
    *arguments: str,
    ...
    """Build the one Python launch prefix used by controlled NORAD children."""
...
def require_controlled_python_runtime() -> None:
    ...
    if sys.pycache_prefix != CONTROLLED_PYTHON_CACHE_PREFIX:
        raise SourceCheckoutError(
            "Controlled NORAD Python children require "
...
def _package_matches_checkout_revision(
    *,
    ...
    try:
        ...
        tree = ops.run_git(
            ["git", "ls-tree", "-r", "-z", revision, "--", "src/norad"],
            cwd=source_checkout.root,
            env=environment,
            check=True,
            capture_output=True,
