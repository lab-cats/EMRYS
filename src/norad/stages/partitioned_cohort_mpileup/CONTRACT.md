...
Distinct partitions may run independently when they use distinct output locks
and immutable shared inputs. The
...
VCFs are create-exclusive hard links to those already validated temporary
artifacts, and exact staging/final inode identity carries the validation across
publication without reparsing and recounting the VCFs. The legacy replacement
route retains final-path VCF revalidation. The receipt itself is checked inside
the owned rollback boundary; its mere presence is not independent proof of a
successfully completed immutable computation.
...
which proves the bytes consumed by both scientific pipelines remained stable.
Receipt construction and publication consume only the resulting VCFs, so the
producer does not rehash the entire large BAM/BAI cohort a third time merely
before publication; the small manifest hashes are still rechecked because they
are embedded directly in receipt identity. These hashes are in-attempt
stability guards; they are not added to the native receipt. Direct invocations
retain complete-set replacement and the legacy manifests-only stability
boundary unless the flag is supplied.
...
gating, delegation, module-state logging, and final path checks; it does not own
pileup or publication logic.
...
- [`test_step_07_bcftools_mpileup_by_chrom_and_strand.sh`](../../../../tests/stages/partitioned_cohort_mpileup/test_step_07_bcftools_mpileup_by_chrom_and_strand.sh)
  ...
  rollback failures, signals, mutation gaps, and provenance omissions. Legacy
  replacement still exercises final-path validation faults; no-clobber relies
  on the common create-exclusive ownership helpers for exact inode identity.
...
- Producer/validator selector detail remains asymmetric. The validator may
  publish failed rows with exit `0`, does not invoke bcftools, and does not
  prove selector-bound coordinates, VCF semantic fields, filter compliance,
  immutable inputs, or current-attempt identity.
