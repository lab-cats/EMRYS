...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *) handle_execute_or_help "$1"; shift ;;
    esac
done
...
view_99_command=("$samtools_bin" view -@ "$threads" -b -f 99 "$input_bam" -o "$tmp_99_bam")
view_147_command=("$samtools_bin" view -@ "$threads" -b -f 147 "$input_bam" -o "$tmp_147_bam")
view_83_command=("$samtools_bin" view -@ "$threads" -b -f 83 "$input_bam" -o "$tmp_83_bam")
view_163_command=("$samtools_bin" view -@ "$threads" -b -f 163 "$input_bam" -o "$tmp_163_bam")
merge_fwd_command=("$samtools_bin" merge -@ "$threads" -o "$tmp_fwd_bam" "$tmp_99_bam" "$tmp_147_bam")
merge_rev_command=("$samtools_bin" merge -@ "$threads" -o "$tmp_rev_bam" "$tmp_83_bam" "$tmp_163_bam")
index_fwd_command=("$samtools_bin" index "$tmp_fwd_bam")
index_rev_command=("$samtools_bin" index "$tmp_rev_bam")
input_count_command=("$samtools_bin" view -c "$input_bam")
flag_99_count_command=("$samtools_bin" view -c -f 99 "$input_bam")
flag_147_count_command=("$samtools_bin" view -c -f 147 "$input_bam")
flag_83_count_command=("$samtools_bin" view -c -f 83 "$input_bam")
flag_163_count_command=("$samtools_bin" view -c -f 163 "$input_bam")
fwd_count_command=("$samtools_bin" view -c "$tmp_fwd_bam")
rev_count_command=("$samtools_bin" view -c "$tmp_rev_bam")
...
confirm_final_set_state() {
    ...
    if [[ "$no_clobber" == true ]]; then
        [[ "$final_count" == "0" ]] || die "Step 06 --no-clobber requires all five final outputs to be absent; found: $final_count"
        previous_final_set_present=false
        return
    fi
...
rollback_publish() {
    if [[ "$backup_started" != true || "$final_publish_complete" == true ]]; then return 0; fi
    printf 'Rolling back Step 06 read-orientation outputs...\n' >&2
    if [[ "$no_clobber" == true ]]; then
        local rollback_ok=true
        remove_owned_published_file "Step 06 FWD BAM" "$tmp_fwd_bam" "$output_fwd_bam" || rollback_ok=false
        remove_owned_published_file "Step 06 FWD BAI" "$tmp_fwd_bai" "$output_fwd_bai" || rollback_ok=false
        remove_owned_published_file "Step 06 REV BAM" "$tmp_rev_bam" "$output_rev_bam" || rollback_ok=false
        remove_owned_published_file "Step 06 REV BAI" "$tmp_rev_bai" "$output_rev_bai" || rollback_ok=false
        remove_owned_published_file "Step 06 counts" "$tmp_counts_tsv" "$output_counts_tsv" || rollback_ok=false
        [[ "$rollback_ok" == true ]]
        return
    fi
    if [[ "$previous_final_set_present" == true ]]; then
        if [[ "$fwd_bam_backed_up" == true && -e "$backup_fwd_bam" ]]; then rm -f "$output_fwd_bam"; mv "$backup_fwd_bam" "$output_fwd_bam" || true; fwd_bam_backed_up=false; fi
        if [[ "$fwd_bai_backed_up" == true && -e "$backup_fwd_bai" ]]; then rm -f "$output_fwd_bai"; mv "$backup_fwd_bai" "$output_fwd_bai" || true; fwd_bai_backed_up=false; fi
        if [[ "$rev_bam_backed_up" == true && -e "$backup_rev_bam" ]]; then rm -f "$output_rev_bam"; mv "$backup_rev_bam" "$output_rev_bam" || true; rev_bam_backed_up=false; fi
        if [[ "$rev_bai_backed_up" == true && -e "$backup_rev_bai" ]]; then rm -f "$output_rev_bai"; mv "$backup_rev_bai" "$output_rev_bai" || true; rev_bai_backed_up=false; fi
        if [[ "$counts_tsv_backed_up" == true && -e "$backup_counts_tsv" ]]; then rm -f "$output_counts_tsv"; mv "$backup_counts_tsv" "$output_counts_tsv" || true; counts_tsv_backed_up=false; fi
    else
        rm -f "$output_fwd_bam" "$output_fwd_bai" "$output_rev_bam" "$output_rev_bai" "$output_counts_tsv"
    fi
}

cleanup() {
    local status="$1" rollback_ok=true
    set +e
    if [[ "$status" -ne 0 ]]; then rollback_publish || rollback_ok=false; fi
    if [[ "$rollback_ok" == true ]]; then
        rm -f "$tmp_99_bam" "$tmp_147_bam" "$tmp_83_bam" "$tmp_163_bam"
        rm -f "$tmp_fwd_bam" "$tmp_fwd_bai" "$tmp_rev_bam" "$tmp_rev_bai" "$tmp_counts_tsv"
        if [[ "$status" -eq 0 || "$backup_started" == true ]]; then
            rm -f "$backup_fwd_bam" "$backup_fwd_bai" "$backup_rev_bam" "$backup_rev_bai" "$backup_counts_tsv"
        fi
        remove_owned_lock
    else
...
refuse_stale_paths() {
    local path
    for path in "$tmp_99_bam" "$tmp_147_bam" "$tmp_83_bam" "$tmp_163_bam" "$tmp_fwd_bam" "$tmp_fwd_bai" "$tmp_rev_bam" "$tmp_rev_bai" "$tmp_counts_tsv" "$backup_fwd_bam" "$backup_fwd_bai" "$backup_rev_bam" "$backup_rev_bai" "$backup_counts_tsv"; do
        [[ ! -e "$path" ]] || die "Refusing to reuse stale Step 06 path: $path"
    done
}

validate_orientation_outputs() {
    local fwd_bam="$1" fwd_bai="$2" rev_bam="$3" rev_bai="$4" counts_tsv="$5" label="$6"
...
write_counts_tsv() {
    local input_records flag_99_records flag_147_records flag_83_records flag_163_records fwd_like_records rev_like_records assigned_records unassigned_records assigned_fraction
    ...
    (( assigned_records <= input_records )) || die "assigned_records exceeds input_records: $assigned_records > $input_records"
    ...
    assigned_fraction="$(awk -v assigned="$assigned_records" -v input="$input_records" 'BEGIN { printf "%.6f", assigned / input }')"
    {
        ...
        printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$sample_id" "$input_records" "$flag_99_records" "$flag_147_records" "$flag_83_records" "$flag_163_records" "$fwd_like_records" "$rev_like_records" "$assigned_records" "$unassigned_records" "$assigned_fraction"
    } > "$tmp_counts_tsv"
}
...
mode="dry-run"; [[ "$execute" == true ]] && mode="execute"
...
print_command "${input_count_command[@]}"; print_command "${flag_99_count_command[@]}"; print_command "${flag_147_count_command[@]}"; print_command "${flag_83_count_command[@]}"; print_command "${flag_163_count_command[@]}"; print_command "${fwd_count_command[@]}"; print_command "${rev_count_command[@]}"
...
printf '  4. Under --no-clobber, prove finals are the validated staging inodes; legacy replacement mode revalidates final paths.\n'
...
if [[ "$no_clobber" == true ]]; then
    require_no_owner_residue "Step 06" "$output_dir" ".${sample_id}.step06.*"
    require_no_owner_residue "Step 06" "$qc_dir" ".${sample_id}.step06.*"
fi
if [[ "$execute" != true ]]; then printf 'Dry-run only. Add --execute to run samtools and publish Step 06 outputs.\n'; exit 0; fi

mkdir -p "$output_dir" "$qc_dir"
refuse_stale_paths
on_exit() { local status=$?; trap - EXIT HUP INT TERM; cleanup "$status"; exit "$status"; }
trap on_exit EXIT HUP INT TERM
acquire_lock "Step 06"
if [[ "$no_clobber" == true ]]; then confirm_final_set_state; fi
printf 'samtools version:\n'; "$samtools_bin" --version
"${view_99_command[@]}"; "${view_147_command[@]}"; "${view_83_command[@]}"; "${view_163_command[@]}"
"${merge_fwd_command[@]}"; "${merge_rev_command[@]}"; "${index_fwd_command[@]}"; "${index_rev_command[@]}"
write_counts_tsv
validate_orientation_outputs "$tmp_fwd_bam" "$tmp_fwd_bai" "$tmp_rev_bam" "$tmp_rev_bai" "$tmp_counts_tsv" "Replacement"
if [[ "$no_clobber" == true ]]; then
...
if [[ "$previous_final_set_present" == true ]]; then
    backup_started=true
    mv "$output_fwd_bam" "$backup_fwd_bam"; fwd_bam_backed_up=true
    mv "$output_fwd_bai" "$backup_fwd_bai"; fwd_bai_backed_up=true
    mv "$output_rev_bam" "$backup_rev_bam"; rev_bam_backed_up=true
    mv "$output_rev_bai" "$backup_rev_bai"; rev_bai_backed_up=true
    mv "$output_counts_tsv" "$backup_counts_tsv"; counts_tsv_backed_up=true
else
    backup_started=true
fi

if [[ "$no_clobber" == true ]]; then
    publish_file_create_exclusive "Step 06 FWD BAM" "$tmp_fwd_bam" "$output_fwd_bam"
    publish_file_create_exclusive "Step 06 FWD BAI" "$tmp_fwd_bai" "$output_fwd_bai"
    publish_file_create_exclusive "Step 06 REV BAM" "$tmp_rev_bam" "$output_rev_bam"
    publish_file_create_exclusive "Step 06 REV BAI" "$tmp_rev_bai" "$output_rev_bai"
    publish_file_create_exclusive "Step 06 counts" "$tmp_counts_tsv" "$output_counts_tsv"
    require_owned_published_file "Step 06 FWD BAM" "$tmp_fwd_bam" "$output_fwd_bam"
    require_owned_published_file "Step 06 FWD BAI" "$tmp_fwd_bai" "$output_fwd_bai"
    require_owned_published_file "Step 06 REV BAM" "$tmp_rev_bam" "$output_rev_bam"
    require_owned_published_file "Step 06 REV BAI" "$tmp_rev_bai" "$output_rev_bai"
    require_owned_published_file "Step 06 counts" "$tmp_counts_tsv" "$output_counts_tsv"
else
    ...
    validate_orientation_outputs "$output_fwd_bam" "$output_fwd_bai" "$output_rev_bam" "$output_rev_bai" "$output_counts_tsv" "Published"
fi

if [[ "$no_clobber" == true ]]; then
    rm -f -- "$tmp_fwd_bam" "$tmp_fwd_bai" "$tmp_rev_bam" "$tmp_rev_bai" "$tmp_counts_tsv"
    [[ ! -e "$tmp_fwd_bam" && ! -L "$tmp_fwd_bam" && ! -e "$tmp_fwd_bai" && ! -L "$tmp_fwd_bai" && ! -e "$tmp_rev_bam" && ! -L "$tmp_rev_bam" && ! -e "$tmp_rev_bai" && ! -L "$tmp_rev_bai" && ! -e "$tmp_counts_tsv" && ! -L "$tmp_counts_tsv" ]] || die "Step 06 could not remove owned publication anchors."
fi
