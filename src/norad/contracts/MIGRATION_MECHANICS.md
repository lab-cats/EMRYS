...
| R | Compare direct and `Rscript` invocation where supported, arbitrary working directory, arguments, guarded environment selection, stdout/stderr, exit status, exact tables and plots, deterministic ordering/serialization, file mode, and the applicable independent scientific corpus or guarded real-R suite. Preserve statistical method, thresholds, provisional orientation language, and dependency policy; relocation never restores or installs packages. | Prefer direct caller cutover because an R forwarding file can change `commandArgs`, source paths, call frames, and exit behavior. Use a temporary R wrapper only for a named caller after those effects pass parity, and keep it free of library/bootstrap logic. Roll back SLURM/shell callers before the R path, restore the recorded execution mode, and reuse the pre-move locked environment rather than changing dependencies. |
| SLURM | Compare shebang, exact `#SBATCH` directives, strict mode, file mode, `EXECUTE` validation, submit/caller working-directory behavior, module calls and failure policy, environment/default resolution, delegation arguments, dry-run side effects, final-path checks, logs, and child exit using the independent mocked-wrapper suite plus syntax checks. Local mocks remain local evidence; do not claim scheduler or cluster parity without separately authorized cluster inspection. | Move the delegated implementation first and make the existing job invoke its final path. Then move the SLURM file and all known submission callers atomically. Do not leave two files carrying duplicated scheduler directives. A legacy SLURM wrapper is allowed only when it can be directive-free and parity-preserving; otherwise an unmovable named submitter blocks cutover. Roll back submitters and the job path before the delegated implementation. |
