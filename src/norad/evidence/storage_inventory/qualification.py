...
SCHEMA = "norad.storage-qualification.v1"
EVIDENCE_DIRECTORY = ".norad-storage-qualification"
CHECKS = (
...
def _probe_root(
    ...
    probe = root / f".norad-storage-probe-{qualification_id[:16]}-{role}"
    try:
...
def _validate_compute(
    ...
    for role, root, row in zip(ROLES, roots, rows, strict=True):
        ...
        expected_probe = root / (
            f".norad-storage-probe-{qualification_id[:16]}-{role}"
        )
