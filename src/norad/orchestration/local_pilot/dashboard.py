#!/usr/bin/python3
"""Responsive, read-only NORAD/Slurm live dashboard v4.9.

This is an operational view over append-only scheduler streams. It does not
replace NORAD inspection or completion evidence.
"""
...
STAGES = [
    ...
    ("02", "Canonical BAM", 6, "construct_canonical_BAM",
     "Normalizes each STAR alignment into NORAD's indexed canonical BAM. This "
    ...
    ("03", "RSeQC orientation", 6, "collect_RSeQC_paired_orientation_evidence",
     ...
     "model and infer library orientation. NORAD records the observed orientation "
    ...
    ("FINAL", "Aggregate target", 1, "local_pipeline_slice",
     ...
     "state. Final NORAD inspection remains the completion authority after Slurm exits.",
...
def accounting_log_selection(job_id, log_dir, metadata=None):
    ...
    selection = validate_log_selection(
        job_id,
        os.path.join(log_dir, "norad-local-pilot-%s.out" % job_id),
        os.path.join(log_dir, "norad-local-pilot-%s.err" % job_id),
    )
...
def validate_log_selection(job_id, out_path, err_path, allow_missing=False):
    ...
    expected_out = "norad-local-pilot-%s.out" % job_id
    expected_err = "norad-local-pilot-%s.err" % job_id
    out_path = os.path.abspath(out_path)
    err_path = os.path.abspath(err_path)
    if os.path.basename(out_path) != expected_out:
        raise DiscoveryError("stdout does not match the NORAD wrapper contract: %s" % out_path)
    if os.path.basename(err_path) != expected_err:
        raise DiscoveryError("stderr does not match the NORAD wrapper contract: %s" % err_path)
...
def resolve_selection(job_id=None, log_dir=None, out_path=None, err_path=None,
    ...
    raise DiscoveryError(
        "no recent current-user NORAD wrapper job could be proven%s; "
...
def configuration_text(identity):
    ...
    return "not yet reported by the NORAD control plan"
...
def stage_resource_text(stage, identity, fallback):
    ...
    if stage in {
        ...
        return "Resource plan not yet reported by the NORAD control stream."
    return fallback
...
def activity_lines(model, slurm, identity, now):
    ...
    if terminal:
        ...
        lines.append(
            "Next: run final NORAD inspection; this dashboard is not completion authority."
        )
...
def render_header(screen, job_id, view, attrs):
    safe_add(screen, 0, 1, "NORAD LIVE DASHBOARD v4.9", attrs["title"])
...
def render_overview(screen, job_id, slurm, identity, model,
    ...
    if height < 20 or width < 72:
        safe_add(screen, 0, 0, "NORAD LIVE DASHBOARD v4.9", attrs["title"])
...
def snapshot(job_id, slurm, identity, model):
    now = time.time()
    complete, total, remaining = progress_values(model)
    print("NORAD LIVE DASHBOARD v4.9 snapshot | job %s" % job_id)
...
def parse_args(argv):
    parser = argparse.ArgumentParser(description="Responsive NORAD/Slurm live dashboard")
    ...
    env_job = os.environ.get("NORAD_DASHBOARD_JOB_ID", "").strip()
    env_log_dir = os.environ.get("NORAD_DASHBOARD_LOG_DIR", "").strip()
