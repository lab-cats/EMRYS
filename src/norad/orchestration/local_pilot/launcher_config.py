...
ADJACENT_CONFIG_NAME = "norad.launcher.yaml"
DOTENV_NAME = ".env"
BATCH_MARKER = "norad-local-pilot-batch-v1"
...
_FIELD_ENV_NAMES = {
    ("slurm", "account"): "NORAD_SLURM_ACCOUNT",
    ("slurm", "partition"): "NORAD_SLURM_PARTITION",
    ("slurm", "qos"): "NORAD_SLURM_QOS",
    ("slurm", "cpus_per_task"): "NORAD_SLURM_CPUS",
    ("slurm", "memory"): "NORAD_SLURM_MEMORY",
    ("slurm", "time"): "NORAD_SLURM_TIME",
    ("slurm", "exclusive"): "NORAD_SLURM_EXCLUSIVE",
    ("slurm", "nodelist"): "NORAD_SLURM_NODELIST",
    ("paths", "log_dir"): "NORAD_LOG_DIR",
    ("paths", "request"): "NORAD_REQUEST",
    ("paths", "workspace"): "NORAD_WORKSPACE",
    ("paths", "runtime_profile"): "NORAD_RUNTIME_PROFILE",
    ("paths", "scratch_parent"): "NORAD_SCRATCH_PARENT",
    ("modules", "mode"): "NORAD_MODULE_MODE",
    ("modules", "init"): "NORAD_MODULE_INIT",
    ("modules", "load"): "NORAD_MODULES",
}
...
def _submission_command(
    ...
    *,
    ...
    fields = (
        ...
        _export_value("NORAD_SUBMIT_UID", str(live_uid)),
        _export_value("NORAD_SUBMIT_USER", live_user),
        _export_value("USER", live_user),
        _export_value("LOGNAME", live_user),
        _export_value("NORAD_SLURM_CPUS", str(plan.cpus_per_task)),
        _export_value("NORAD_SOURCE_CHECKOUT", str(source_checkout)),
        _export_value("NORAD_PYTHON", str(workflow_python)),
        _export_value("NORAD_REQUEST", str(plan.request)),
        _export_value("NORAD_WORKSPACE", str(plan.workspace)),
        _export_value("NORAD_RUNTIME_PROFILE", str(plan.runtime_profile)),
        _export_value("NORAD_MODULE_MODE", plan.module_mode),
        _export_value("NORAD_MODULE_INIT", plan.module_init),
        _export_value("NORAD_MODULES", ":".join(plan.modules)),
        _export_value("NORAD_SCRATCH_PARENT", str(plan.scratch_parent)),
        _export_value("NORAD_EXECUTE", "1" if execute else "0"),
    )
    command = [
        sbatch,
        "--parsable",
    ...
    command.extend(
        (
            ...
            f"--output={plan.log_dir}/norad-local-pilot-%j.out",
            f"--error={plan.log_dir}/norad-local-pilot-%j.err",
            "--export=" + ",".join(fields),
            str(wrapper),
            BATCH_MARKER,
        )
...
def submit_from_args(
    ...
    *,
    ...
    source_checkout = _generation_path(
        selected_environment,
        "NORAD_LAUNCHER_SOURCE_CHECKOUT",
        directory=True,
    )
    workflow_python = _generation_path(
        selected_environment,
        "NORAD_LAUNCHER_PYTHON",
        directory=False,
    )
    ...
    submission_environment = {
        ...
        if not name.startswith("SBATCH_") and name != "NORAD_EXECUTE"
    }
    try:
    ...
    stdout_path = plan.log_dir / f"norad-local-pilot-{job_id}.out"
    stderr_path = plan.log_dir / f"norad-local-pilot-{job_id}.err"
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Submit one generated NORAD local-pilot allocation."
    )
