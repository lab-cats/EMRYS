...
def _require_external_absent_output(value: str | Path, root: Path) -> Path:
    ...
    if output == root or output in root.parents or root in output.parents:
        raise OnboardingError(
            f"output directory must not overlap the NORAD checkout: {output}"
        )
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    ...
    NORAD_TOOL_THREADS="${NORAD_TOOL_THREADS:-$NORAD_SLURM_CPUS}"
    NORAD_SAMPLE_CONCURRENCY="${NORAD_SAMPLE_CONCURRENCY:-1}"
    [[ "$NORAD_SLURM_CPUS" =~ ^[1-9][0-9]*$ ]] || die "NORAD_SLURM_CPUS must be a positive integer"
    [[ "$NORAD_TOOL_THREADS" =~ ^[1-9][0-9]*$ ]] || die "NORAD_TOOL_THREADS must be a positive integer"
    [[ "$NORAD_SAMPLE_CONCURRENCY" =~ ^[1-9][0-9]*$ ]] || die "NORAD_SAMPLE_CONCURRENCY must be a positive integer"
    (( NORAD_TOOL_THREADS <= NORAD_SLURM_CPUS )) || die "NORAD_TOOL_THREADS cannot exceed NORAD_SLURM_CPUS"
    (( NORAD_SAMPLE_CONCURRENCY <= NORAD_SLURM_CPUS )) || die "NORAD_SAMPLE_CONCURRENCY cannot exceed NORAD_SLURM_CPUS"
    ...
    export_spec="NORAD_SLURM_CPUS=$NORAD_SLURM_CPUS,NORAD_TOOL_THREADS=$NORAD_TOOL_THREADS,NORAD_SAMPLE_CONCURRENCY=$NORAD_SAMPLE_CONCURRENCY,NORAD_SOURCE_CHECKOUT=$NORAD_SOURCE_CHECKOUT,NORAD_PYTHON=$NORAD_PYTHON,NORAD_REQUEST=$NORAD_REQUEST,NORAD_WORKSPACE=$NORAD_WORKSPACE,NORAD_RUNTIME_PROFILE=$NORAD_RUNTIME_PROFILE,NORAD_MODULE_INIT=$NORAD_MODULE_INIT,NORAD_MODULES=$NORAD_MODULES,NORAD_EXECUTE=$NORAD_EXECUTE"
...
NORAD_TOOL_THREADS="${NORAD_TOOL_THREADS:-$NORAD_SLURM_CPUS}"
NORAD_SAMPLE_CONCURRENCY="${NORAD_SAMPLE_CONCURRENCY:-1}"
for name in NORAD_SLURM_CPUS NORAD_TOOL_THREADS NORAD_SAMPLE_CONCURRENCY \
    NORAD_SOURCE_CHECKOUT NORAD_PYTHON NORAD_REQUEST NORAD_WORKSPACE \
    NORAD_RUNTIME_PROFILE NORAD_MODULE_INIT NORAD_MODULES NORAD_EXECUTE; do
    require_value "$name"
done
...
"$NORAD_PYTHON" -X pycache_prefix=/dev/null -I -m norad doctor local-pilot \
...
run_arguments=(
    ...
    --workspace "$NORAD_WORKSPACE"
    --runtime-profile "$NORAD_RUNTIME_PROFILE"
    --threads "$NORAD_TOOL_THREADS"
    --workflow-cores "$NORAD_SLURM_CPUS"
    --sample-concurrency "$NORAD_SAMPLE_CONCURRENCY"
)
...
def starter_members(
    *,
    ...
    runtime = runtime.replace(
        "/absolute/path/to/norad/.venv/bin/python", str(selected_python)
