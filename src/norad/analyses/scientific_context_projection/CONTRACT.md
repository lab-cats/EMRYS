...
public identity is `norad.analysis.project_candidate_scientific_context.v1`;
...
Execute mode uses the bound `NORAD_SHA256_PYTHON` launcher, or an absolute
...
`norad.contracts.scientific_evidence.scientific_context`. The receipt binds
...
The grouped `python -I -m norad validate scientific-context-projection` route
