...
1. Finish the documentation-only strategy-card bootstrap package, then verify
   it is clean, pushed, and upstream-equal. Card creation records future work;
   it does not select or authorize any of the new cards.
2. Separately plan and, only after approval, implement
   [`CONCURRENCY-02`](docs/tasks/TODO/CONCURRENCY-02-define-integration-fragment-protocol.md)
   to establish the manual integration-fragment protocol and a synthetic,
   non-substantive exchange record without reviewing the preserved pilot.
3. Then separately plan
   [`PROGRAM-01`](docs/tasks/TODO/PROGRAM-01-define-rolling-wave-planning-and-coordination-cohorts.md)
   and reconcile the existing refactor plan, tranche contract, and lifecycle
   semantics under its separately approved execution package.
4. Reassess the maintenance and main-refactor order after `PROGRAM-01`. The
   likely infrastructure sequence begins with `DOC-GATE-01`, followed by the
   dependency-valid portions of `CONCURRENCY-03`, `TASK-LIFECYCLE-01`, and
   `TASK-EPIC-01`; this is roadmap guidance, not blocker metadata or task
   authorization. The preserved pilot is the intended first substantive
   integration candidate, but only through the separate card generated by
   `PROGRAM-01` after the applicable protocol, validator, enforcement, and
   lifecycle infrastructure are complete.

The inherited main-refactor baseline remains `TEST-01C` through `TEST-01F`,
...
`PLAN-02Z`. `PROGRAM-01` must confirm or revise that main-refactor sequence
before a post-program delivery card is selected; this does not prevent the
separately approved `CONCURRENCY-02` policy package that precedes it.
