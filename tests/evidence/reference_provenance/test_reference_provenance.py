...
def test_parser_error_is_role_local_and_preserves_other_contig_rows(
    ...
    contigs = list(
        csv.DictReader(generated["contigs"].decode().splitlines(), delimiter="\t")
    )
    parser_rows = [
...
def test_reports_mismatch_missing_and_hash_mismatch(tmp_path: Path) -> None:
    ...
    inventory.write_text(
        text.replace(
            "\tNA\tsource\trelease1\tfasta",
            "\t" + "0" * 64 + "\tsource\trelease1\tfasta",
            1,
        )
    )
    output = tmp_path / "out"
...
def test_publication_failure_restores_complete_reference_predecessor(
    ...
    assert not [
        child for child in (output / "ref1").iterdir() if child.name.startswith(".")
    ]
