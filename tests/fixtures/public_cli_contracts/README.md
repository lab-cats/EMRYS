...
tests for bare and absolute `make` and `gmake` forms. GNU Make `3.81` and `4.3`
differ by one recipe-prefix tab when printing continued recipe lines in dry-run
mode. The test therefore accepts either the committed literal rendering or the
complete rendering with exactly one leading tab removed from every indented
line; mixed indentation is rejected. Commands, arguments, ordering, quoting,
versions, line boundaries, and nested target expansion remain literal. The test
reads this committed fixture; it does not derive expected commands from the
Makefile under test.
