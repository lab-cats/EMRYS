...
def test_tampered_prior_receipt_provenance_is_rejected(
    ...
    with pytest.raises(
        (
            ...
            REPORTING_VALIDATION.ReportingTransactionError,
        ),
        match=token,
    ):
        REPORTING_VALIDATION.validate_run_summary_transaction(
            source_checkout=REPO_ROOT,
            artifact_source_root=run_summary_fixture.root,
            run_id=run_summary_fixture.run_id,
            artifact_receipt=run_summary_fixture.artifact_receipt,
...
def test_combined_readmission_keeps_summary_json_and_views_byte_identical(
    ...
    assert result.receipt_path == run_summary_fixture.summary_receipt_path
