...
```text
Steps 00a-05 cluster-proven
-> Step 06 implemented/local-tested, ABE_EV_3 cluster-validated, cohort validation pending
...
- Reference prep and sample preprocessing are cluster-proven through Step `05` across the six-sample cohort.
- Step `06` is implemented and locally tested, cluster-validated on `ABE_EV_3`, and pending cohort validation.
...
- Do not call Step `06` cohort-proven unless validation has completed and docs say so.
