...
The command resolves exactly
`/absolute/path/to/emrys.execution.site.yaml`. Use the mutually exclusive
`--execution-profile /absolute/path/to/profile.yaml` for an arbitrary admitted
path. EMRYS does not scan or consult a registry.
