...
| Inspection prints `resume_available` and `Resume available: yes` | Run `.venv/bin/python -X pycache_prefix=/dev/null -I -m emrys resume --run-root ... --runtime-profile ...` without `--execute`; review reusable and pending jobs; then add `--execute` to that resume command. EMRYS re-admits completed work before reuse. |
