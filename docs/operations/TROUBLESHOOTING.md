...
| Readiness passed, but execution has not started | Run `.venv/bin/python -X pycache_prefix=/dev/null -I -m emrys run ...` without `--execute` and review the complete no-write plan. Add `--execute` only to the identical admitted request after review. |
...
| Scientific Results are `complete` | Scientific resume refusal is expected. Reporting may separately be complete, incomplete, or blocked. Use admitted Results and verified report locations; a new analysis requires a separately admitted request and Run identity. |
