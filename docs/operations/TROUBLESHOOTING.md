...
| --- | --- |
| Doctor prints `NOT READY` | Read every `BLOCKER` and `REMEDIATION` from `.venv/bin/python -X pycache_prefix=/dev/null -I -m emrys doctor local-pilot ...`. Correct the declared input, checkout, workspace, or runtime outside the doctor, then rerun it. Do not start `emrys run`. |
...
| Readiness passed, but execution has not started | Run `.venv/bin/python -X pycache_prefix=/dev/null -I -m emrys run ...` without `--execute` and review the complete no-write plan. Add `--execute` only to the identical admitted Project after review. |
...
| Inspection prints incomplete scientific Results, a failed or interrupted Attempt, and `Recovery available: yes` | Run `.venv/bin/python -X pycache_prefix=/dev/null -I -m emrys resume --run-root ... --runtime-profile ...` without `--execute`; review reusable and pending jobs; then add `--execute` to that resume command. EMRYS re-admits completed work before reuse. |
...
| Scientific Results are `complete` | Scientific resume refusal is expected. Reporting may separately be complete, incomplete, or blocked. Use admitted Results and verified report locations; a new Analysis requires a separately admitted Project revision and Run identity. |
...
| Project contains an unknown/duplicate field or YAML merge | The temporary Project-adapter schema is intentionally closed and merge expansion is disabled. | Start from the matched starter and author each literal supported field once. Do not use anchors, templates, or environment interpolation. |
...
| Workspace already exists | Initial publication is create-absent and refuses adoption. | Inspect it if it is an EMRYS workspace. Otherwise choose a different absent child under an existing writable parent; do not delete an uncertain directory. |
| Workspace or reference sidecars are on unqualified storage | Locking, hard-link, rename, visibility, durability, or head/compute access is not admitted. | Run both phases of `emrys inspect storage-qualification` for the exact workspace/reference pair. Stop if it fails; scheduler availability is not storage support. |
...
| `TMPDIR [/local/tmp] is not writeable` | The inherited temporary directory is unusable. | Set generated-wrapper `EMRYS_SCRATCH_PARENT` to an existing writable compute path. Confirm the logged private `TMPDIR`, filesystem, and capacity; do not rely on fallback `/tmp`. |

## Result and report questions

| Observation | Interpretation |
| --- | --- |
| Standalone Steps `07`–`09` passed but no HTML reports exist | Expected: standalone wrappers publish native outputs and validation TSVs only. They are not adopted into orchestration state. Use `emrys run` for automatic reporting; `emrys build report` requires an existing canonical run summary. |
