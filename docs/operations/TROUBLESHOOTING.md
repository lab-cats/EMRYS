...
| Project root or reference sidecars are on unqualified storage | Locking, hard-link, rename, visibility, durability, or head/compute access is not admitted. | Run both phases of `emrys inspect storage-qualification` for the exact Project-root/reference pair. Stop if it fails; scheduler availability is not storage support. |
