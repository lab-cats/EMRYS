...
| --- | --- |
| Standalone Steps `07`–`09` passed but no HTML reports exist | Expected: standalone wrappers publish native outputs and validation TSVs only. They are not adopted into orchestration state and are not eligible for `emrys report`. Use `emrys run` for an immutable Run with automatic reporting. |
