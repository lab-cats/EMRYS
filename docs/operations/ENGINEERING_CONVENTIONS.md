...
| [`pyproject.toml`](../../pyproject.toml) | Build metadata for the unreleased internal `norad-rna-workflow` distribution and the canonical Ruff configuration. It discovers only explicit packages and ships only named schemas/report resources without restoring runtime dependencies. Migrated commands use the grouped module interface through the selected installed interpreter in isolated mode; unmigrated owner directories and commands enter the distribution only through an owner-local cutover. |
