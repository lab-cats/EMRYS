...
Project-owned admitted runtime path. Initial Pixi Managed-environment resources
...
update qualification remain Open with Managed installation/repair, Explicit
...
| `AC-SLICE-08` | Execution configuration | `4` | `4` | Decide whether named execution profiles are accepted and, if so, define them independently of runtime acquisition modes | One v1 file-bound format, built-in direct default, explicit selector, current precedence, and source/effective provenance are implemented. Run-bound resources, Attempt-local placement, and runtime provisioning remain distinct. Institution-provided runtime discovery now publishes one Project-owned authority; named execution profiles, Managed install/repair, Explicit definition, and broader management remain Open with `OPS-01` and `RUNTIME-01` | The discovery route and canonical path are settled; remaining acquisition names and APIs are unselected |
...
| `AC-SLICE-13` | Golden path | `5` | `5` | Deliver a supported fresh-install-to-valid-synthetic-result golden path after ratifying its capability order | Supplied-path manifest drafting is complete under `SETUP-01`; guided Project setup, runtime admission/provisioning, Doctor readiness/repair, and the integrated golden-path proof remain | Capstone acceptance outcome, not an atomic implementation package |
...
| `AC-SLICE-19` | Doctor | `4` | `5` | Define Doctor repair ownership, supported mutations, preview/reporting, and safety contracts | Expansion of `DOCTOR-01`; Doctor delegates dependency solving/installation to established package managers | Doctor verifies explicit operator authority and owns orchestration, provenance, reporting, and requalification; exact managers/actions remain Open |
