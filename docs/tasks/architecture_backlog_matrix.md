...
| `AC-SLICE-03` | Public model | `5` | `4` | Establish the compact public application model and introduce it only after exact field, identity, authority, recovery, compatibility, and retirement decisions | Audit, model/boundary, and semantic decisions completed as `ARCH-MODEL-AUDIT-01`, `ARCH-MODEL-DECISION-01`, and `ARCH-MODEL-FIELDS-01`; successor Run-authority, projection-retirement, separated read-only status, grouped-CLI Run-control, Execution-Plan-derived Attempt provenance, and the first narrow Project vertical are implemented; card remains Open | `project.yaml`/`--project` now admit immutable `ProjectAdmission.analysis` into the existing Analysis/Execution-Plan/Run/Results path without a new schema or compatibility alias. Final Project nesting/persistence, broader public Analysis/Results APIs and role disclosure, generalized backend/policy boundaries, and remaining migrations remain Open. |
