...
| `AC-SLICE-08` | Execution configuration | `4` | `4` | Decide whether named execution profiles are accepted and, if so, define them independently of runtime acquisition modes | **Implementation complete; Verification pending.** `run`/`resume --profile NAME` resolve one safe Project-local `<project-root>/emrys.execution.NAME.yaml` fragment; the mutually exclusive explicit-path selector remains supported. Both use the existing v1 admission and source/effective provenance. A new Run or resource-declaring resume uses default → fragment → CLI-resource precedence; placement-only resume preserves predecessor resources before CLI overrides and explicit resources must match immutable Run identity. The selector name adds no schema, registry, identity, runtime mode, or automatic search. Site/global profile management remains `OPS-01`; runtime acquisition remains `RUNTIME-01`. | Focused local gates pass; exact-head aggregate and selected 130-pair hosted evidence remain required for terminal closure. |
