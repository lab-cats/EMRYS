...
| `AC-SLICE-04` | Scientific boundary | `3` | `4` | **Complete.** Retain private `TaskDispatch`; do not add a universal Stage/Operation representation. Step `08` now uses one owner-local Python coordinator while native R science and independent validation remain. | Completed; the wider collaborator boundary subsequently completed under `ANALYSIS-02`: paired Step 09/10 computation and assets share one implementation identity, while scientific report-provider identity remains report-only and fixed EMRYS reporting owns generic evidence/operations. Broad `ARCH-01` remains Open. | Transformation, scientific-analysis, and evidence owners mapped; reporting remains separate from immutable scientific computation identity. Caller, provenance, docs, and tests migrated without a compatibility wrapper or retained-evidence deletion. |
