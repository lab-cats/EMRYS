...
| `AC-SLICE-03` | Public model | `5` | `4` | Establish the compact public application model and introduce it only after exact field, identity, authority, recovery, compatibility, and retirement decisions | Audit, model/boundary, and semantic decisions completed as `ARCH-MODEL-AUDIT-01`, `ARCH-MODEL-DECISION-01`, and `ARCH-MODEL-FIELDS-01`; successor Run-authority, projection-retirement, separated read-only status, grouped-CLI Run-control, and immutable current request-to-Analysis intake boundaries are implemented; card remains Open | Immutable current intake and Analysis/Execution-Plan/Run records, Run-last persistence, direct successor Run admission, Attempt-owned reporting inputs, historical read/resume, independent Run/Attempt/Results/reporting/recovery inspection, and one supported grouped-CLI Run-control surface are implemented. Public Project/Analysis/Results representation and control, generalized backend/policy boundaries, and remaining migrations remain Open |
