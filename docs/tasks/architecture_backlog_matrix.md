...
A subsequent execution-boundary cut makes the current topology explicit without
selecting a profile API: successor Attempt executor provenance comes from the
immutable Execution Plan, direct versus Slurm placement is structured
Attempt-local allocation provenance, historical allocation records remain
readable, and the Slurm wrapper delegates readiness to grouped Run control.
A controlled proof now establishes exact planning/materialization parity when
effective resource resolution is held constant and only direct versus larger
admitted Slurm allocation changes. Allocation-sensitive effective-resource,
distinct-Attempt outcome, real scheduler/site, runtime/module,
failure/recovery, and report-publication parity remain open, so this does not
complete or rerank `AC-SLICE-05` or `AC-SLICE-08`.
...
| `AC-SLICE-05` | Execution | `4` | `5` | Ratify the execution guarantee contract, select the minimum justified capability boundary, and prove equivalent declared guarantees across direct and Slurm-placed realizations of each supported backend | Current local-backend/outer-Slurm boundary, Execution-Plan-derived Attempt executor, structured allocation provenance, wrapper delegation, and controlled planning/materialization parity with equal effective resource resolution are implemented. Allocation-sensitive effective-resource, distinct-Attempt outcome, real scheduler/site, runtime/module, failure/recovery, and report-publication parity plus any additional backend remain Open with `OPS-02` | Remaining cross-placement integration and outcome parity drive the estimate; no second backend or API is preselected |
...
| `AC-SLICE-08` | Execution configuration | `4` | `4` | Define named execution profiles independently of Managed/Site/Explicit runtime modes | Backend semantics and Attempt-local placement provenance are separated; profile taxonomy, persistence, precedence, selection, and public surface remain Open with `OPS-01` and `RUNTIME-01` | Current groundwork intentionally adds no profile class, registry, command, or selector |
...
| `AC-SLICE-10` | Operations | `4` | `4` | Define high-level status and safe resume/recovery UX over existing fail-closed internals | Read-only domain separation, recovery gating, and deterministic safe next-action guidance are implemented. The focused audit fixes persisted EMRYS records as authority, current/latest Attempt elapsed semantics, no inferred ETA, and progressive disclosure; milestone implementation remains Open with `OBS-02`, `OBS-01`, and `LOG-05` | The stale dashboard is frozen outside this slice and its retirement is deferred until campaign completion; recovery and fault evidence remain part of closure |
...
| `AC-SLICE-17` | Retirement | `3` | `5` | Retire duplicated lifecycle, validator, infrastructure, adapter, shell, or compatibility paths after each replacement is proven; retained evidence may be deleted only after an exact scoped proposal and the user's explicit approval | The focused register now identifies low-risk same-process checks, immutable-handoff and inspection consolidation, individual-operation Slurm retirement, generated-wrapper reduction, conditional Steps 07–09 Python ownership, and a stale-dashboard retirement candidate deferred until the campaign is complete. These remain bounded implementation slices, never one cleanup task | Low-risk impossible-state protections need no artificial replacement; high-risk, ambiguous, user-facing, execution-boundary, or evidence-validation changes require explicit approval; any approved evidence deletion stays isolated and never offsets product growth |
