...
| `AC-SLICE-10` | Operations | `4` | `4` | Define high-level status and safe resume/recovery UX over existing fail-closed internals | Read-only domain separation, recovery gating, and deterministic safe next-action guidance are implemented; milestones, elapsed time, and remaining role-aware UX stay Open with `OBS-02` | Recovery and fault evidence are part of closure |
