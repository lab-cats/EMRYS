...
`emrys.execution-profile.v1` document. `--profile NAME` resolves exactly one
safe Project-local `<project-root>/emrys.execution.NAME.yaml` file; the
mutually exclusive `--execution-profile PATH` remains supported. Omission uses
packaged direct-placement defaults for a new Run. Resume omission or a selected
placement-only fragment preserves predecessor resources before CLI overrides;
a resource-declaring fragment uses built-in → fragment → CLI precedence and
must match immutable Run identity. The name adds no schema, registry, identity,
runtime mode, or automatic search.
...
| `AC-SLICE-04` | Scientific boundary | `3` | `4` | **Complete.** Retain private `TaskDispatch`; do not add a universal Stage/Operation representation. Step `08` now uses one owner-local Python coordinator while native R science and independent validation remain. | Completed; the wider collaborator extension remains In progress under `ANALYSIS-02` because the built-in Step 10 implementation/assets remain outside its declared implementation package and its renderer still has exact transition `SRC-TRANS-013`, while `ARCH-01` remains Open | Transformation, scientific-analysis, and evidence owners mapped; reporting correctly remained separate. Caller, Slurm, provenance, docs, and tests migrated without a compatibility wrapper. |
...
| `AC-SLICE-08` | Execution configuration | `4` | `4` | Decide whether named execution profiles are accepted and, if so, define them independently of runtime acquisition modes | **Complete.** `run`/`resume --profile NAME` resolve one safe Project-local `<project-root>/emrys.execution.NAME.yaml` fragment; the mutually exclusive explicit-path selector remains supported. Both use the existing v1 admission and source/effective provenance. A new Run or resource-declaring resume uses default → fragment → CLI-resource precedence; placement-only resume preserves predecessor resources before CLI overrides and explicit resources must match immutable Run identity. The selector name adds no schema, registry, identity, runtime mode, or automatic search. Site/global profile management remains `OPS-01`; runtime acquisition remains `RUNTIME-01`. | Focused local gates plus exact implementation commit `220343f6c0fd5126e3b916df1bbc907980d0888f` passing all four Python 3.11 shards, complete-and-disjoint shard-receipt verification, and the selected 130-pair hosted lane in run [33497912732](https://github.com/lab-cats/EMRYS/actions/runs/33497912732) close the slice. The 100,000-pair and ordinary non-selected manual-dispatch lanes were skipped. |
...
| `AC-SLICE-18` | Documentation | `4` | `3` | Rewrite navigation and documentation around scientist/operator/developer journeys | The bounded `DOC-05` retirement tranche is complete. Broad role-journey work remains Open under `DOC-01`; it uses the accepted `DOC-02`/`DOC-03`/`DOC-05` traces and coordinates with open `DOC-04`. | Scope follows the settled role journeys; stale-source retirement does not by itself complete this card. |
