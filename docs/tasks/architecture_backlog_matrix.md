...
| `AC-SLICE-08` | Execution configuration | `4` | `4` | Decide whether named execution profiles are accepted and, if so, define them independently of runtime acquisition modes | One v1 file-bound format, built-in direct default, explicit selector, current precedence, and source/effective provenance are implemented. Run-bound resources, Attempt-local placement, and the separate runtime profile remain distinct. Named profiles are likely necessary; acceptance, taxonomy, registry/discovery, management, storage/runtime integration, and commands remain Open with `OPS-01` and `RUNTIME-01` | Managed/Site/Explicit remain proposed acquisition journeys converging on one runtime authority; names and APIs are unselected |
