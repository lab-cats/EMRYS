...
qualification and Explicit definition remain Open under `RUNTIME-01`.
Site/global execution-profile discovery, precedence, and management remain
Open under `OPS-01`.
...
| `AC-SLICE-05` | Execution | `4` | `5` | Complete the declared guarantee and parity contract across direct and Slurm placement of the current backend, then evaluate the generalized-backend boundary near campaign closure | Successful hosted parity is proven. Controlled failure/resume implementation is complete and awaits selected 130-pair evidence. The generalized-backend evaluation is complete as a no-op: Slurm remains transport around the sole Snakemake backend. Institutional-site/module, multi-node, and production proof remain `SITE-PARITY-01`. | No second backend or net-negative consolidation case was found; do not add an abstraction |
...
matrix above remain authoritative until their routed tranche completes.
...
| `ARCH-CLOSE-02` | `AC-SLICE-05`, residual `OPS-02`, `LOG-05` | **Implementation complete; selected recovery evidence pending.** The 130-pair driver causes a deterministic Snakemake profile-admission failure before task entry, then resumes direct and Slurm Runs through the public command without mutating inputs or creating incomplete task state. The generalized-backend evaluation is complete as a no-op: Slurm remains transport around the sole Snakemake backend. No scheduler `cancel` command is added without durable pre-Run submission ownership; a future demonstrated need requires a separate task. |
...
|---|---|
| `AC-SLICE-11` / `FILESYSTEM-01` | Ratified no-op: no Run Bundle; retain the current layout and only complete predictable automatic directory ownership. A future demonstrated portability/archive/share need requires separately approved `RUN-BUNDLE-01`. |
...
| `OPS-04` | Rename the private application owner caller-completely to `run_coordinator` in `ARCH-CLOSE-03`; preserve historical evidence spellings and add no compatibility package. |
