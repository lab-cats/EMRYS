...
| `AC-SLICE-08` | Execution configuration | `4` | `4` | Decide whether named execution profiles are accepted and, if so, define them independently of runtime acquisition modes | **Implementation present; verification pending.** See [campaign Section 13.26](architecture_campaign.md#1326-bounded-slice-record-project-local-named-execution-profiles); broader site/global management remains `OPS-01` and runtime acquisition remains `RUNTIME-01`. | Exact-head standard CI and the selected 130-pair hosted lane remain required for terminal closure. |
...
|---|---|---|
| `ARCH-CLOSE-01` | `AC-SLICE-08`, `AC-SLICE-13`, residual `OPS-02`, `RUNTIME-01`, `ANALYSIS-02` | One path-light installed-command journey from the exact Project root: ordinary `validate`, `run`, `inspect [RUN]`, `resume [RUN]`, and `report [RUN]`; Project-local `runtime/profiles/default.yaml` plus one named-or-absolute `--profile`; deterministic human Run names with ID/prefix fallback and terminal selection; retained technical inspection under `debug`; managed runtime, Doctor, collaborator dependencies/resources, automatic reporting, no-write refusal, and the golden path verified together. Old `validate project`, `inspect run`, ordinary `--run-root`, root-level named profiles, and `--execution-profile` retire directly. |
| `ARCH-CLOSE-02` | `AC-SLICE-05`, residual `OPS-02`, `LOG-05` | Direct/whole-Run-Slurm failure, resume, Results/reporting, provenance, and single-log parity; generalized backend explicitly rejected unless a concrete extension or net-negative consolidation appears. A scheduler-aware `cancel` may be evaluated only if safe submission ownership and terminalization are demonstrated; it is not an `ARCH-CLOSE-01` requirement. |
...
| `OPS-01` | Ratified no-op for any site/global registry; Project-local named profiles and exact absolute paths are the complete selected lookup model. |
