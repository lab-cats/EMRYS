...
| `AC-SLICE-17` | Retirement | `3` | `5` | Retire duplicated lifecycle, validator, infrastructure, adapter, shell, or compatibility paths after each replacement is proven; retained evidence may be deleted only after an exact scoped proposal and the user's explicit approval | Completed cuts retire the generated wrapper/configuration split and the Step `07`–`09` shell owners plus their shell-only suites after caller-complete replacements. The private batch bootstrap and all sixteen owner-local `.slurm` files remain; Steps `07`–`09` delegate to Python and the other thirteen retain their prior forms. All other registered reductions remain deferred, so this card remains **Open**. No retained evidence is deleted | Low-risk impossible-state protections need no artificial replacement; high-risk, ambiguous, user-facing, execution-boundary, or evidence-validation changes require explicit approval; any approved evidence deletion stays isolated and never offsets product growth |
