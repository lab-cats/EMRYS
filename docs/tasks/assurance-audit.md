...
**Caller trace.** Project admission and the producer reject out-of-FAI BED coordinates before normal Run work (`src/emrys/orchestration/run_coordinator/onboarding.py:1631-1683`; `src/emrys/stages/partitioned_cohort_mpileup/producer.py:118-169`), and Run binds the selector as a stable input (`src/emrys/orchestration/run_coordinator/materialization.py:742-755`; `src/emrys/orchestration/run_coordinator/task.py:2788-2800`). Reporting's native Step 07 reconciliation checks receipt/VCF relationships but not selector bounds (`src/emrys/reporting/_artifact_index/reconcile_native.py:187-269`). Thus the fixture proves a standalone report false pass, not a supported-Run false pass. The report's intended selector claim still needs an owner decision.
...
| Tests | A01-05's central roster permutations, A01-30's 36 scheduler combinations, and A01-39's repeated suite collection are cost candidates under `QUAL-01` (`docs/tasks/backlog_matrix.md:155`) | Keep distinct scheduler faults, independent oracles, complete/disjoint shard proof, and exact receipts |
...
| Configuration | A01-37 records duplicate Snakemake dependency declaration; A01-47 records repeated Viking resource defaults; A01-74 identifies a stale root settings template | Preserve installed dependencies, placement admission, generated defaults, and operator examples before trimming |
...
Walk every stage, analysis, evidence, orchestration, library, reporting, runtime, contract, and public CLI owner against its supported command or input, distinct fault, protection, and evidence level. Extend the same map to fixtures, scripts, schemas, configuration, documentation checks, packaging, and each CI lane. For each proposed removal, name the surviving defense and compare its boundary before any edit. Resolve A01-04 and A01-11 inventory/roster interfaces, trace A01-73 diagnostic CLI admission and A01-74 setup guidance, then continue A01-41/42 cross-owner classification and A01-56/60/64 malformed or misleading evidence. Keep source-composed Run impacts separate from full-Run reproduction. Update these entries in place as evidence develops; keep implementation and task status in their existing owners.
