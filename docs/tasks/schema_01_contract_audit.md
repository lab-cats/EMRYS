...
| S06 — Prerelease distribution | Partial | The package declares Alpha and `0.1.0.dev0`. On 2026-09-22, the exact PyPI project lookup returned 404, GitHub listed no releases or tags, and bounded public code searches found no indexed repository outside EMRYS using three exact identifiers. The [Quickstart](../../quickstart.md) documents source installation. A wheel test and public CLI, artifact-validation, provider and reporter interfaces establish supported external surfaces, not observed third-party use. | Inventory known collaborators, source installs, private distributions, and downstream readers. The checked public channels do not prove absence of external consumers. |
...
| S10 — Reduction opportunities | Partial | The schema-structure screen found no unreferenced `$defs` among 79 definitions, several small exact duplicate shapes, and two conditional forms worth comparing; large state unions encode distinct contracts. Both registries repeat framework setup but differ in policy. The field screen found qualified profile, Attempt, reporting, reference, and placement candidates; repeated values can be independent checks. | Measure schema syntax and maintained product code separately. Test selected factoring against accepted/rejected records, diagnostics, identity, and all callers. Keep the two owner registries unless a complete measured migration demonstrates a net reduction. |
...
creates a private `TMPDIR`, and [Task execution](../../src/emrys/orchestration/run_coordinator/task.py)
derives its own scratch. None populates the Attempt field. Inspection,
lifecycle, Task, backend, and reporting still admit the whole closed Attempt
record. A removal therefore changes exact Attempt bytes and references even
without a direct field reader; retained records and the possible value of
per-Attempt provenance remain open.

For Run-summary scope issues, the current producer copies artifact issues,
but the [valid fixture](../../tests/contracts/artifacts/fixtures/artifact_schema_v2/valid/run_summary.json)
has different scope, artifact, and Run warning messages. Semantic admission
does not enforce their equality. An independent report golden uses that
fixture but does not render the scope warning text. No owner decision was
found saying whether distinct scope wording is
intentional. The full summary reaches reporter plugins and its exact bytes
are bound by the [report receipt](../../src/emrys/reporting/_run_report/receipt.py)
and [transaction revalidation](../../src/emrys/reporting/transaction_validation.py).
Removing the arrays or adding an equality check would therefore be a contract
...
A same-day [GitHub code search](https://docs.github.com/en/rest/search/search#search-code)
used exact queries for `"emrys-rna-workflow"`, `"urn:emrys:schema:"`, and
`"emrys.analysis_modules"`, each with `-repo:lab-cats/EMRYS`. The REST
`search/code` response returned `total_count: 0` and
`incomplete_results: false` for each. The unrestricted queries found matches
in EMRYS itself, confirming these identifiers were indexed there. GitHub's
REST code search indexes only default branches and files under 384 KB, so
this is not a census of public code. No independent public consumer was
observed by this bounded search. Private or unindexed code, local source
installs, copied or renamed contracts, and retained Projects remain outside
its evidence.
...
baseline, not a proposed deletion count.

### Schema size and structure screen

A standard-library JSON traversal found 79 named `$defs` entries and 388
`$ref` occurrences across the 20 resources. Every named definition has a
reference within the packaged set. There are 35 `oneOf`, eight `allOf`, and
11 `if` constructs; those are counts of syntax, not redundant branches. The
largest files are `application_model` (12,713 bytes, 283 physical lines) and
`workflow_attempt` (11,497 bytes, 435 lines). The former's top-level choices
admit three distinct record kinds; the latter's placement and Task branches
separate real execution states. File size alone does not show needless
complexity.

| Structural opportunity | Bounded saving or simplification | Required check and current disposition |
| --- | --- | --- |
| [Attempt receipt](../../src/emrys/contracts/schemas/orchestration/v2/attempt_receipt.schema.json) task-start and task-attempt roster items | Two exact JSON shapes occupy separate 27-line array blocks. A local `$defs` entry could replace one body while keeping the two named rosters. | Compare schema errors and the rosters' distinct Task-chain roles; this Run-root schema change alters new Run identity. Gross repeated text is not net maintained savings. |
| [Run summary](../../src/emrys/contracts/schemas/artifacts/v3/run_summary.schema.json) `analysis_policy` and table item | Their closed path/hash/size objects are identical. One local `$defs` entry might remove roughly 6–8 physical lines. | Check validation error paths, reporter input, retained summaries, and Attempt package provenance. This is a small syntax candidate, not a record-field deletion. |
| [Artifact entry](../../src/emrys/contracts/schemas/artifacts/v2/artifact_record.schema.json) complete-status conditions | Two `completion_status == complete` conditionals could combine, with a rough ceiling near 15 physical lines. | Compare accepted and rejected fixtures plus schema error paths. No differential validation has established this as safe or worth the package-provenance change. |
| [Workflow Attempt](../../src/emrys/contracts/schemas/orchestration/v1/workflow_attempt.schema.json) placement conditionals | The direct/Slurm `if` blocks may admit a shorter `if`/`else` expression while retaining the outer request and scheduler-ID constraints. | The current branches enforce distinct placement states, and a [test](../../tests/contracts/orchestration/test_orchestration_contracts.py) expects scheduler-ID diagnostics. Differential admission and exact diagnostics are needed; this is exploratory. |
| Cross-resource direct-edge and path/hash copies | Profile's direct-edge item matches application-model's definition; Run-summary `run_contract_file` matches receipt `resource`. | Sharing adds `$ref` coupling between separate current resources for a small text saving. Artifact common `path_hash` is broader and cannot replace the strict objects. Retain local shapes absent a measured full-closure benefit. |
| [Resource configuration](../../src/emrys/contracts/schemas/orchestration/v3/resource_config.schema.json) stage keys | Three maps repeat 33 explicit stage-key references. A pattern or shared key rule could shorten text. | Preserve the exact accepted key sets and current additional-property diagnostics; a compact regex may be harder to maintain. No compression is selected. |

Small one-line or formatting reductions are not a complexity result. Any
schema-byte change affects new Attempt package provenance; the orchestration
rows above are also explicit Run implementation roots. Even a
payload-equivalent structural rewrite therefore changes new Run identity and
can change schema diagnostic paths. Compare complete source, tests, config,
docs, and retained-record impact before counting a net saving.

### Registry and adjacent code screen

The artifact loader's
...
| Non-v1 `reporting_start` | Its schema file is outside the explicit Run-root list, but a complete `$id` reset must also update the exact ID in Run-bound orchestration registry code. Reporting ledger writer/reader and installed-package provenance also depend on it. | Open as a reporting/recovery decision; a complete `$id` reset changes new Run identity through the edited registry code. |

The next table closes the 11 non-v1 IDs at source level. `Direct Run root`
means the schema file itself is in
[Run implementation identity](../../src/emrys/orchestration/run_coordinator/run_implementation.py).
All 11 are packaged and exposed through a record selector, appear in the
[wheel roster](../../tests/test_package_distribution.py), and affect the
[installed-package tree digest](../../src/emrys/libraries/installed_package_identity.py)
retained for new Attempts. A no in that column does not make a full reset
Run-neutral: edited registry or admission code can also be a Run root.

| Non-v1 `$id` | Direct Run root | Other exact coupling and retained check |
| --- | --- | --- |
| Artifact entry v4 | No | Run-summary `$ref`; report receipt records artifact-entry `4.0.0`; nested retained summaries and public validator. |
| Run summary v8 | No | Serialized `8.0.0`; report receipt binds summary name, version, and hash; retained summary transaction and reporter input. |
| Report receipt v8 | No | Serialized `8.0.0` in receipt and TSV; binds both HTML output hashes; retained report transaction revalidation. |
| Reporting start v2 | No | Reporting ledger and verified-reporting record reference; exact orchestration registry ID also changes in a complete reset. |
| Run lock v2 | Yes | Active/released lock projection, prepared or terminal receipt, and inspection byte comparisons. |
| Task start v3 | Yes | Task-attempt start reference, Attempt receipt roster, and retained Task chain. |
| Task attempt v4 | Yes | Carries a Task-start record reference; verified Task references this Task attempt; Attempt receipt roster and recovery inspection. |
| Verified Task v2 | Yes | Task-attempt reference and Attempt receipt verified roster. |
| Workflow Attempt v4 | Yes | References execution-profile v1; immutable Attempt, lock, Task, and reporting origins. |
| Attempt receipt v3 | Yes | Prepared/terminal closure, literal current-version checks, reporting and successor admission. |
| Profile v2 | Yes | Serialized `emrys.profile.v2` plus `profile_version=v2`; retained profile snapshot, Attempt digest, backend/functional checks. |

These are affected authorities, not evidence that any actual retained Project
contains each class. Schema `$ref` edges are only part of the dependency
closure. The [report receipt schema](../../src/emrys/contracts/schemas/artifacts/v5/report_receipt.schema.json)
and [writer](../../src/emrys/reporting/_run_report/receipt.py) bind artifact
entry and Run-summary version labels without `$ref`. A `$id`-only change may
leave serialized record labels intact while changing new Run implementation
identity for direct roots and new Attempt package provenance for every row.
Resetting serialized labels is a separate choice that changes retained-record
admission. Neither choice rewrites an existing Run ID or retained evidence.
...
include `application-model`, which admits three current labels: analysis
revision v2, Execution Plan v1,
...
| P5c — Schema structure | Partial | Count named definitions, references, conditional and union branches, exact duplicate subtrees, and closed key sets by resource. For a selected local factoring or branch rewrite, compare representative accepted/rejected records and exact diagnostics against current behavior; measure net source lines and clarity separately from bytes. | Keep distinct state branches and error protections; syntax savings alone do not justify identity or recovery disruption. |
...
| P7 — Compression | Partial | Count schema resources, product code, tests, scripts, config, docs, and evidence separately. Compare existing owner code, standard library, `jsonschema`/`referencing`, and maintained tools. The current two-registry sharing idea has no demonstrated net saving; audit every selected field or structural change's duplicate callers and retirement path. | Require meaningful caller-complete net product reduction or a quantified, explicitly approved exception; no evidence deletion as an offset. |
...
| Factor selected schema structure | Exact local duplicates exist in Attempt receipt and Run summary; a few conditional blocks might be shorter. | Estimated syntax savings are small, and `$ref` coupling or changed diagnostics can raise complexity. Schema-byte changes affect Attempt provenance and direct Run roots affect new Run IDs even if record payloads stay identical. | Compare a concrete diff and differential validator behavior before selecting; no blanket refactor. |
