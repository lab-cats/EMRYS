...
The [Runbook's Project creation section](../operations/RUNBOOK.md#create-a-project-for-your-own-data) and coordinator [publication contract](../../src/emrys/orchestration/run_coordinator/CONTRACT.md#no-write-and-publication-boundaries) both describe one-pass FASTQ hashing, deferred STAR values, reference identity, and no-write confirmation; the [configuration guide](../../configs/README.md#L67) already owns exact STAR derivation. DOCS-01 can shorten Runbook internals while retaining its operator choices, confirmation, and recovery cue, then link exact guarantees to their owners. [Documentation structure tests](../../tests/documentation/test_validate_structure.py#L298) protect paths and anchors rather than prose or novice usability. The [polish campaign's history](polish-campaign.md#existing-capabilities-and-overlapping-work) needs proposal disposition before shortening, and the [cluster backlog](cluster_verification_backlog.md#verified-scope-and-remaining-evidence) owns live CV acceptance until exact evidence reaches the [validation history](../history/validation-evidence.md). Both are possible temporary exceptions with explicit retirement triggers, not immediate deletion candidates. Exact evidence deletion requires its own approval and commit.

## Coordinator planning, admission, and submission owners

### materialization.py

[Run and Attempt planning](../../src/emrys/orchestration/run_coordinator/materialization.py#L175) builds immutable identity, step commands, Task records, and create-exclusive publication. Its fixed-step and typed-module planners emit similar record shapes but use different inputs and producers. Artifact expansion already delegates to the inventory owner, and [Run binding is committed last](../../src/emrys/orchestration/run_coordinator/materialization.py#L1724) while incomplete residue is quarantined. [Plan and interruption tests](../../tests/orchestration/run_coordinator/test_materialization.py#L1470) protect direct/Slurm equality and partial publication. Replacing the command switch with another registry needs all-step argv and provenance parity plus a net-line comparison; relocation alone is not reduction.
