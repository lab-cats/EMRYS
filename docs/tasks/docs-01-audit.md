...
Of 161 records, 24 were dismissed after recheck; their reasons remain in the
...
| [F123](docs-01-discoveries-fourth.md#f123-repeated-stage-resource-defaults-in-the-configuration-guide) | Numeric drift risk | The configuration guide's stage table repeats eight numeric memory minimums and workflow/shared settings from the packaged default and Viking example profiles. | The table explains stage and tool behavior and has an inbound owner link; no physical-line saving or runtime capacity conclusion follows. |
