...
| [F05](docs-01-discoveries.md#f05-generic-study-versus-named-evpum1-route) | Reader route | Root and configuration indexes link to the EV/PUM1 Quickstart; the configuration guide says to replace its study choices. The Runbook separates generic choices but still sends own-data readers to Quickstart steps 4–7, which use the named Project and Slurm. | The generic route remains unverified as a reader journey. |
...
| [F07](docs-01-discoveries.md#f07-doctor-repair-does-not-always-install) | Doctor wording drift | Runbook's first-setup shorthand is qualified later; the decision and coordinator contract still describe Slurm repair as installation, the decision assigns it `uv`, and Troubleshooting labels qualification failure “after installation.” | Verification-only Slurm plans can submit checks without package work; preview, confirmed verification, and manager changes have different effects. |
| [F08](docs-01-discoveries.md#f08---version-and-local-env) | Source-ordering limit | Runbook promises `--version` from any directory; a malformed marked or non-regular `.env` in that directory or an ancestor is rejected before version dispatch. | Source inference and unrun installed-command behavior remain separate. |
...
| [F13](docs-01-discoveries.md#f13-doctor-profile-proposal) | Prior-revision proposal | Polish item 8's dated missing-`--profile` proposal is implemented in current source and covered by CV-07 and the coordinator contract; institutional acceptance remains pending. | A shorter dated owner route may save five to six of 11 lines; preserve original audit tense, selection refusals and no-write limits. No net saving is verified. |
| [F14](docs-01-discoveries.md#f14-old-source-attestation-cost-candidate) | Historical cost and selection prompt | Dated optimization candidate records 24 former Git calls and still asks future selectors to measure Git and preserve changed-HEAD detection; its summary asks for package/commit attribution, while current task entry observes installed bytes without Git. | Keep the dated count; an 11-to-six-line selection sketch suggests five conditional local lines. Preserve installed bytes and available build origin, with no measured cost or verified net saving. |
...
| [F19](docs-01-discoveries.md#f19-doctor-experiment-evidence-in-workflow-readme) | Evidence placement | CI workflow README's 16-line completed Doctor section repeats CV-26's artifact, trials, limits and decision; its current measurement-driver note still matters. | Eight to ten local lines are conditional with the heading retained. CV-26 must later transfer to validation history before backlog retirement; repository net saving and future artifact availability remain open. |
...
| [F28](docs-01-discoveries.md#f28-repeated-owner-boilerplate) | Compression candidate | Six stage owners and eight stage/evidence test guides repeat 58 physical lines; twelve contract evidence-ceiling openings add a separate 24-line review span. | A tests-root explanation and local routes suggest 15–16 repository lines; stage-index clarification and routes suggest 10–11 more, or 25–27 conditional net lines. Contract lines are review material, not established savings. No replacement is verified. |
...
| [F31](docs-01-discoveries-continued.md#f31-historical-slurm-username-recovery-advice) | Operator recovery clarity | Troubleshooting's eight-line username case combines a useful error and recovery route with an unspecified “submission fix” revision and older-wrapper detail. | The historical synthetic resume succeeded after the fix; do not infer incompatibility. Preserve error, no-passwd cause, evidence and offered resume. At most zero to two local lines are conditional. |
...
| [F33](docs-01-discoveries-continued.md#f33-report-receipt-version-in-the-scientist-diagram) | Receipt version error and wording ambiguity | Scientist diagram names a v4 report receipt although the current schema is v8; its combined reporting phase and “read-only” label can blur separate publication and input immutability. | The phase diagram does not establish a transaction defect. Summary and HTML publication are separate transactions; report creation leaves scientific inputs unchanged. |
...
| [F36](docs-01-discoveries-continued.md#f36-cross-owner-history-in-runtime-test-guidance) | Placement candidate | Runtime test README ends with a sentence about retired optional runtime-report-publisher tests; the polish card retains their retirement and defects. | Current public `emrys report` is distinct; trimming the sentence alone saves no physical line. No evidence deletion is authorized. |
| [F37](docs-01-discoveries-continued.md#f37-bed12-dependency-in-the-scientist-diagram) | Diagram ambiguity | Combined QC/orientation node lacks RSeQC's BED12 dependency; the reference node groups produced FAI/BED12 with external FASTA/GTF. | Concise non-authoritative depiction can blur an input edge; no workflow DAG or Run-completion defect is established. |
...
| [F39](docs-01-discoveries-continued.md#f39-validation-roster-inventory-claim) | Scope wording | Parent test index says “every current validator,” but the fixed map covers all 14 current validation-report check-ID producers, not other validator kinds. | The current producer roster is complete; neither README promises future source-owner discovery. |
| [F40](docs-01-discoveries-continued.md#f40-concurrency-in-the-local-workflow-profile) | Dismissed after recheck | Local workflow profile guide says “sample concurrency”; current policy resolves per-stage concurrency. | The phrase is loose shorthand in a profile overview, with no false global-cap claim or useful reduction established. |
...
| [F42](docs-01-discoveries-continued.md#f42-report-transfer-in-the-coordinator-test-index) | Evidence placement | Coordinator test table's one-line report-transfer row links a Runbook procedure, not a test; CV-27 holds the tiny local observation. | One-line reduction is plausible; preserve the Runbook procedure, CV evidence limits, and guide heading's inbound link. |
...
| [F44](docs-01-discoveries-continued.md#f44-internal-worker-command-ownership) | Dismissed for DOCS-01 | RSeQC's opening says independently runnable, but its owner guide and contract identify an internal runner worker; source/test labels call delegated commands “public.” | The labels establish no supported standalone command or useful documentation reduction; retain independent help and argument checks. |
...
| [F48](docs-01-discoveries-continued.md#f48-project-name-lookup-from-the-repository-root) | Dismissed after recheck | Root guide permits `--project NAME_OR_PATH` outside a Project; cwd-relative names and explicit paths both satisfy that wording. | Neither the guide nor Quickstart promises saved-home or global bare-name lookup; no useful DOCS-01 change is established. |
| [F49](docs-01-discoveries-continued.md#f49-allocation-preview-needs-execution-flag-boundary) | Recovery safety wording | Troubleshooting says to preview a rejected submission with `--verbose </dev/null`; an earlier command may retain `--execute`. | The prior operation supplies context, but input redirection does not make an executing command read-only. |
...
| [F57](docs-01-discoveries-continued.md#f57-make-fixture-public-target-label) | Dismissed after recheck | Fixture guide says it covers every public Make target, and the test map also inventories internal and operator targets. | “Every public” does not mean “only public”; no false coverage claim or useful reduction is established. |
...
| [F62](docs-01-discoveries-third.md#f62-benchmark-value-can-be-label-only) | Benchmark evidence ceiling | Scripts guide says the helper measures commands at declared resource values; producer argv need not contain the value placeholder. | Actual substitution depends on manifest argv; SETUP-02 already owns helper retirement, and no benchmark was run. |
...
| [F64](docs-01-discoveries-third.md#f64-star-mechanics-in-a-scientific-decision) | Responsibility overlap | Scientific decision's 15 body lines mix current STAR Init/validation mechanics with original study settings and rationale. | A shorter owner route might save four to six lines; keep the STAR scaling reason, biological boundary, pinned manual, and decision heading. Unverified. |
...
| [F66](docs-01-discoveries-third.md#f66-step-09-qc-summary-input-scope) | Input-scope wording | Step 08/09 guides accurately name computational inputs; the Step 09 Run task also binds the Step 08 QC summary as a stable input. | Only the task-input and provenance role is absent from the owner descriptions; no incomplete stage-map roster is established. |
...
| [F68](docs-01-discoveries-third.md#f68-slurm-diagnostic-artifact-bounds) | Evidence-scope overclaim | CI guide calls uploaded Slurm diagnostics bounded and redacted; capture has finite file scope but no byte/line cap or content-redaction step. | Private accounting files are excluded; no artifact contents or disclosure were assessed. |
...
| [F70](docs-01-discoveries-third.md#f70-omitted-site-does-not-always-mean-direct) | Conditional reader-route error | Runbook and configuration guide say omitting `--site` creates a direct profile; `EMRYS_SITE=viking` from process or saved settings makes both Init parsers select Slurm. | This does not affect the no-default case; no command was run. |
| [F71](docs-01-discoveries-third.md#f71-quickstart-projects-home-default-and-later-path) | Conditional reader-route error | Quickstart and the Projects index imply the checkout Projects home, while later real-study paths hard-code it. | An inherited alternate home changes the saved real-study destination. No command was run. |
...
| [F85](docs-01-discoveries-third.md#f85-status-vocabulary-exceptions) | Status vocabulary | Main matrix defines six states, but `REPORT-ROSTER-01` uses undefined `Needs decision`. | CV-12's `Discard` is an explicitly explained terminal disposition, not a normalization target; no decision is reopened. |
| [F86](docs-01-discoveries-third.md#f86-step-02b-parallel-validation-claim) | Dismissed for DOCS-01 | Step 02b's independently callable operation may overlap Step 02 validation after a stable pair exists; ordinary Run scheduling waits for the same sample's verified marker. | The contract and Run graph describe different boundaries. No false claim or useful compression is established; no Run was executed. |
...
| [F88](docs-01-discoveries-third.md#f88-old-slurm-memory-preflight-proposal) | Dismissed after recheck | Polish item 36 describes a missing memory preflight at its explicit historical audit revision; current `SCHED-01` records the implemented check and pending verification. | No present-tense guidance error or useful compression is established; institutional acceptance remains open. |
...
| [F90](docs-01-discoveries-third.md#f90-completed-tooling-history-in-polish-campaign) | Dismissed after recheck | Five completed polish sections repeat seven status/CI lines within 77 lines of distinct mechanics and evidence. | Reflow suggests at most zero to two saved lines; no useful independent reduction is established. |
| [F91](docs-01-discoveries-third.md#f91-repeated-stage-and-evidence-contract-openings) | Compression candidate | Twelve stage/evidence contract openings use 48 lines; analogous Step 09 analysis wording adds four, for a 52-line review span. | The original twelve might save roughly one line each; Step 09 remains unestimated. Keep owner alias, worker/validator role and analysis classification; 52 is not a saving estimate. |
| [F92](docs-01-discoveries-third.md#f92-python-lock-checks-before-institutional-r-restoration) | Compression and prerequisite question | Runbook puts ten lines of Python lock checks before institutional R restoration; its R targets and restore script do not require uv. | No repository-owned R gate for uv was found; an independent institutional policy is unverified. Roughly seven local lines may be available while preserving R recovery. |
| [F93](docs-01-discoveries-third.md#f93-repeated-partition-selector-rule) | Small duplication | Config guide twice states that `--region` and `--regions-file` can combine only with unique partition IDs. | At most one physical line is a candidate; keep selector advice, coordinate examples and manifest exclusions. |
| [F94](docs-01-discoveries-third.md#f94-dashboard-retirement-closeout-tense) | Temporal framing | CV backlog's live remaining-acceptance table still assigns standalone retirement as pending, and the coordinator contract retains isolated-loading wording; the main matrix records wrapper/caller retirement. | Dated PR #169/CV-16 accounts remain history; standard CI and institutional visual verification remain pending. No installed-package import failure is inferred. |
...
| [F97](docs-01-discoveries-third.md#f97-past-audit-priority-order-in-polish-campaign) | Historical selection order | Polish campaign keeps a nine-line second/third-pass priority account whose proposals and status have other owners. | A shorter route might save five to six local lines; retain complete-command measurement and no-parallel-initiative boundaries. Unverified. |
...
| [F100](docs-01-discoveries-fourth.md#f100-gtf-worker-detail-in-the-coordinator-contract) | Owner-detail overlap | Coordinator runner section repeats the GTF worker's shared-normalization fact already in its stage contract. | Only the final worker-detail sentence could save one line; runner publication and recovery remain distinct. |
...
| [F102](docs-01-discoveries-fourth.md#f102-historical-e09-example-in-current-lifecycle-rules) | Dismissed after recheck | Lifecycle contract's one-sentence E09 limit prevents its new prepared-finalization rule from implying recovery of that historical Run. | The CV register/card retain the observation; removing the guard would lose a useful limit without meaningful compression. |
| [F103](docs-01-discoveries-fourth.md#f103-unpublished-fastq-experiment-in-the-owner-guide) | Dismissed after recheck | Sample-manifest owner guide retains the unique NUL-header counterexample and source-derived logical-pass limit beside the helper's byte boundary. | The campaign already delegates that exact boundary to the owner; no deletable span, measured I/O, or saving is established. |
| [F104](docs-01-discoveries-fourth.md#f104-automatic-reports-after-successful-computation) | Dismissed after recheck | Runbook says successful computation generates reports by default, then immediately tells readers to check completion or finish reporting. | The adjacent recovery route makes incomplete reporting explicit; no misleading completion promise or useful reduction is established. |
| [F105](docs-01-discoveries-fourth.md#f105-retired-scheduler-wrapper-in-the-stage-map) | Dismissed after recheck | Stage map's brief retired-wrapper explanation tells readers why the old 00a coupling did not create semantic edges to 00b/00c. | The edge table alone does not explain that absence; no graph defect or useful reduction is established. |
...
| [F107](docs-01-discoveries-fourth.md#f107-retired-shell-publication-tests-in-a-current-test-guide) | Dismissed after recheck | Shared-library test guide retains the retired shell fault and its original revision beside the current runner suite; seven owner contracts retain distinct failure modes. | Test and owner locations match their evidence roles; no redundant exact account or safe saving is established. |
...
| [F120](docs-01-discoveries-fourth.md#f120-superseded-init-replay-in-the-active-cv-backlog) | Dismissed after recheck | CV-U18's later “Current confirmation correction” explicitly supersedes its earlier generated-command description and classes replay fixtures as historical. | The chronology preserves why the interface changed; no reader ambiguity or safe compression is established. |
...
| [F122](docs-01-discoveries-fourth.md#f122-step-06-optimization-source-after-publication-moved-to-the-runner) | Dismissed after recheck | Optimization candidate 1 cites a pinned Step 06 producer that then owned extraction and publication; the current runner owns publication. | The campaign explicitly dates all source links and requires current-path recheck; it makes no present-tense owner promise. |
...
| [F124](docs-01-discoveries-fourth.md#f124-reliability-diagram-collapses-two-validation-orders) | Validation-order ambiguity | Reliability diagram shows validation before every publication; Step 02 decision also says “validation-first” without distinguishing staged worker checks from post-publication grouped validation. | The runner owns recovery; post-commit failure preserves native outputs. The [decision recheck](docs-01-discoveries-sixth.md#step-02-decision-recheck-for-f124-at-35668cc8) preserves the valid staged-check reading. No runtime defect is inferred. |
...
| [F133](docs-01-discoveries-fifth.md#f133-fourteen-workflow-owners-labeled-scientific) | Dismissed after recheck | Architecture guide calls the built-in path's fourteen owners scientific, while its adjacent boundary and phase tables classify the two evidence collectors. | The phrase is broad path shorthand; local context prevents owner confusion. Removing one adjective saves no physical line or useful standalone correction. |
| [F134](docs-01-discoveries-fifth.md#f134-cv-25-implementation-account-beside-current-log-owners) | Dismissed after recheck | Completed CV-25 retains three selected slices, distinct checks, and evidence ceilings beside the current log-discovery owner. | The owner contract does not replace completion genealogy; no safe reduction follows from the 58-line review span. |
| [F135](docs-01-discoveries-fifth.md#f135-cv-24-repeats-the-current-watch-action-protocol) | Dismissed after recheck | CV-24 repeats current watch `p`/`b`/`s` action rules held by the coordinator contract and operator Runbook. | The card needs its selected interface with test evidence and pending institutional acceptance; no useful reduction is established. |
| [F136](docs-01-discoveries-fifth.md#f136-private-planning-helper-narration-in-the-coordinator-contract) | Compression candidate | Coordinator contract names private materialization helpers and caller locals inside public planning guidance. | Helper narration at lines 854–857 is a 2–3-line candidate; preserve composition and distinct STAR/reference/input/R construction. |
...
| [F139](docs-01-discoveries-fifth.md#f139-storage-command-route-in-the-evidence-index) | Reader-route ambiguity | Evidence index lists the manual storage debug command without distinguishing Doctor's normal storage-qualification path. | Keep Doctor's normal route and the manual phase/residue limits distinct; no command was run or line saving established. |
| [F140](docs-01-discoveries-fifth.md#f140-generic-selection-policy-repeated-in-the-polish-campaign) | Compression candidate | Polish campaign's 21-line generic selection checklist repeats workflow and architecture guardrails. | An 11-line linked route suggests ten local lines; retain the specific parallel-machinery ban and bounded tooling/correctness exception. No replacement or net saving is verified. |
...
| [F142](docs-01-discoveries-fifth.md#f142-collaborator-acceptance-repeated-in-the-polish-campaign) | Acceptance duplication | Polish item 29 repeats external provider/reporter acceptance in a 17-line span and the authoritative `EXTENSION-01` row. | About ten lines may be available only after transferring the sampled-test, DSL, versioned-interface, and service limits. The 31-line section is not a saving estimate. |
| [F143](docs-01-discoveries-fifth.md#f143-unrouted-reporting-run-contract-example) | Dismissed for DOCS-01 | Eight-line run-contract example has no filename-specific non-audit Markdown link or call site found; current coordinator projects the six-field record. | The config guide already routes specialist examples; a named link would add prose, and no safe reduction is established. |
...
| [F150](docs-01-discoveries-fifth.md#f150-named-init-review-roster-repeated-in-adjacent-cv-cards) | Dismissed after recheck | CV-U02/U03 repeat the same September 21 normal-versus-verbose named-Init field roster held by the coordinator contract. | Each dated card needs its distinct outcome and pending acceptance to stand alone; cross-linking saves no useful detail. |
...
| [F159](docs-01-discoveries-fifth.md#f159-polish-integration-genealogy-repeated-in-its-introduction) | Compression candidate | Polish campaign's 109–118 account repeats integration and hosted-CI evidence held in the main matrix and later dispositions. | A 7–9-line reduction is conditional on keeping item 22's exact `34306975901` link; retain 68–79 tested-tree identity and CS-20/22 limits. |
...
| [F162](docs-01-discoveries-fifth.md#f162-synthetic-e2e-help-overstates-one-run-parity) | Tool help scope | Synthetic E2E driver help promises direct/Slurm parity for both profiles; 130 compares two placements of the same immutable Run identity, while 100000 selects Slurm alone. | Distinct 130 workspaces and Attempt histories do not refute its one-Run identity; no driver or hosted lane ran in this audit. |
...
| [F164](docs-01-discoveries-sixth.md#f164-optimization-bounded-delivery-repeats-workflow) | Process compression candidate | Optimization campaign's 14-line Bounded delivery section repeats workflow and backlog rules already linked in its opening. | A seven-line account suggests seven local lines; retain caller-complete retirement, exact benchmark binding and no parallel status ledger. Unverified. |
...
| [F166](docs-01-discoveries-sixth.md#f166-final-source-resource-policy-repeated-in-main-checklist) | Dismissed after recheck | Main closure checklist overlaps selected resource details in CV-U06 and the test-tool guide, but it owns the durable final-source and CI scheduling conditions. | A three-to-four-line local sketch has no established useful net saving; CV-U06 is temporary and the test-tool guide omits the parallel/serial CI rule. |
| [F167](docs-01-discoveries-sixth.md#f167-cv-campaign-remaining-delivery-summary-repeats-matrix) | Summary compression candidate | CV campaign's 28-line remaining-delivery block restates accepted matrix rows and delegated CV status; F03 covers its INIT-02 overclaim. | A 12-line private sketch suggests 16 local lines; preserve inbound links, no-new-execution, CV-U06/CV-10 limits and campaign authority. No edit or net saving is verified. |
...
| [F177](docs-01-discoveries-sixth.md#f177-cv-06-novice-failure-detail-repeated-across-cards) | Dated-account compression candidate | CV-06 recounts September 16 novice-path gaps detailed in CV-U08/U09/U11/U18/U20. | A private sketch suggests nine local lines, not twelve; preserve the integrated negative Viking experience, Open checkpoint and later correction. Net saving is unverified. |
| [F178](docs-01-discoveries-sixth.md#f178-cv-16-watch-failure-detail-repeated-across-cards) | Dated-account compression candidate | CV-16 recounts September 16 watch failures detailed in CV-U13/U14/U16/U17. | A private sketch suggests eight local lines; preserve combined failure, screenshot's diagnostic ceiling, Open checkpoint and later correction. Net saving is unverified. |
| [F179](docs-01-discoveries-sixth.md#f179-cv-01-integrated-journey-account-under-review) | Conditional within-card compression | CV-01's 37-line selected journey uniquely ties its actions, resource fixture, and evidence limits; test and recovery guides do not replace that dated account. | A 32-line within-card copyedit suggests five local lines; preserve three Slurm Attempts, direct parity, controlled versus site evidence, pending exact-commit CI, and institutional limits. Net saving is unverified. |
...
| [F184](docs-01-discoveries-sixth.md#f184-runtime-probe-mechanics-in-the-coordinator-contract) | Cross-owner detail candidate | Coordinator contract repeats Snakemake startup mechanics owned by the runtime guide. | Keep head/compute/execution-preflight boundaries and the head-success limit; two to three local lines are conditional, with four an unverified upper bound. |
| [F185](docs-01-discoveries-sixth.md#f185-canonical-bam-lbpl-contractsource-disagreement) | Contract/source disagreement | Canonical BAM contract requires exact `LB` and `PL` read-group fields; shell producer's reuse and final checks use substring matching, while grouped validation checks exact `ID`/`SM` but omits `LB`/`PL`. | A longer `LB:sample-extra` appears able to pass the hard-link path; no real-tool case or Run was executed. The possible behavior gap belongs to the canonical BAM owner, outside DOCS-01 remediation authority. |
| [F186](docs-01-discoveries-sixth.md#f186-cv-u21-superseded-star-heuristic-chronology) | Dated-account compression candidate | CV-U21 retains a 47-line initial first-record STAR heuristic, tests, selector extension and adversarial reversal ahead of its current repair. | A private 31-line accounting sketch suggests 16 local lines; preserve the defect, selector/publication boundaries, dated checks and pending→Open→pending chronology. Net saving is unverified. |
...
| [F191](docs-01-discoveries-seventh.md#f191-console-detail-tiers-in-the-execution-decision) | Console-detail wording drift | An execution decision suggests separate verbose/debug detail, and the coordinator contract calls inspection rows “Debug”; the public interface exposes them through `--verbose` only. | Keep normal/verbose output and durable-log boundaries distinct. This is a static interface comparison, not an observed command failure. |
