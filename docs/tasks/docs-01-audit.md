...
| [F24](docs-01-discoveries.md#f24-named-profile-procedure-placement) | Audience question | Config guide holds the named-profile operator commands while Runbook links to them and retains head-node Doctor/Run steps. | The linked routes and exact coordinator contract serve distinct roles; no duplicate Runbook procedure or saving is established. |
...
| [F38](docs-01-discoveries-continued.md#f38-slurm-request-in-the-reliability-diagram) | Diagram omission | The generic reliability diagram goes from approval to Attempt creation and omits the separate Slurm request path. | It does not label that edge Slurm; request/submission precede compute-side Run admission. |
...
| [F75](docs-01-discoveries-third.md#f75-validation-lane-diagnostic-bounds) | Volume ambiguity | Test baseline calls failed and cancelled lane diagnostics bounded; the validation driver retains complete logs and prints complete failed logs. | No byte or line bound is shown; lane/file scope may still be finite. No lane ran. |
...
| [F154](docs-01-discoveries-fifth.md#f154-runtime-inventory-mechanics-in-the-runbook) | Operator-detail overlap | Runbook's discovery route repeats inventory-column/policy mechanics owned by runtime guidance, with some freshness detail owned by coordinator. | Confirmation's freshness reassurance and `--execute` are operator-critical; only parts of 355–356 and 360–363 are under review, with no saving estimated. |
