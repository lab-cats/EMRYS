...
| [F97](docs-01-discoveries-third.md#f97-past-audit-priority-order-in-polish-campaign) | Historical selection order | Polish campaign keeps nine lines of second/third-pass priorities whose proposals and accepted status are recorded elsewhere. | The measured surface is not a saving; retain measurement prerequisites and dated decision meaning. |
| [F98](docs-01-discoveries-third.md#f98-worker-prerequisites-inside-validation-sections) | Contract placement | Step 00c and Step 05 validation sections repeat Java/GATK/hash-launcher prerequisites used by internal workers, not their grouped validators. | Preserve both worker environment policy and each validator's separate input and evidence limits. |
| [F99](docs-01-discoveries-third.md#f99-step-07-data-inputs-versus-run-gate) | Execution-order wording | Step 07 says it needs only Step 06 BAM/BAI files, while the admitted Run waits for the Step 06 verified marker before starting it. | Distinguish worker data inputs from Run scheduling; no runtime defect is inferred. |
