...
>
> Intake closed on **2026-08-24**, and the campaign's implementation, no-op
> decisions, and exact-head verification are complete through
> `ARCH-CLOSE-03`. This document remains the temporary authority for context not
> yet dispositioned into the [backlog matrix](backlog_matrix.md).
...
| `OPS-04` | **Complete.** The private application package, live imports, tests, current documentation links, package data, and enforcement rosters use `run_coordinator`; historical evidence spellings and stable serialized identifiers remain unchanged, no compatibility package is added, and exact-head CI passes. |
...
| `ARCH-01` | **Complete.** The compact public model, immutable Run boundary, responsibility layers, execution parity, conditional abstraction gates, caller-complete private-owner rename, aggregate accounting, and exact-head verification are reconciled. No second backend, Run Bundle, Artifact Store, or evidence deletion is introduced. |
...
| `ARCH-CLOSE-03` | **Complete.** Implementation commit `88a489e2d23d78962d99dd1f99f5704a920e0f2c` and closeout record `f3622f791e90fd6ed15079abcbcbe9b7003cbb6a` passed full ordinary CI [run 33653717181](https://github.com/lab-cats/EMRYS/actions/runs/33653717181) and [CodeQL run 33653716112](https://github.com/lab-cats/EMRYS/actions/runs/33653716112). Scientist, advanced-scientist, operator, automation, and developer journeys satisfy the role boundary recorded in Section 14. The private application owner moves caller-completely from `emrys.orchestration.local_pilot` to `emrys.orchestration.run_coordinator`, with no compatibility package; 25 product package files, 24 test files, and one diagram move without file growth. `LifecycleOutcome.workflow_result`, `TaskOutcome.task_attempt`, `TaskOutcome.verified_task`, and unused synthetic `SEED`/`CONTIG_LENGTH` constants retire because they have no production consumers; surviving canonical records and paths remain authoritative. Maintained product is 61 paths, `+184/-197`, net `-13`; tests are 45 paths, `+121/-124`, net `-3`; tooling is four paths, `+4/-4`; current documentation is 14 paths, `+39/-40`, net `-1`; the implementation commit is 124 paths, `+348/-365`, net `-17`. No retained evidence is added, moved, rewritten, or deleted. This closes `AC-SLICE-06`, `AC-SLICE-07`, `AC-SLICE-14`, `AC-SLICE-17`, `CONTROL-01`, `OPS-04`, `FILESYSTEM-01`, `REVIEW-UX-03`, and `ARCH-01`; `AC-SLICE-18` routes to `DOC-COMPRESS-01`. |
...
Architecture-campaign implementation and verification are complete. The
separate user disposition plus Importance/Complexity scoring pass remains; it
does not reopen campaign design or silently authorize the named follow-ons.
That pass is backlog governance, not another implementation tranche.
...
is separate and passed exact-head CI as recorded in Section 13.30. File measures enumerate tracked
...
|---|---|---|
| `AC-SLICE-01` | Ratified all 27 architectural invariants and five migration/test guardrails against live contracts and representative tests | Completed as `ARCH-CONST-01`; broad `ARCH-01` is Complete. |
| `AC-SLICE-02` | Ratified responsibility clusters, three separate dependency graphs, forbidden authority transfers, a current-owner crosswalk, and a fast Python source-boundary ratchet for exact CLI seams and transitional imports | Completed as `ARCH-LAYER-01`; broad `ARCH-01` is Complete. |
...
| `OPS-04` | **Complete.** The private `local_pilot` application package and all live callers become `run_coordinator`; historical evidence spellings and stable serialized identifiers remain unchanged, with no compatibility package. |
...
| `ARCH-01` | **Complete.** The responsibility layers, compact public model, immutable Run boundary, execution/logging parity, caller-complete `run_coordinator` ownership, conditional abstraction gates, and exact-head verification are reconciled. Attempt-v1 evidence and Results authority remain unchanged; no second backend, scheduler, facade, Run Bundle, Artifact Store, or evidence deletion is introduced. |
