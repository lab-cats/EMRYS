...
| `OPS-02` | Its user-facing command burden is complete through `ARCH-CLOSE-01`; only failure/recovery placement parity remains with `ARCH-CLOSE-02`. |
...
| `LOG-05` | The retained-operation roster is complete; finish direct/Slurm failure-path parity in `ARCH-CLOSE-02`, then close. |
...
| `ARCH-CLOSE-02` implementation | **Implementation complete at `52874c71`; selected recovery evidence pending.** The existing CI-owned real-synthetic driver causes one deterministic Snakemake profile-admission failure before any task owner enters, admits the resulting failed direct and Slurm Attempts, resumes both through the public command, and compares immutable authority, Attempt lineage, recovery, Results/reporting, scheduler provenance, and one compute-side application log per operation. The admitted inputs remain unchanged and no incomplete task state is created. The 100,000-pair profile remains success-only. Maintained product, public surface, schema/configuration authority, mutable Run state, compatibility, and retained evidence change by zero; protections/test tooling are two existing Python files, `+460/-144`, net `+316`; test-tool documentation is one existing file, `+8/-4`, net `+4`; the whole implementation is `+468/-148`, net `+320`, with no file growth. |
...
`ARCH-CLOSE-02`, `AC-SLICE-05`, residual `OPS-02`, and `LOG-05` remain open
until the selected 130-pair failure/resume lane passes on the exact implementation
revision. No recovery-parity CI claim is made by this pre-evidence record.
...
| `AC-SLICE-05` | Complete the declared guarantee and parity contract across direct and Slurm placement of the current backend, then evaluate the generalized-backend boundary near campaign closure | Successful hosted parity is proven. Controlled failure/resume implementation is complete and awaits the selected 130-pair evidence gate. The generalized-backend evaluation is complete as a no-op: Slurm remains transport around the sole Snakemake backend. Institutional-site/module, multi-node, and production proof remain `SITE-PARITY-01`. |
...
| `OPS-02` | The path-light Project-root command surface is complete through `ARCH-CLOSE-01`. Hosted 130-pair success-path parity and all sixteen owner-wrapper retirements remain as recorded. Controlled failure/recovery parity is implemented and awaits `ARCH-CLOSE-02` evidence; institutional-site/module, multi-node, and production proof remain follow-ons, while the private source rename remains `ARCH-CLOSE-03`. |
...
| `LOG-05` | The retained-operation roster is complete: grouped `run`/`resume` execution owns one protected compute-side application attempt; automatic reporting continues in that log; standalone generation owns one reporting log; and confirmed Doctor repair owns one maintenance log. Initialization, validation, runtime discovery, diagnosis/preview/refusal, planning/reuse, inspection/debug, scheduler submission, and delegated tasks own none. Hosted success-path parity is proven; only direct/Slurm failure-path parity remains open. Logging degradation cannot change evidence, recovery, or exit. |
