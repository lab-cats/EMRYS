...
Terminal `AC-SLICE-08` disposition remains pending exact-head acceptance. The
selected hosted 130-pair lane is sufficient for this configuration-selection
cut; the 100,000-pair lane is not required. Even when it passes, its evidence
ceiling is disposable single-node synthetic Slurm engineering evidence, not
...
| `AC-SLICE-08` | Decide whether named execution profiles are accepted and, if so, define them independently of runtime acquisition modes | **Implementation complete; Verification pending.** `run`/`resume --profile NAME` resolve one safe Project-local `<project-root>/emrys.execution.NAME.yaml` fragment; the mutually exclusive explicit-path selector remains. Both use the existing v1 admission and source/effective provenance. A new Run or resource-declaring resume uses default → fragment → CLI-resource precedence; placement-only resume preserves predecessor resources before CLI overrides and explicit resources must match immutable Run identity. The selector name adds no schema, registry, identity, runtime mode, or automatic search. Section 13.26 records the net-negative cut; exact-head aggregate and selected 130-pair hosted acceptance remain. Site/global profile management stays `OPS-01`; runtime acquisition stays `RUNTIME-01`. |
