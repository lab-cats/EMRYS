...
scientific-review, or biological claim. One execution-profile format combines
resource declaration and placement. Project-local `--profile NAME` resolution
is implemented without scanning, a registry, Project-schema mapping, identity,
or runtime coupling. Site/global discovery, precedence, and management remain
open under `OPS-01`; runtime acquisition remains `RUNTIME-01`.
...
default when no file is selected, and accepts either one safe Project-local
name resolving exactly `emrys.execution.NAME.yaml` or one mutually exclusive
explicit path plus the existing admitted resource overrides. It does not select
runtime provisioning mode, storage policy, or a second application backend.
Site/global discovery, precedence, registry/management, storage integration,
and the final safe override roster remain open under `OPS-01`.
...
the built-in direct profile by default, one Project-local `--profile NAME`, or
one explicit path; a Slurm profile causes exactly one outer submission and the
allocated process runs the same local scientific backend. Automatic site/global
discovery, broader precedence/management, and the eventual role-aware surface
remain open.
...
- The implemented portion of the `ANALYSIS-02` v1 boundary admits one explicitly selected
  ...
  evidence admission remain EMRYS-owned. The external-provider path satisfies
  the selected ownership boundary. The built-in paired-CMH module still plans
  Step 10 through sibling implementation and motif assets outside its declared
  implementation package/hash, and preserves its retained scientific HTML
  through exact transition `SRC-TRANS-013`. Moving the paired-specific Step 10
  implementation, assets, and renderer under that module with parity remains
  required.
...
| Documentation | The legacy launcher regression transcript names retired files and commands after its live safeguards moved to owner contracts, tests, CI, the runbook, and configuration guide. | **RETIRE** `LOCAL_PILOT_LAUNCHER_TEST_PLAN.md`; Git history preserves the non-authoritative transcript. `ORCHESTRATION_READINESS.md` remained the sole open `DOC-05` transition source at this cut and was retired by the later final tranche. | This is stale-document retirement, not retained-evidence deletion. |
...
| Surface/category | Finding | Disposition and surviving authority | Preconditions/evidence |
|---|---|---|---|
| Public selection and authority | Operators could select only an explicit path even when a reusable profile belonged to one Project. | **Extend without a registry.** `run` and `resume` accept `--profile NAME`, resolve exactly `<project-root>/emrys.execution.NAME.yaml`, and feed that path to the existing execution-profile admission owner. `--execution-profile PATH` remains supported and is mutually exclusive. | Safe-name rejection, missing-file diagnostics, Project-root resolution, run/resume coverage, and unchanged explicit-path behavior. |
| Identity, precedence, and delegation | A human selector must not become another execution identity or be reinterpreted inside an allocation. | The name is selection metadata only. The existing effective computational declaration remains Run-bound; exact source path/bytes, placement, allocation, and scheduler job remain Attempt provenance. A new Run, or a resume fragment declaring resources, follows packaged default → one selected fragment → CLI resource overrides. Placement-only resume preserves predecessor resources before CLI overrides; explicit resources must remain compatible with immutable Run identity. The Slurm delegate receives the exact resolved path plus the existing effective-profile digest. | Equal named/explicit fragments retain equal effective semantics; mutation before delegation still fails the digest check. |
| Product compression | Run/resume repeated execution-selector, resource, and logging argument setup; resume mirrored whether resources were explicit; resource merging retained a one-use helper and a schema-impossible fallback. | Consolidate shared arguments, derive explicit-resource status from immutable `ResourcePolicy.config_path`, inline the one-use merge, and rely on the surviving pre-merge schema validation for mapping shape. | Existing external-input/schema, resource, resume, allocation, and delegate defenses remain. |
| Configuration/tooling compression | The generic Slurm example and hosted driver copied the packaged default resource block. | Remove the duplicate resource copy. Placement-only fragments inherit packaged resources for a new Run and predecessor resources on resume. Retain the Viking profile's genuinely customized resource policy. The driver's outer `--profile 130|100000` remains its dataset-scale selector; its public scheduled EMRYS invocation uses `--profile ci`. | Placement-only fragments remain schema-valid and preserve the applicable new-Run or resume resource policy. |
| Protection, logging, mutation, and evidence | The selector reaches an execution boundary but changes no execution or evidence authority. | Preserve canonical/nonsymlink reads, closed YAML/schema admission, retired-adjacent fail-closed behavior, exact source/effective digests, allocation checks, no-write/no-submit behavior, lifecycle re-admission, and historical reads. Existing run/resume `LOG-05` ownership applies; no logger or durable event changes. No mutable registry/cache or Run mutation is added. | No retained evidence is moved, rewritten, or deleted. |
| Deferred scope | Project-local fixed-name resolution does not establish broader management. | **Defer** site/global search, precedence, registry/management, storage integration, and final override roster to `OPS-01`. Managed/Site/Explicit runtime acquisition, portability, and taxonomy remain `RUNTIME-01`. | No automatic selection, environment lookup, Project-schema mapping, second backend, or fixed global profile taxonomy. |

| Accounting category | Exact implementation actual |
|---|---|
| Maintained product Python | Two existing paths, `+36/-40`, net `-4`; no product-file growth. Shared selector/resource/log argument composition, mirrored resume state, and the one-use merge layer compress while every admission authority remains. |
| Protections/tests/tooling | Three existing paths, `+148/-16`, net `+132`; safe names, missing files, mutual exclusion, run/resume resolution, inheritance, driver behavior, and retained explicit-path behavior are covered. |
| Configuration | One existing path, `+2/-40`, net `-38`; the generic Slurm fragment inherits the byte-identical packaged resource default, while the custom Viking policy remains. |
| Documentation | Nine existing paths, `+159/-90`, net `+69`, including canonical operator guidance, lifecycle status, and this closeout record. |
| Retained evidence and compatibility | Zero retained evidence added, moved, rewritten, or deleted. Explicit-path and historical-format support remain; no compatibility alias is added. |
| Public concepts and mutable state | One new flag spelling on two existing commands; no schema, package export, backend, runtime mode, public noun, registry, cache, or mutable authority. |
| Shell and `LOG-05` | No shell surface changes. Existing run/resume logging ownership applies; no event, log owner, or durable diagnostic changes. |
| Whole slice | Fifteen existing paths, `+345/-186`, net `+159`; no file growth. Categories remain independently interpreted. |

`AC-SLICE-08` is **Complete**. Exact implementation commit
`220343f6c0fd5126e3b916df1bbc907980d0888f` passed all four Python 3.11 shards,
complete-and-disjoint shard-receipt verification, the selected 130-pair real
synthetic direct/disposable-single-node-Slurm E2E, retained-evidence uploads,
checkout cleanliness, and final selected-lane enforcement in hosted run
[33497912732](https://github.com/lab-cats/EMRYS/actions/runs/33497912732).
The 100,000-pair and ordinary non-selected manual-dispatch lanes were skipped.
This is disposable single-node synthetic Slurm engineering evidence, not
institutional-site/global management, multi-node or production execution,
failure/recovery parity, scientific review, or biological validation.

### 13.27 Bounded slice record: final `DOC-05` transition-source retirement

| Surface/category | Selected implementation and boundary |
|---|---|
| Authority trace | The former orchestration-readiness matrix contributes no surviving authority. Profile membership and direct edges remain with `STAGE_MAP.md`, the workflow profile, and exact projection tests; shared task admission, semantic all-pass, evidence, retry, and resume guarantees remain with the orchestration contract and test policy; exact scientific and transaction behavior remains owner-local. |
| Retirement and protection | The 112-line Campaign B transition matrix retires after section-level reconciliation. The documentation gate moves it from the required visibly marked legacy roster to the returned-retired-source guard, matching the already retired launcher transition plan. Git history preserves both sources. |
| Compression and non-goals | This is documentation-net-negative and removes one maintained file. It changes no product implementation, configuration, schema, workflow, public interface, mutable authority, retained evidence, runtime, log, scientific behavior, or profile/task roster. `DOC-04`, `DOC-01`, broad `AC-SLICE-17`/`AC-SLICE-18`, `ARCH-01`, and `OPS-04` remain Open. |
| Evidence boundary | The repository-and-delivery decision contains the [durable source-to-owner trace](../design/decisions/repository-and-delivery.md#doc-05-orchestration-readiness-trace-2026-09-01). Documentation structure, return guards, all-pass/profile protections, source-dependency enforcement, and exact diff checks are the proportionate local gates. No runtime, scheduler/site, production, scientific-review, or biological claim follows from this retirement. |
...
| `AC-DEC-005` | What broader merge semantics remain beyond the implemented execution-profile boundary? | A new Run, or a resume fragment declaring resources, uses packaged defaults, one named or explicit closed fragment, then owner-defined CLI resource overrides. Placement-only resume preserves predecessor resources before CLI overrides. Source/effective provenance remains retained. Project-local named selection is fixed; site/global precedence and broader list/map/null semantics remain Open under `OPS-01`. |
| `AC-DEC-006` | How are runtime and named execution choices represented? | One file-bound direct/Slurm execution profile is implemented; a safe Project-local name selects one fixed file without a registry, while an explicit-path selector remains. Runtime is separate: institution-provided discovery publishes one Project-owned admitted authority that ordinary commands derive, and Doctor implements the first bounded Managed installation/repair. Site/global execution-profile registry/management remains `OPS-01`; Managed/Site/Explicit runtime acquisition, final taxonomy, broader portability, and persistence remain `RUNTIME-01`. External mechanisms stay behind owned boundaries and supported realizations owe equivalent declared guarantees. |
...
| `AC-SLICE-08` | Decide whether named execution profiles are accepted and, if so, define them independently of runtime acquisition modes | **Complete.** `run`/`resume --profile NAME` resolve one safe Project-local `<project-root>/emrys.execution.NAME.yaml` fragment; the mutually exclusive explicit-path selector remains. Both use the existing v1 admission and source/effective provenance. A new Run or resource-declaring resume uses default → fragment → CLI-resource precedence; placement-only resume preserves predecessor resources before CLI overrides and explicit resources must match immutable Run identity. The selector name adds no schema, registry, identity, runtime mode, or automatic search. Section 13.26 records the net-negative cut. Exact implementation commit `220343f6c0fd5126e3b916df1bbc907980d0888f` passed all four Python 3.11 shards, complete-and-disjoint shard-receipt verification, and selected 130-pair hosted acceptance in run [33497912732](https://github.com/lab-cats/EMRYS/actions/runs/33497912732). Site/global profile management stays `OPS-01`; runtime acquisition stays `RUNTIME-01`. |
...
| `AC-SLICE-18` | Rewrite navigation and documentation around scientist/operator/developer journeys | The bounded `DOC-05` retirement tranche is complete. Broad role-journey work remains Open under `DOC-01`; use the accepted `DOC-02`/`DOC-03`/`DOC-05` traces and coordinate with open `DOC-04`. |
...
| `OPS-01` | The current operator surface is one optional closed execution-profile fragment selected by safe Project-local name or mutually exclusive explicit path, plus resource overrides, with implemented precedence and source/effective provenance. The name is selection metadata only. Site/global discovery, registry/management, broader precedence, storage/operator integration, and the final safe override roster remain Open. |
...
| `ANALYSIS-02` | **In progress.** One explicitly selected installed v1 module declares configuration admission, one or two downstream Step 09/10 tasks, typed dependencies and artifacts, validation, runtime/resource needs, one implementation package, and provider provenance/trust. External modules own bespoke scientific rendering over admitted results; EMRYS owns evidence/operations rendering and report validation, publication, receipt, default invocation, opt-out, and independent regeneration. The built-in paired-CMH module still plans Step 10 through sibling implementation and motif assets outside its declared implementation package/hash and delegates its retained scientific HTML through exact transition `SRC-TRANS-013`; moving those paired-specific owners into the module with parity remains required. No universal Stage hierarchy, workflow language, second scheduler, auto-installation, generic report schema, or customizable-section DSL is introduced. Mandatory compression/exception closeout and exact-head CI also remain pending. |
...
| `DOC-02` | Completed repository-wide documentation disposition and authority cutover; bounded documentation migration and retirement now remain under completed `DOC-03`/`DOC-05` and open `DOC-04`; `CLEAN-01` is Verification pending and `CLEAN-02` is complete |
...
| `DOC-05` | **Complete.** The launcher transition plan and orchestration-readiness matrix are retired after every durable safeguard moved or was traced to current execution-profile, transport, onboarding, package, orchestration, semantic-owner, workflow/stage-map, test, CI, runbook, and configuration authorities. Both paths are guarded against return; stale chronology and transition dispositions remain in Git history. |
