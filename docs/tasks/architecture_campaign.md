...
|---|---|---|---|
| `ARCH-CLOSE-01` Supported ordinary journey | From the exact Project root, the installed `emrys` command discovers `project.yaml` and `runtime/profiles/default.yaml`; ordinary `validate`, `run`, `inspect`, `resume`, and `report` require no interpreter path, Run-root path, or repeated Project/profile arguments. `--analysis` is needed only to disambiguate Analyses. Named or absolute profiles use one `--profile` option. Omitted Run selectors resolve the sole Run or offer an interactive human-name picker; automation must supply an unambiguous human name, exact Run ID, or unique ID prefix. Reconcile runtime terminology and verify managed runtime, Doctor, collaborator dependency/resource admission, automatic reporting, no-write refusal, and the resulting golden path together. | `AC-SLICE-08`, `AC-SLICE-13`, residual `OPS-02`, `RUNTIME-01`, and `ANALYSIS-02` verification | Focused command/noninteractive tests, ordinary CI, and the managed real-tool direct golden path. |
...
| `OPS-01` | `ARCH-CLOSE-01` verifies Project-local profiles. **Ratified no-op closure** for a site/global profile registry; named profiles resolve only beneath the selected Project, while an absolute path remains explicit. A future demonstrated multi-Project/site need would require a separately approved `PROFILE-REGISTRY-01`. |
...
| `AC-SLICE-08` | Decide whether named execution profiles are accepted and, if so, define them independently of runtime acquisition modes | **Implementation present; verification pending.** Section 13.26 owns the selected boundary and compression record. Exact-head standard CI and the selected 130-pair hosted lane remain; site/global management stays `OPS-01` and runtime acquisition stays `RUNTIME-01`. |
...
| `OPS-01` | Section 13.26 records implemented Project-local execution-profile selection. Omission selects `runtime/profiles/default.yaml`; a safe name selects that Project's `runtime/profiles/NAME.yaml`; an absolute path is exact. No site/global registry is accepted. Exact-head proof in `ARCH-CLOSE-01` is the remaining closure gate. |
...
| `RUNTIME-01` | Discovery is the one dry-run-first admission route publishing Project-owned `runtime/runtime.tsv`, which run, resume, and Doctor derive; generic inventory-driven inspection remains advanced evidence. Managed Doctor repair delegates locked `.venv`, native/R, and R-package provisioning to `uv`, Pixi, and `renv`; Site and Explicit users prepare their environments with existing institutional or package-manager tools and then use the same discover/admit contract. Ordinary CI proves the unchanged managed lock in Rocky, Ubuntu, and Debian userspaces; the real-tool direct golden-path job remains the exact-head gate. Container and institutional execution proof remain separate follow-ons, not additional runtime object types or a `runtime define` command. |
...
| `ARCH-01` | Consumes the completed prerequisites and current vertical cutovers. Sections 13.6, 13.19, and 13.20 leave one execution-profile owner, one private whole-Run Slurm transport/bootstrap, one closed project-v1/named-Analysis source, and grouped control while retiring split configuration, the generated wrapper, all sixteen owner-local scheduler routes, and active request-v3 intake. Run remains immutable, placement/diagnostics remain Attempt-local, Attempt-v1 evidence and Results authority remain unchanged, and no second backend, scheduler, facade, Run Bundle, Artifact Store, or evidence deletion is introduced. Only `ARCH-CLOSE-02` parity and `ARCH-CLOSE-03` caller/measurement reconciliation—including the selected `run_coordinator` rename—remain campaign work. Generalized-backend evaluation remains required near closure. |
