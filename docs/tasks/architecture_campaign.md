...
|---|---|
| Public selection | `run` and `resume --profile NAME` resolve exactly one safe Project-local `<project-root>/emrys.execution.NAME.yaml`. The mutually exclusive `--execution-profile PATH` remains the advanced escape hatch; both use the existing v1 admission owner. No scan, registry, schema field, runtime mode, or identity is added. |
| Resume precedence | New Runs use packaged defaults → one fragment → CLI resource overrides. A resume with selected computational fields uses the same precedence and must match immutable Run identity. Placement-only and reporting-only fragments inherit predecessor computation; selected reporting values overlay predecessor reporting, and CLI overrides remain last. |
...
| `AC-DEC-006` | How are runtime and named execution choices represented? | Sections 9.1 and 13.26 settle the current independent runtime and execution paths. Site/global profile management remains `OPS-01`; remaining acquisition, taxonomy, portability, and persistence remain `RUNTIME-01`. |
