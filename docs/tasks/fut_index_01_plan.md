...
| F02 | Existing validation does not prove build-source bytes | The seven Step 00a checks cover minimum membership, source paths, contigs, and three values, but no source/index hashes or producer release. | Define a fail-closed content and build-provenance admission rule and its honest evidence claim. |
| F03 | Native version and generation settings are incomplete proof | STAR metadata records paths and settings; versionGenome is a compatibility floor, not the producer release. Other index-affecting settings exist. | Specify a closed supported generation recipe and exact producer/runtime version policy. |
| F04 | The whole directory matters | Fifteen files are required, but STAR may add files; alignment receives the directory and the Task boundary snapshots its top-level members. | Bind complete membership and bytes, and settle the treatment of extras, nesting, and drift. |
...
[STAR 2.7.11b writes](https://raw.githubusercontent.com/alexdobin/STAR/2.7.11b/source/genomeParametersWrite.cpp) source path strings, settings, and output sizes, without source-content digests. Matching present-day paths and contig lengths cannot prove which exact sequence and annotation bytes generated a historical index.
...
STAR's [versionGenome default](https://raw.githubusercontent.com/alexdobin/STAR/2.7.11b/source/parametersDefault) describes the earliest index version compatible with that release, not the exact release that generated the index. The same tagged defaults and [metadata writer](https://raw.githubusercontent.com/alexdobin/STAR/2.7.11b/source/genomeParametersWrite.cpp) show other potentially relevant settings and inputs, including suffix-array sparsity, genome transformation, extra splice-junction sources, and GTF feature/tag choices. The [current worker](../../src/emrys/stages/star_index/step_00a_build_star_index.sh) passes the three explicit Project values to STAR genome generation.

**Open decision:** define the supported externally generated recipe, including acceptable defaults and whether extra inputs or transformations are rejected. For a first fail-closed route, require a declared exact producer release matching the admitted runtime STAR release unless a separately reviewed compatibility rule is supported. The runtime executable's identity and a supplier's producer-release claim are distinct facts.

### F04 — Directory closure and change detection

The [stage contract](../../src/emrys/stages/star_index/CONTRACT.md) lists fifteen nonempty regular members as the protected minimum and permits additional native files. The [alignment worker](../../src/emrys/stages/star_alignment/step_01_star_align.sh) passes the entire directory as genomeDir. The [Task boundary](../../src/emrys/orchestration/run_coordinator/task.py) enumerates and content-snapshots every top-level input-directory member when that directory is planned as a Task input. That execution defense does not bind an external directory into Project or Run identity. The shared [input helpers](../../src/emrys/libraries/validation/inputs.py) already support stable, no-follow directory listing and streamed hashing.

**Proposed direction:** admit a deterministic ordered inventory of every present top-level regular member, require the fifteen named members, and bind each name, size, and SHA-256. Reject links and unsafe nesting unless source review demonstrates a required supported STAR form. Recheck membership and bytes at the same trust boundaries as other immutable inputs. Measure repeated hashing cost on representative shared storage before a performance claim.
