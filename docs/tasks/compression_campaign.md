...
|---|---|---|
| CI control and latency | Manual dispatch now independently selects the ordinary and long CI groups. The remaining concern is setup and test latency. | `CI-01` owns measurement and substantive critical-path reduction. Do not propose manual lane selection again. |
...
scores, or implementation approval. The matrix owns accepted work. Except for
the runtime-model direction recorded there, new findings await selection;
...
| 8. Artifact CLI version admission | The [artifact schema API](../../src/emrys/contracts/artifacts/_artifact_contracts/schema.py) uses default schema versions when its unversioned validation helper is called. The artifact CLI follows that route, while current reporting uses newer run-summary and report-receipt versions. Source inspection predicts rejection of current records. | Proposed extension of `CONTRACT-API-01`; its accepted scope currently concerns orchestration records only. Verify the artifact CLI behavior before expanding that row. |
...
| 10. Duplicate runtime definitions | [Private models](../../src/emrys/evidence/runtime_availability/_runtime_model.py) and [public inspection models](../../src/emrys/evidence/runtime_availability/inspector.py) repeat check and observation fields and copy between them. The agreed direction is one immutable definition of each, using path objects internally for resolved filesystem locations. | Captured as `RUNTIME-MODEL-01` in the [backlog](backlog_matrix.md#repository-maintenance). Its acceptance criteria and implementation authority live there. |
| 11. Whole reference reads | [Reference inspection](../../src/emrys/evidence/reference_provenance/_reference_contigs.py) reads whole FASTA and STAR Genome files to calculate hashes and lengths, and builds whole-text input for parsing. Existing streaming hash and iterable parsing mechanisms may remove these allocations. Preserve the second observation where it detects changes during inspection. | Proposed memory reduction. No timing or peak-memory measurements were taken. |
| 12. Repeated FASTQ scans | The [FASTQ check](../../src/emrys/ingestion/sample_manifest_admission/check_fastq_pairs.sh) counts each complete file and then scans it again for each selected read ID: the default prefix of 20 requires 21 passes per mate. A single pass could preserve complete record counts, decompression failures, and the explicitly limited prefix comparison. | Existing `OPS-03` first decides whether this standalone helper remains useful, then owns any migration or retirement. No benchmark was run. |
| 13. Repeated processing declarations | [Materialization](../../src/emrys/orchestration/run_coordinator/materialization.py) repeats task commands, inputs, outputs, and validation details in command and dispatch construction. Investigate deriving equivalent declarations from existing admitted facts. | Existing Run-coordinator concentration finding below. No new registry, framework, or particular shared design is selected. |
| 14. Source-topology rosters | [Dependency checks](../../tests/tools/source_dependencies.py), tests, and [source topology](../../src/emrys/contracts/SOURCE_TOPOLOGY.md) maintain overlapping path and composition rosters. Determine which can be derived or replaced by a maintained boundary-checking tool while preserving real dependency rules. | Existing artifact/source-topology family. Tool suitability has not been assessed. |
...
| 16. Parallel configuration normalization | [Project normalization](../../src/emrys/orchestration/run_coordinator/normalization.py) and [application modeling](../../src/emrys/contracts/orchestration/application_model.py) retain overlapping flat paired-CMH configuration and module-policy forms. Trace current and historical inputs before proposing one surviving representation. | Existing configuration and compatibility families. Public migration and the achievable reduction remain undecided. |
| 17. Repeated reporting declarations | Reporting command arguments, outputs, and kinds recur across owners, and reporting materialization contributes to the implementation identity bound to a Run. Derive equivalent declarations once and prevent reporting-only changes from changing scientific Run identity. | Existing `REPORT-ROSTER-01`; preserve immutable Runs, historical reads, report regeneration, and module-specific reporting. |
| 18. Report check identities | Existing [artifact-adapter tests](../../tests/reporting/test_artifact_adapters.py) show that reordered or different unique check IDs can still be treated as complete. Completion should follow the admitted roster's actual identity and order when that roster is defined. | Existing `REPORT-ROSTER-01`. External modules' expected check IDs still need a contract decision; preserve independent expected-result tests. |
