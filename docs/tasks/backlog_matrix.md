...
| ID | Kind | Status | Importance | Complexity | Required outcome | Acceptance |
|---|---|---|---:|---:|---|---|
| `COMPRESS-DOC-01` | Developer tooling | Verification pending | `2` | `2` | Retire documentation-path bans whose integration purpose has ended. | Remove the 18 retired-file and five task-directory exclusions and their dedicated tests; preserve current repository, canonical-document, owner, link, anchor, and Mermaid checks. An otherwise valid old pathname becomes admissible. No retained evidence or repository files are deleted. Approved tooling-only exception: product code stays unchanged; reduce developer-tool and test code, with no new files or dependencies. |
| `COMPRESS-IMPORT-01` | Developer tooling | Open | `2` | `2` | Consolidate fixed reporting import permissions into the existing exact-exception mechanism. | Preserve all 12 current source/target permissions and their durable purposes, reject neighboring imports, and newly reject stale reporting permissions through existing validation. Retire the separate reporting bypass; add no dependency, policy registry, or checker. Approved tooling-only exception: product code stays unchanged; reduce developer-tool code and preserve independent policy tests. |
