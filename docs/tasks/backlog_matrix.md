...
boundaries, remaining caller migration, and temporary-adapter retirement
remain Open.
...
migration, and historical read/resume compatibility. Public Project/Results,
role-aware APIs/CLI, generalized backend and policy interfaces, remaining
migration, and temporary-projection retirement remain Open. A
...
| ID | Priority (provisional) | Status | Required outcome | Acceptance |
|---|---|---|---|---|
| <code>CONTROL-01</code> | P1 | Open | Realize the ratified compact role-aware public model <code>Project -&gt; Analysis -&gt; Run -&gt; Results</code>, with Attempt progressively disclosed and generated internals hidden from ordinary operation but inspectable. | Project is mutable organizational context; an admitted Analysis revision is immutable scientific intent; public immutable Run binds exactly one Analysis revision to one internal immutable inspectable Execution Plan and owns the primary ordinary ID; Results is read-only; Attempt appears for retry, failure, recovery, or advanced inspection. Dataset, Reference, and ExperimentalDesign remain scientific-definition sections, Runtime/profile is operator-facing, Artifact advanced, Task internal, and Report a downstream Results capability. The first internal successor boundary now supplies immutable Analysis/Execution-Plan/Run records and migrates the current low-level run path, but public Project/Results types, role-aware APIs/CLI, progressive disclosure, storage surface, remaining compatibility migration, and adapter retirement remain Open. |
...
| <code>RUN-03</code> | P1 | Open | Realize a coherent journey around public Run as the immutable binding of one admitted Analysis revision and one internal immutable Execution Plan while eliminating manual transfer of run roots or internal state between phases. | Scientific-intent changes create a new Analysis revision and Run; changes to identity-bearing effective toolchain/backend/profile/resource-policy semantics create a new Run; raw config, locator, placement, logging, and report-only changes do not. Re-executing the same plan creates an Attempt; actual realization may vary only inside the Run's declared permissible envelope. Attempt state cannot alter or reconstruct Run. Reporting is default-on for full runs, disable-able, independently regenerable, and creates neither Run nor Attempt. The current path now constructs and commits the immutable successor Run before allocation/Attempt, supports zero-Attempt inspection and compatible resume, and keeps historical Runs readable/resumable. The coherent public one-command journey, role-aware operation signatures, generalized executor/policy realization, remaining migration, and temporary adapter retirement remain Open. |
| <code>IDENTITY-01</code> | P2 | Open | Realize the ratified smallest role-appropriate identity model while preserving detailed identities as evidence metadata. | Ordinary users need one primary Run ID; Analysis may be human-named while retaining internal immutable identity; Attempt ID is surfaced only when truthful retry, failure, recovery, or advanced inspection requires it. Relocation-independent Analysis/Execution-Plan identities and the domain-separated Run digest over their canonical digests are now implemented for successor Runs; human aliases, raw formatting/order, paths, Attempt facts, reporting, and backend-adapter-only code are excluded. Historical-reader mechanics preserve existing Runs. Ordinary public exposure, progressive disclosure, remaining caller migration, and legacy/projection retirement remain Open. Existing commit, package, runtime, request, task, artifact, receipt, and publication identities remain detailed evidence metadata, and no subsystem may reconstruct a competing Run identity. |
...
| <code>CONTROL-01</code>, <code>RUN-03</code>, <code>IDENTITY-01</code>, <code>ARCH-01</code> | The completed audit and decisions selected model C, the Run/Attempt boundary, exact semantic fields, domain-separated identities, symbolic resource envelope, logical authorities, recovery ownership, and separated status domains. The first successor cutover now provides immutable canonical Analysis-revision, Execution-Plan, and Run-binding records; publishes the Run binding last; migrates new-Run planning/root selection/execution/resume/inspection; preserves historical read/resume; and supplies workflow/task/reporting through a temporary one-way projection. | Public Project/Results, role-aware APIs/CLI, generalized backend/policy boundaries, public status vocabulary, remaining caller migration, compatibility duration, and projection retirement remain Open. Current low-level names are not automatically public, historical evidence remains readable, and no evidence deletion is authorized. |
