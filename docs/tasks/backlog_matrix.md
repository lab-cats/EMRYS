...
| <code>PERF-06</code> | P1 | Open | Replace Step 06's repeated filters, merges, count scans, and temporary BAMs with a one-pass orientation dispatcher. | Header, record membership, flags, coordinate order, counts, indexes, failure behavior, and downstream Step 07 evidence satisfy the accepted parity oracle; the implementation removes the four flag-specific temporary BAMs and demonstrates lower read/write traffic, peak scratch, and wall time at representative scale. |
