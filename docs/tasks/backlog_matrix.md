...
| `DEV-01` | Developer feedback | In progress | `3` | `2` | Add ShellCheck, consistent Python formatting, and fast local pre-commit hooks through existing tool owners. | Preserve the completed Bash syntax gate, broadened Ruff correctness rules, and shared validation inventory. Run ShellCheck and Ruff formatting checks in the established quality gate; use Ruff for the mechanical formatting baseline and the maintained pre-commit tool for applicable staged-file checks. Hooks perform no workflow, R, or full-suite execution. Keep formatting separate from functional changes and report it separately from compression savings. |
| `CLI-VERSION-01` | Public CLI | In progress | `2` | `1` | Show installed EMRYS version information with `emrys --version [-v]`. | The ordinary flag reports the package version; `-v` adds available installation/source details through existing information owners. It works without a Project or scientific runtime, does not create logs or mutate state, and retains normal command parsing and errors. |
