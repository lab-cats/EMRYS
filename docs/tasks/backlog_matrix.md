...
The [RELEASE-01 readiness investigation](release-readiness.md) records the
working findings and evidence questions. This matrix retains the outcome,
...
selection and reuse intent. CV-26 is Completed under the owner's revised
structural acceptance: its original complete-operation attribution and
institutional E11 timing requirements were superseded on 2026-09-22.
...
- [ ] **One institutional campaign:** fresh novice setup using the
  ...
  acceptance; CV-26 has no institutional timing gate. Retain exact
  package/Run/Attempt/profile/input/runtime identities.
...
| `CLUSTER-VERIFY-01` | Cluster verification campaign | Open | `4` | `5` | Resolve the recorded cluster-walkthrough failures and operator gaps while preserving scientific and recovery authority. | The [campaign charter](cluster_verification_campaign.md) owns scope and evidence; this matrix delegates `CV-01` through `CV-27`, `CV-U01` through `CV-U33`, and `CV-UX-01` status, acceptance and priorities to the [cluster verification backlog](cluster_verification_backlog.md). Several selected source and guide corrections are implemented; `INIT-02` and `CV-U22` retain the original-intent gaps named above and in their cards. CV-26 is Completed to its owner-revised structural criterion without an institutional timing gate or speedup claim. Preserve accepted trust limits and unexplained historical observations. Remaining acceptance also includes exact final-source hosted/disposable-Slurm evidence, coordinated institutional execution for the other applicable cards, separate review records and verified dispositions; Deferred work stays with `CLEANUP-01` and `INTERACTIVE-01`. Follow the [closure checklist](#cluster-verification-closure-checklist), then close only under the charter's disposition and evidence criteria. |
