...
| <code>PERF-03</code> | P1 | In progress | Make bounded BAM/BAI signature validation actually bounded in file size. | Validation reads only the required fixed prefix, retains exact short/malformed-file rejection and diagnostics, and increasing-size benchmarks demonstrate constant bytes read and bounded memory. |
