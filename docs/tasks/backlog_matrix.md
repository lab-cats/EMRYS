...
| `REPORT-ROSTER-01` | Architecture reduction | In progress | `3` | `4` | Remove duplicated reporting declarations while preserving bespoke module reports. | The [fixed-output consolidation](../design/decisions/execution-evidence-and-reporting.md#fixed-report-output-consolidation) and [reporting lifecycle retirement](../design/decisions/execution-evidence-and-reporting.md#reporting-lifecycle-compression) are implemented in PRs #145 and #147 and passed ordinary hosted CI in PR #140, awaiting master integration. The latter removed 386 net product lines with no new product files, retiring six callback carriers, the HTML facade, and private replacement publication across all three producers. Neither delivered slice closes the broader outcome; the following work remains. [CS-02–05 and CS-11](compression_backlog_matrix.md#working-queue) own the finite remaining reporting slices and their decision gates. Derive kinds, counts, order, outputs, command inputs, validation roster, paths, and implementation identity from existing admitted authorities only where the complete caller audit establishes equivalent semantics; do not create an inspection/reporting cycle. Report-only materialization must live outside the Run-bound implementation closure so a reporting-only change cannot alter Run identity or prevent a scientifically compatible resume. Delete `reporting_memory_mb` from active configuration if no execution consumes it, otherwise move it to its real execution owner. Retire only caller-complete duplicate tables, permissions, and validation; different roster, status, historical, or trust-boundary semantics remain until an explicit replacement decision. Preserve artifact-index -> run-summary -> HTML ordering, independent regeneration, transaction schemas, locks, receipt-last publication, source/roster rechecks, historical reads, independent goldens, module-derived indexing, the minimal `emrys.analysis_reporters` extension point, and EMRYS's fixed Evidence/operations renderer; add no generic report DSL or catalog service. |

### Repository maintenance

| ID | Kind | Status | Importance | Complexity | Required outcome | Acceptance |
|---|---|---|---:|---:|---|---|
| `COMPRESS-01` | Finite compression campaign | In progress | `4` | `4` | Execute and close the finite owner-level compression backlog with meaningful maintained-product reduction. | The [temporary compression backlog](compression_backlog_matrix.md) owns CS-01–15, their remaining scope, status, prerequisites, and acceptance; the [campaign](compression_campaign.md) owns scope and retirement. Begin selection with substantial processing materialization and report-adapter candidates, not the opportunistic small edits. Close every card by evidenced delivery, retain/reject rationale, or explicit transfer; give every routed original finding a destination or reopening trigger. Qualification must produce a complete negative proposal or a reasoned no-change result. Product implementation still requires its bounded approval; this conversion does not approve it. Preserve decisions and evidence ceilings in durable owners, reconcile broader parent outcomes here, and obtain final user disposition before explicitly scoped retirement of both temporary documents. |
| `DEV-01` | Developer feedback | Verification pending | `3` | `2` | Add ShellCheck, consistent Python formatting, and fast local pre-commit hooks through existing tool owners. | The existing lint gate now runs locked ShellCheck on every tracked shell file and Ruff formatting checks; actionlint also checks embedded workflow shell. Three pre-commit hooks check applicable staged files using the same locked tools, with no workflow, R, or full-suite execution. The formatting baseline is a separate mechanical commit with unchanged parsed Python code. Local static, shell-owner, workflow-lint, and hook checks pass; final hosted CI remains pending. These additions are the approved tooling exception, not product compression. |
| `CLI-VERSION-01` | Public CLI | Verification pending | `2` | `1` | Show installed EMRYS version information with `emrys --version [-v]`. | Implemented through the existing package version and public parser: the ordinary flag reports the version; `-v` adds loaded-package path and Python version/executable. The display works without a Project or scientific runtime, writes no logs, and retains checkout admission for ordinary commands. Focused CLI and source-policy checks pass; final hosted CI remains pending. No new product file or version registry was introduced. |

## Completed outcomes reconciled in this tranche

The following implemented outcomes passed their applicable Python, guarded-R,
managed-golden, and independent-contract checks in
