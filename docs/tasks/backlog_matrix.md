...
| ID | Priority (provisional) | Status | Required outcome | Acceptance |
|---|---|---|---|---|
| <code>CONTROL-01</code> | P1 | Open | Realize the ratified compact role-aware public model <code>Project -&gt; Analysis -&gt; Run -&gt; Results</code>, with Attempt progressively disclosed and generated internals hidden from ordinary operation but inspectable. | Project is mutable organizational context; an admitted Analysis revision is immutable scientific intent; public immutable Run binds exactly one Analysis revision to one internal immutable inspectable Execution Plan and owns the primary ordinary ID; Results is read-only; Attempt appears for retry, failure, recovery, or advanced inspection. Dataset, Reference, and ExperimentalDesign remain scientific-definition sections, Runtime/profile is operator-facing, Artifact advanced, Task internal, and Report a downstream Results capability. The internal successor boundary supplies immutable Analysis/Execution-Plan/Run records, no longer uses a successor execution projection, derives Run integrity, scientific Attempt outcome, Results, reporting, and recovery independently, and exposes one supported grouped-CLI Run-control surface instead of a parallel direct Python planning API. Public Project/Analysis/Results intake and control, progressive disclosure, storage surface, and remaining public migration remain Open. |
...
| <code>CONTROL-01</code>, <code>RUN-03</code>, <code>IDENTITY-01</code>, <code>ARCH-01</code> | The completed audit and decisions selected model C, the Run/Attempt boundary, exact semantic fields, domain-separated identities, symbolic resource envelope, logical authorities, recovery ownership, and separated status domains. Successor canonical Analysis-revision, Execution-Plan, and Run-binding records are committed Run-last; workflow/task admit exact Run authority; reporting inputs are Attempt-owned and origin-bound; historical read/recovery remains; no successor execution projection remains; the read-only status cutover separates Run integrity, scientific Attempt outcome, Results, reporting, and recovery while preserving receipt-v1; and the grouped CLI is the only supported Run-control surface. | Public Project/Analysis/Results intake and control, generalized backend/policy boundaries, progress/guidance, and remaining public/campaign migration remain Open. Private planning helpers and current low-level names are not automatically public, historical evidence remains readable, and no evidence deletion is authorized. |
