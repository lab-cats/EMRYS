...
| <code>CONTROL-01</code>, <code>RUN-03</code>, <code>IDENTITY-01</code>, <code>ARCH-01</code> | The completed audit and decisions selected model C, the Run/Attempt boundary, exact semantic fields, domain-separated identities, symbolic resource envelope, logical authorities, recovery ownership, and separated status domains. Successor canonical Analysis-revision, Execution-Plan, and Run-binding records are committed Run-last; workflow/task admit exact Run authority; reporting inputs are Attempt-owned and origin-bound; historical read/recovery remains; no successor execution projection remains; and the read-only status cutover separates Run integrity, scientific Attempt outcome, Results, reporting, and recovery while preserving receipt-v1. | Public Project/Results control and intake, role-aware APIs/CLI, generalized backend/policy boundaries, progress/guidance, and remaining public/campaign migration remain Open. Current low-level names are not automatically public, historical evidence remains readable, and no evidence deletion is authorized. |
