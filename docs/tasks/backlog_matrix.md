...
[architecture campaign](architecture_campaign.md). Computational scaling
diagnoses, optimization hypotheses, and experiment rules are preserved in the
[performance campaign](performance_campaign.md). The hashed source attachments
remain the verbatim architecture-intake record. Campaign documents are temporary
sources of context that has not yet been converted into accepted backlog items.
This matrix remains authoritative for task IDs, status, required outcomes,
acceptance conditions, and dispositions; campaign documents do not create or
authorize tasks by themselves.
...
The [performance backlog matrix](performance_backlog_matrix.md) likewise records
only provisional performance-campaign ranking, risk, benchmark, and parity
information. Integration status and acceptance remain here.
...
| <code>PERF-02</code> | P1 | In progress | Make computational performance inspectable in CI through schema-versioned per-stage and full-E2E measurements. | A null comparison validates the harness; retained fixtures cover Steps 02–09 and task-boundary execution across independently varied reads, samples, partitions, candidate rows, and skew; the 100k E2E publishes wall time, CPU, peak RSS, read/write I/O, and peak scratch with dataset, runtime, runner, cache state, baseline SHA, and candidate SHA. Long checks run in CI, and heterogeneous hosted-runner timings are not hard release gates. |
| <code>PERF-04</code> | P3 | In progress | Replace quadratic contig-duplicate detection with linear membership validation. | Valid order and exact duplicate diagnostics are preserved, malformed cases remain fail-closed, and the retained 1,000/4,000/16,000-contig comparison plus deterministic operation-count oracle demonstrate linear rather than quadratic growth. |
| <code>PERF-05</code> | P1 | Open | Eliminate repeated full-byte proof of the same immutable artifact within an admitted task/run lifecycle while preserving content-bound identity and mutation detection. | One authoritative digest or admitted immutable identity is reused instead of task and owner layers repeatedly hashing the same bytes; streamed producer hashing is used where safe; replacement, retargeting, truncation, same-mtime mutation, incomplete writes, and recovery cases fail closed; receipts and provenance remain exact; measured bytes scale with artifact production/admission rather than proof-pass count. This task does not independently settle the broader Artifact Store or cross-run cache architecture. |
| <code>PERF-06</code> | P1 | In progress | Replace Step 06's repeated filters, merges, count scans, and temporary BAMs with a one-pass orientation dispatcher. | Header, record membership, flags, coordinate order, counts, indexes, failure behavior, and downstream Step 07 evidence satisfy the accepted parity oracle; the implementation removes the four flag-specific temporary BAMs and demonstrates lower read/write traffic, peak scratch, and wall time at representative scale. |
| <code>PERF-07</code> | P2 | In progress | Consolidate Step 02 BAM count, read-group, sort/canonicality, and validation scans without weakening admission. | Canonical and noncanonical paths preserve exact BAM/BAI, RG, count, validation, and failure semantics while representative BAM-size benchmarks demonstrate fewer full scans and reduced I/O or wall time. |
| <code>PERF-08</code> | P2 | In progress | Safely reuse valid sidecar indexes produced by Steps 04 and 05 instead of regenerating or rescanning them unnecessarily. | Existing, missing, stale, mismatched, and producer-failure cases are covered; only content-compatible sidecars are admitted; downstream outputs and evidence remain equivalent; benchmarks demonstrate reduced scans, scratch, or wall time. JVM heap and scratch changes remain resource-policy work under <code>PERF-15</code>. |
| <code>PERF-09</code> | P1 | Open | Reduce Step 07 shared-input proof amplification and partition stragglers while enabling safe orientation/partition concurrency. | Shared cohort inputs are proved once through <code>PERF-05</code>; partition work is estimated from accepted indexed evidence rather than equal bases alone where beneficial; uniform and skewed fixtures cover sample, partition, and concurrency scales; final VCFs, evidence, ordering, failure attribution, and local/scheduled behavior retain parity. |
| <code>PERF-10</code> | P1 | Open | Process Step 08 VCF evidence in one deterministic bounded-memory parsing and aggregation path. | Lexical validation and extraction do not independently parse the same complete VCF; deterministic chunks support one-large-file and many-file workloads; worker results do not require retaining all expanded frames; 10k, 100k, and 1m candidate fixtures demonstrate bounded working memory and improved I/O or wall time while final tables, ordering, hashes, and malformed-input behavior satisfy parity. |
| <code>PERF-11</code> | P2 | Open | Reuse a compiled annotation interval model across compatible analyses. | The cache is bound to exact GTF content, runtime/package identity, annotation policy, and schema; incompatible or corrupt entries fail closed; repeated-analysis benchmarks avoid reconstructing the complete model; annotations remain equivalent. |
| <code>PERF-12</code> | P2 | Open | Stream large strict TSV validation without retaining redundant byte, text, row-list, and row-dictionary copies. | Step 08/09 valid and malformed corpora retain accepted diagnostics and global semantic checks; only explicitly required global state is retained; increasing row/column fixtures demonstrate bounded or materially reduced peak memory without weakening validation. |
| <code>PERF-13</code> | P2 | Open | Remove Step 09 per-row allocation overhead through an exact batched or chunked implementation without changing the statistical contract. | Candidate universe, eligibility, counts, CMH method, numeric behavior, multiple-testing family, ordering, and selections satisfy an independent oracle and accepted tolerance; representative candidate/replicate scales demonstrate improvement. No alternative statistical method may enter through this task. |
| <code>PERF-14</code> | P3 | Open | Evaluate compressed, indexed, or columnar internal representations and compression settings that reduce data movement and space without changing admitted scientific content. | Each VCF/BCF, TSV/columnar, BAM/CRAM, or compression-level experiment is isolated; decoded content, ordering, export compatibility, provenance, recovery, and downstream evidence meet its accepted parity contract; only a demonstrated time/space Pareto improvement is integrated. |
| <code>PERF-15</code> | P2 | Open | Replace whole-workflow resource reservations and serial defaults with measured stage-specific CPU, memory, JVM, thread, process, and concurrency policies. | Resource recommendations derive from retained measurements; oversubscription and OOM/failure cases are covered; local and scheduler-shaped execution preserve artifacts and evidence; representative workloads improve makespan or utilization without silently applying node-specific policy. Cross-node scaling remains <code>PERF-01</code>. |
| <code>PERF-16</code> | P2 | Open | Use node-local scratch and explicit ephemeral-artifact retention where it reduces shared-storage traffic without weakening publication or recovery. | Owned scratch is safely created and cleaned; only admitted durable outputs are published; failure, resume, rollback, and cleanup fault cases prevent premature deletion or identity loss; shared-storage I/O and peak-space benchmarks demonstrate benefit. Canonical lifecycle and retirement decisions coordinate with <code>ARCH-01</code> and <code>AC-SLICE-07</code>/<code>17</code>. |
| <code>PERF-17</code> | P2 | Open | Amortize STAR index loading and FASTQ decompression across compatible co-located work. | Qualified index identity remains owned by <code>FUT-INDEX-01</code>; shared-index lifecycle, teardown, failure isolation, and sample concurrency are safe; serial versus shared/loading and decompression trials preserve exact scientific outputs and demonstrate lower aggregate I/O, CPU, memory, or wall time. |
| <code>PERF-18</code> | P3 | Open | Reduce repeated process, interpreter, package, JVM, and scheduler startup for many small compatible tasks. | Persistent-worker and task-batching experiments preserve task isolation, identity, logs, failure attribution, cleanup, and deterministic outputs; small-task fixtures demonstrate a repeatable net win; no engine or public execution-interface decision is silently settled. |
| <code>PERF-19</code> | P3 | Open | Stream subprocess logs with bounded memory and backpressure instead of retaining arbitrarily large output in process memory. | Large-output, slow-consumer, failure, cancellation, and shutdown cases retain complete ordered forensic evidence and accepted lifecycle events while peak memory remains bounded. Role-facing logging adoption remains <code>LOG-05</code>. |
| <code>PERF-20</code> | P2 | Open | Evaluate content-addressed reuse with hierarchical cache keys for compatible immutable artifacts without creating a competing identity or lifecycle authority. | Cache keys bind scientific inputs and policy, tool/runtime identity, source/package identity, and relevant execution contract; corruption, mutation, incompatible policy, failed publication, recovery, and eviction cases fail closed; repeated-run benchmarks demonstrate avoided computation or I/O. Any integrated cache follows the accepted artifact lifecycle and <code>ANALYSIS-01</code> reuse contract. |
| <code>PERF-21</code> | P1 | In progress | Qualify and integrate only independently proven performance improvements on one performance-integration branch. | Every candidate is a separate logical commit with baseline/candidate SHAs, parity result, raw CI measurements, and evidence ceiling; existing 100k runs are reused as correctness evidence where applicable; no-signal, regressing, or parity-failing changes are excluded; after every admitted change the combined branch reruns the affected retained matrix and 100k E2E, and final aggregate evidence records interactions and remaining regressions. |
...
| 2026-08-26 | <code>PERF-03</code> | Descriptor-bound validation requests and returns exactly four bytes from each 8-GiB sparse BAM/BAI fixture while preserving short, empty, symlink, replacement, mutation, and diagnostic behavior. In the independently audited <code>alignment-signatures-mib</code> case from [Phase 1 CI run #63](https://github.com/lab-cats/EMRYS/actions/runs/32935564047), all 30 trials and 90 phase records passed with one exact parity digest for baseline [<code>c5a9bcc9</code>](https://github.com/lab-cats/EMRYS/commit/c5a9bcc963e328b488871bb27c4bb1f37f16a196) and head [<code>e99163ae</code>](https://github.com/lab-cats/EMRYS/commit/e99163ae48342fa57e416b1f2f4b7111a1e020f5). Across 10, 100, and 1,024 MiB sparse pairs, head median RSS remained 33,398, 33,368, and 33,366 KiB while baseline median RSS grew to 53,190, 237,872, and 2,135,642 KiB; paired median wall-time savings were 0.016712, 0.146991, and 2.159269 seconds. | Exact fixed-prefix engineering tests plus case-scoped hosted warm-cache synthetic evidence. Run #63 itself failed only after this complete identity case, so it is not recorded as an aggregate pass. No cold-I/O, shared-storage, scheduler, cluster, production-data, scientific-review, or biological claim is made. |
...
|---|---|---|---|
| 2026-08-26 | <code>PERF-03</code> | Complete | Fixed-prefix byte-count tests, fail-closed regressions, exact artifact parity, and increasing-size retained measurements satisfy every acceptance condition; broader immutable-artifact proof remains <code>PERF-05</code>. |
