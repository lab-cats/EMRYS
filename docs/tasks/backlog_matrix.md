...
| `RUNTIME-MODEL-01` | Owner consolidation | Open | `3` | `2` | Use one immutable definition of a runtime check and one of its observation throughout runtime inspection. | Profile loading, probes, report rendering, inspection, and Doctor use the same definitions; retire duplicate models and field-copying conversions across all callers. Resolved filesystem locations use `pathlib.Path` or `None` internally, with text conversion only at existing serialization boundaries; executable names, R namespaces, and other non-path values remain text. Preserve public imports/interfaces, exact TSV bytes and ordering, statuses, diagnostics, immutability, path/symlink semantics, and read-only inspection. Reuse the existing owner and standard-library data classes; make a meaningful net reduction in product code with no new product files, adapter layer, or generic runtime framework. Focused production-path tests cover the surviving behavior. Implementation requires a separately approved bounded plan. |
