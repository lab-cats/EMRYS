...
| ID | Kind | Status | Importance | Complexity | Required outcome | Acceptance |
|---|---|---|---:|---:|---|---|
| `QUICKSTART-01` | Reader journey | Open | `4` | `3` | Make the Quickstart a short, actionable path from inputs to understood Results. | Keep critical scientific choices, actions, expected outcomes and recovery links. Remove history such as retired STAR questions, derivation/hashing narration, exhaustive preview field lists, inactive background thresholds and internal command mechanics. Keep necessary comparison direction and target-change choices clear. Explain where scientific data, reports and essential provenance are and what each output file represents, linking detailed owner references. Move the optional synthetic smoke test into its own Markdown guide and link it where setup/reuse decisions arise. Coordinate `INIT-01` through `INIT-03` and `SCRATCH-01`; describe delivered behavior rather than promise pending UX changes. |
