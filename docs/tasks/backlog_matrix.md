...
The method notation is an intake illustration, not a selected application API
or authorization for a mutable Run aggregate. Wherever the campaign uses
`Run`, it means the immutable plan; whether Run is public, how it is represented
or persisted, and all surrounding nouns and nesting remain post-audit choices.
...
Project/Analysis/Result vocabulary and nesting, the public exposure of Run, and
setup/init, validate/check, status/resume, synthetic/demo, inspect, and report
spellings remain open design choices. The binding requirement is a small
coherent role-appropriate surface, not these exact names.
...
authority. Except for Run meaning the immutable plan, exact expert interfaces,
public vocabulary and nesting, layers, APIs, backends, policies, and facade use
...
**Run is the immutable plan**; every other application-model choice remains
post-audit, meaning after audit review and a separate approved decision. A
...
| ID | Priority (provisional) | Status | Required outcome | Acceptance |
|---|---|---|---|---|
| <code>CONTROL-01</code> | P1 | Open | Establish a compact role-aware public conceptual model and explicitly decide the relationship among Project, Analysis, Dataset, Reference, ExperimentalDesign, Runtime, Run, Result, Attempt, Task, Artifact, and Report. | Ordinary scientific work can be explained and operated through the smallest accepted vocabulary; advanced scientists, operators, and developers can progressively inspect the deeper model; generated manifests, control files, engine state, and low-level identities remain available but are not required user-authored concepts. Run is bindingly the immutable plan. The task decides which changes create a new Run rather than a new Attempt; whether Run itself is public and every other noun, nesting, cardinality, field, identity input, API, persistence, and compatibility choice remain post-audit decisions. |
...
| <code>RUN-03</code> | P1 | Open | Define a coherent run journey around Run as the immutable plan while eliminating manual transfer of run roots or internal state between phases. | Run is never modified in place; any changed plan is a distinct Run value, and attempt-local state cannot alter it or reconstruct a competing plan. The accepted journey presents scientific intent, effective execution choice, resources, reuse decisions, and the immutable Run at the appropriate point; supports interactive and noninteractive operation; and preserves provenance plus Local/HPC correctness. A single command followed by confirmation is a source proposal, not settled command structure. Run identity/storage, command boundaries, ordering, Attempt/Result relationships, operation signatures, and executor-selection policy remain post-audit decisions. |
...
|---|---|---|
| <code>CONTROL-01</code>, <code>REVIEW-UX-03</code> | The public surface must become substantially simpler and use progressive disclosure for scientist, advanced-scientist, operator, and developer roles. Project/Run/Result remains a leading source-proposed grouping, but wherever Run is used it means the immutable plan. Project/Analysis/Run/Result and the placement of Dataset, Reference, Design, Runtime, Attempt, Task, Artifact, and Report remain alternatives to evaluate. | Simplification preserves guarantees and uses binding role-based progressive disclosure. Every effective operational value and source remains inspectable, while only owner-defined safe values are overrideable. Except for Run meaning the immutable plan, the exact expert interface, safe override roster, public nouns and nesting, identity/cardinality model, command names, percentages, and concept counts require post-audit decision. |
| <code>CONTROL-01</code>, <code>RUN-03</code>, <code>IDENTITY-01</code>, <code>ARCH-01</code> | Completed <code>ARCH-MODEL-AUDIT-01</code> records that <code>contract/normalized.json</code> is the closest durable immutable contract for the current path-sensitive identity-envelope digest, while <code>AttemptPlan</code> is the closest complete effective executable plan but is ephemeral, attempt-specific, and only shallowly immutable. It maps the dry-run discontinuity, write-before-exact-attempt-preparation and unreceipted-skeleton gap, identity inputs and omissions, Local-within-Slurm wrapper, status/report coupling, owners/callers, mutable boundaries, protected evidence, and conditional compression register. | Run-as-immutable-plan is binding. The audit does not decide whether Run is public or select its fields, identity composition, surrounding nouns or nesting, API, persistence, recovery owner, backend representation, status model, compatibility, or migration. Current implementation type names are descriptive only, and no evidence deletion is authorized. |
