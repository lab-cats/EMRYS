...
| `RECOVERY-01` | Transaction recovery | Open | `4` | `3` | Preserve retryable state when multi-file publication or storage-qualification finalization is interrupted. | Steps 07–09 no-clobber rollback treats an absent final as never published while removing only present finals that still match their owned staging inode. Direct and two-phase storage qualification make the final receipt durable before deleting probe evidence; publication failure retains staged receipt and probes, and cleanup failure never removes durable authority. Focused link-boundary, signal, final-link, and cleanup-failure cases pass without introducing a generic transaction framework. |
