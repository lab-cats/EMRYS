...
| `INIT-02` | Study selection usability | Verification pending | `4` | `3` | Read the Viking EV/PUM1 study's maintained 25-name selection without making the user enter names or a manifest path. | Guided Init offers the packaged `1`–`22`, `X`, `Y`, `MT` selection by name after EV/PUM1 sample assignments and requires an explicit yes. The existing [`step_07_partitions.primary_contigs.tsv`](../../configs/step_07_partitions.primary_contigs.tsv) path links to that same file. Rows pass existing manifest/reference admission, preserve order and exclude extra contigs. Declining continues generic region questions; explicit manifests/selectors bypass the offer and noninteractive callers still supply selectors. The installed command needs no checkout data path. Exact branch-head hosted regression passed; novice Viking acceptance remains under CV-06/U20/U21. |
...
exception is approved. `INIT-02` passed branch-head hosted CI and awaits novice
verification; `CV-U22` remains Open against its original reuse intent. CV-26 is
Completed under the owner's 2026-09-22 structural criterion; its original
full-attribution and institutional E11 timing gates were superseded.
