...
| <code>CONFIG-01</code> | P1 | Open | Replace configuration sprawl with one scientist-facing project definition and explicit scientific, execution, and evidence ownership. | Current request-to-Analysis admission remains internal. The operator path now collapses separate launcher and resource files into one optional admitted execution profile; omission selects packaged direct defaults and admitted CLI resource overrides have highest current precedence. Run-bound resource declaration, Attempt-local placement, and the separate runtime profile remain distinct and inspectable. The one scientist-facing Project definition and automatic generation of remaining scientific/runtime inputs remain Open. |
...
| <code>OPS-02</code> | P1 | Open | Provide a small role-aware public CLI instead of exposing wrappers, implementation variables, scheduler mechanics, or direct engine controls. | Grouped <code>run</code>/<code>resume</code> now select direct or whole-Run Slurm placement from the admitted profile. Slurm submits once through a private transport and re-enters the same one-host backend; the generated wrapper is retired. Dry-run writes and submits nothing, while execute prints exact <code>JOB_ID</code>, <code>OUT</code>, and <code>ERR</code>. Real scheduler/site and allocation-sensitive outcome parity, broader command simplification, progress/status, and stable expert routes remain Open. |
...
| <code>RUN-03</code> | P1 | Open | Realize a coherent journey around public Run as the immutable binding of one admitted Analysis revision and one internal immutable Execution Plan while eliminating manual transfer of run roots or internal state between phases. | Scientific-intent and identity-bearing declared-plan changes create a new Run; locator, placement, logging, and report-only changes do not. Re-execution creates an Attempt, which cannot alter or reconstruct Run. Reporting remains default-on for full runs, disable-able, independently regenerable, and creates neither Run nor Attempt. Current Run authority, historical read/resume, separated Results/reporting, and controlled planning parity are implemented. The same grouped surface now owns direct and whole-Run Slurm realization without a generated wrapper: computational declaration may affect Run identity, while profile location/raw bytes, placement, allocation, scheduler job ID, logging, and private transport state remain Run-neutral Attempt facts. Real placement/outcome parity, the broader one-command journey, report opt-out/regeneration, generalized realization, and remaining public migration remain Open. |
...
| <code>LOG-05</code> | P1 | Open | Activate concise role-appropriate default logging using the implemented foundation, incrementally across every retained applicable operation. | Grouped <code>run</code>/<code>resume</code> execution is the first production adopter. Exactly one compute-side operation owns one application attempt under <code>&lt;workspace&gt;/logs/application</code>; valid dry-run and scheduler submission own none. Normal output is concise, verbose adds allocation detail, debug adds exact safe commands, and bounded failure summaries identify the durable log plus owned recovery paths. Receipt-last evidence remains authoritative and post-open log degradation cannot change execution or exit. The card remains Open for every other retained applicable operation and required parity. |
