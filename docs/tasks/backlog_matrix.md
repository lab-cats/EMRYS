...
authorities. Product types, persistence, APIs, storage, compatibility
mechanics, migration, and implementation remain Open.
...
Attempt are ratified. Exact product types, persistence, APIs, and setup/init,
validate/check, status/resume, synthetic/demo, inspect, and report spellings
remain Open. The binding requirement is a small coherent role-appropriate
surface, not the illustrative commands above.
...
semantic identity fields and logical authorities are selected. Exact expert
interfaces, product types, layers, APIs, backend implementations, policy
interfaces, persistence, storage, compatibility mechanics, migration, and
facade use remain Open decisions.
...
ownership. Product representation, persistence, APIs, backend realization,
policy interfaces, storage, compatibility mechanics, migration, and
implementation remain Open. A
...
| ID | Priority (provisional) | Status | Required outcome | Acceptance |
|---|---|---|---|---|
| <code>CONTROL-01</code> | P1 | Open | Realize the ratified compact role-aware public model <code>Project -&gt; Analysis -&gt; Run -&gt; Results</code>, with Attempt progressively disclosed and generated internals hidden from ordinary operation but inspectable. | Project is mutable organizational context; an admitted Analysis revision is immutable scientific intent; public immutable Run binds exactly one Analysis revision to one internal immutable inspectable Execution Plan and owns the primary ordinary ID; Results is read-only; Attempt appears for retry, failure, recovery, or advanced inspection. Dataset, Reference, and ExperimentalDesign remain scientific-definition sections, Runtime/profile is operator-facing, Artifact advanced, Task internal, and Report a downstream Results capability. <code>ARCH-MODEL-FIELDS-01</code> fixes semantic fields, subordinate identity rules, logical authorities, and recovery ownership; API, product types, persistence, storage, compatibility mechanics, migration, and implementation remain Open. |
...
| <code>RUN-03</code> | P1 | Open | Realize a coherent journey around public Run as the immutable binding of one admitted Analysis revision and one internal immutable Execution Plan while eliminating manual transfer of run roots or internal state between phases. | Scientific-intent changes create a new Analysis revision and Run; changes to identity-bearing effective toolchain/backend/profile/resource-policy semantics create a new Run; raw config, locator, placement, logging, and report-only changes do not. Re-executing the same plan creates an Attempt; actual realization may vary only inside the Run's declared permissible envelope. Attempt state cannot alter or reconstruct Run. Reporting is default-on for full runs, disable-able, independently regenerable, and creates neither Run nor Attempt. <code>ARCH-MODEL-FIELDS-01</code> fixes exact semantic fields, the symbolic resource/Attempt envelope, Run-ID composition, logical authorities, and Run-admission recovery ownership. Identity storage, command boundaries, ordering, operation signatures, executor realization, compatibility mechanics, migration, and implementation remain Open. |
| <code>IDENTITY-01</code> | P2 | Open | Realize the ratified smallest role-appropriate identity model while preserving detailed identities as evidence metadata. | Ordinary users need one primary Run ID; Analysis may be human-named while retaining internal immutable identity; Attempt ID is surfaced only when truthful retry, failure, recovery, or advanced inspection requires it. <code>ARCH-MODEL-FIELDS-01</code> fixes relocation-independent Analysis/Execution-Plan identities and the domain-separated Run digest over their canonical digests; human aliases, raw formatting/order, paths, Attempt facts, and reporting are excluded. Product representation, subordinate exposure, historical-reader mechanics, and migration remain Open. Existing commit, package, runtime, request, task, artifact, receipt, and publication identities remain preserved as detailed evidence metadata, and no subsystem reconstructs a competing Run identity. |
...
| <code>ARCH-01</code> | P2 | Open | Establish formal architectural layers and dependency direction, introduce deliberate application/infrastructure abstractions, and aggressively reduce maintained surface and cross-module coupling while preserving every declared invariant. | Completed <code>ARCH-CONST-01</code> supplies the qualified 27-invariant register and original five binding migration/test guardrails. Completed <code>ARCH-LAYER-01</code> supplies the responsibility model, three distinct dependency graphs, current-owner crosswalk, forbidden authority transfers, exact current CLI-composition and transitional-import rosters, and a fast Python source-boundary ratchet. Completed <code>ARCH-MODEL-AUDIT-01</code> supplies the current application/control representation, lifetime, owner/caller, identity, mutation, protection/evidence, and conditional-compression baseline. Completed <code>ARCH-MODEL-DECISION-01</code> selects model C, the compact public vocabulary, and the Run-versus-Attempt boundary. Completed <code>ARCH-MODEL-FIELDS-01</code> selects exact semantic fields and identity composition, the symbolic Attempt envelope, logical authorities and current-record retirement direction, initialization recovery ownership, and status-domain separation. Campaign governance makes compression and mutation inventories mandatory, defaults each implementation slice to net-negative maintained product code with no product-file growth, requires separately reported category deltas, and reserves exact evidence deletion for separate user approval and a separate commit. <code>ARCH-01</code> remains Open for application, operation, execution, policy, identity, and artifact APIs; product types and persistence; backend and storage realization; authority/caller migrations; Artifact Store decision; package realization; guarantee-preservation contracts; and any facade use/order. One final authority per artifact class or guarantee without a god object, review-relevant scientific visibility, bounded migration, caller migration and parity before completion, eventual retirement, and mapped equal-or-stronger defense replacement are binding rather than open. |
...
|---|---|---|
| <code>CONTROL-01</code>, <code>REVIEW-UX-03</code> | The public surface must become substantially simpler and use progressive disclosure for scientist, advanced-scientist, operator, and developer roles. <code>ARCH-MODEL-DECISION-01</code> ratifies <code>Project -&gt; Analysis -&gt; Run -&gt; Results</code>, with Attempt progressively disclosed; Execution Plan internal/inspectable; Dataset, Reference, and ExperimentalDesign as scientific-definition sections; Runtime/profile operator-facing; Artifact advanced; Task internal; and Report downstream beneath Results. <code>ARCH-MODEL-FIELDS-01</code> fixes semantic fields and subordinate identity rules without exposing generated records. | Simplification preserves guarantees. Every effective operational value and source remains inspectable, while only owner-defined safe values are overrideable. Expert interface, safe override roster, APIs, command names, product types, persistence, storage, compatibility mechanics, migration, percentages, and concept-count targets remain Open. |
| <code>CONTROL-01</code>, <code>RUN-03</code>, <code>IDENTITY-01</code>, <code>ARCH-01</code> | Completed <code>ARCH-MODEL-AUDIT-01</code> records that <code>contract/normalized.json</code> is the closest durable immutable contract for the current path-sensitive identity-envelope digest, while <code>AttemptPlan</code> is the closest complete effective executable plan but is ephemeral, attempt-specific, and only shallowly immutable. Completed <code>ARCH-MODEL-DECISION-01</code> selects model C and the Run/Attempt boundary. Completed <code>ARCH-MODEL-FIELDS-01</code> fixes exact semantic field allocation, domain-separated Run-ID and relocation/content/order rules, the symbolic resource/Attempt envelope, one logical authority per boundary, direct compatibility/retirement direction, Run-admission recovery ownership, and separate Attempt/Results/reporting status domains. | Product types and serialized records, persistence/storage, APIs/packages, backend adapters, public status vocabulary, caller migration mechanics, compatibility duration, tests, and all product changes remain Open. Current implementation type names are descriptive only, historical evidence remains readable, and no evidence deletion is authorized. |
