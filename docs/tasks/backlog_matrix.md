...
Task internal, and Report a downstream Results capability. Exact fields,
identity composition, persistence, APIs, storage, compatibility, migration,
...
Attempt are ratified. Exact types, fields, persistence, APIs, and setup/init,
...
exact expert interfaces, fields, layers, APIs, backends, policies, persistence,
storage, compatibility, migration, and facade use remain Open decisions.
...
govern public vocabulary and Run-versus-Attempt semantics. Exact fields,
authorities, persistence, recovery, APIs, backends, policy interfaces, storage,
compatibility, migration, and implementation remain Open. A
...
| ID | Priority (provisional) | Status | Required outcome | Acceptance |
|---|---|---|---|---|
| <code>CONTROL-01</code> | P1 | Open | Realize the ratified compact role-aware public model <code>Project -&gt; Analysis -&gt; Run -&gt; Results</code>, with Attempt progressively disclosed and generated internals hidden from ordinary operation but inspectable. | Project is mutable organizational context; an admitted Analysis revision is immutable scientific intent; public immutable Run binds exactly one Analysis revision to one internal immutable inspectable Execution Plan and owns the primary ordinary ID; Results is read-only; Attempt appears for retry, failure, recovery, or advanced inspection. Dataset, Reference, and ExperimentalDesign remain scientific-definition sections, Runtime/profile is operator-facing, Artifact advanced, Task internal, and Report a downstream Results capability. Exact fields, subordinate identity, API, persistence, storage, compatibility, migration, and implementation remain Open under the next <code>AC-SLICE-03</code> field-and-authority package. |
...
| <code>RUN-03</code> | P1 | Open | Realize a coherent journey around public Run as the immutable binding of one admitted Analysis revision and one internal immutable Execution Plan while eliminating manual transfer of run roots or internal state between phases. | Scientific-intent changes create a new Analysis revision and Run; declared toolchain/backend/profile/resource-policy changes create a new Run; re-executing the same plan creates an Attempt; actual realization may vary only inside the Run's declared permissible envelope. Attempt state cannot alter or reconstruct Run. Reporting is default-on for full runs, disable-able, independently regenerable, and creates neither Run nor Attempt. Exact fields, envelope, identity/storage, recovery ownership, command boundaries, ordering, operation signatures, executor selection, compatibility, migration, and implementation remain Open. |
| <code>IDENTITY-01</code> | P2 | Open | Realize the ratified smallest role-appropriate identity model while preserving detailed identities as evidence metadata. | Ordinary users need one primary Run ID; Analysis may be human-named while retaining internal immutable identity; Attempt ID is surfaced only when truthful retry, failure, recovery, or advanced inspection requires it. Run ID deterministically binds the admitted Analysis revision and Execution Plan, but exact digest fields, relocation semantics, subordinate identity exposure, and compatibility remain Open. Existing commit, package, runtime, request, task, artifact, receipt, and publication identities remain preserved as detailed evidence metadata, and no subsystem reconstructs a competing Run identity. |
