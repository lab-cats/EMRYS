...
| <code>OBS-02</code> | P1 | Open | Provide a supported high-level progress, status, and recovery-guidance surface while hiding the internal execution/publication state machine. | Inspection now reports Run integrity, scientific Attempt outcome, scientific Results, reporting, recovery, and domain-specific blockers independently; lifecycle/control recovery uses that read model rather than the legacy aggregate status. The remaining outcome must add preparation/alignment/QC/candidate/report milestones, elapsed time, supported next-action guidance, and progressive disclosure while keeping engine jobs, transactions, publication states, and internals in evidence/debug views. |
...
| <code>CONTROL-01</code>, <code>RUN-03</code>, <code>IDENTITY-01</code>, <code>ARCH-01</code> | The completed audit and decisions selected model C, the Run/Attempt boundary, exact semantic fields, domain-separated identities, symbolic resource envelope, logical authorities, recovery ownership, and separated status domains. Current request-to-Analysis intake freezes admitted bytes and authority without introducing a public Project representation. Successor canonical Analysis-revision, Execution-Plan, and Run-binding records are committed Run-last; workflow/task admit exact Run authority; reporting inputs are Attempt-owned and origin-bound; historical read/recovery remains; no successor execution projection remains; the read-only status cutover separates Run integrity, scientific Attempt outcome, Results, reporting, and recovery while preserving receipt-v1; the grouped CLI is the only supported Run-control surface; and successor Attempts derive executor provenance from their immutable Execution Plan while allocation placement remains Attempt-local. | Public Project/Analysis/Results representation and control, generalized backend/policy boundaries, progress/guidance, and remaining public/campaign migration remain Open. Private planning helpers and current low-level names are not automatically public, historical evidence remains readable, and no evidence deletion is authorized. |
