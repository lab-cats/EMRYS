...
surviving equal-or-stronger defense may permit redundant protection removal
without a one-for-one new test. Retained evidence is a separate boundary:
...
| ID | Priority (provisional) | Status | Required outcome | Acceptance |
|---|---|---|---|---|
| <code>CONTROL-01</code> | P1 | Open | Realize the ratified compact role-aware public model <code>Project -&gt; Analysis -&gt; Run -&gt; Results</code>, with Attempt progressively disclosed and generated internals hidden from ordinary operation but inspectable. | Project is mutable organizational context; an admitted Analysis revision is immutable scientific intent; public immutable Run binds exactly one Analysis revision to one internal immutable inspectable Execution Plan and owns the primary ordinary ID; Results is read-only; Attempt appears for retry, failure, recovery, or advanced inspection. Dataset, Reference, and ExperimentalDesign remain scientific-definition sections, Runtime/profile is operator-facing, Artifact advanced, Task internal, and Report a downstream Results capability. Current request-to-Analysis intake now freezes admitted request/profile/construction bytes and Analysis authority while returning only fresh disposable mapping views. The internal successor boundary supplies immutable Analysis/Execution-Plan/Run records, no longer uses a successor execution projection, derives Run integrity, scientific Attempt outcome, Results, reporting, and recovery independently, and exposes one supported grouped-CLI Run-control surface instead of a parallel direct Python planning API. Public Project/Analysis/Results representation and control, progressive disclosure, storage surface, and remaining public migration remain Open. |
| <code>CONFIG-01</code> | P1 | Open | Replace configuration sprawl with one scientist-facing project definition and explicit scientific, execution, and evidence ownership. | Current request-to-Analysis admission provides an immutable internal boundary without selecting the future scientist-facing representation. Scientists ultimately author data/reference/design/analysis intent; operators select and define execution profiles; EMRYS owns evidence configuration and generates normalized requests, manifests, runtime/resource/launcher artifacts, identities, and records; runtime provisioning mode and execution backend/profile remain distinct inspectable choices; every effective value and its source are inspectable. |
...
| <code>OPS-02</code> | P1 | Open | Provide a small role-aware public CLI instead of exposing wrappers, implementation variables, scheduler mechanics, or direct engine controls. | The grouped CLI is now the sole supported Run-control surface and uses one controlled-runtime adapter for run, resume, and inspection. The generated Slurm wrapper validates request compatibility and delegates to grouped Run control instead of repeating Doctor; it remains outer placement around the same local scientific backend. With effective resource resolution held constant, controlled direct and Slurm placement now produce byte-identical planning/materialization except for Attempt-local allocation provenance and its configuration digest. Workstation and scheduled execution still need the same accepted high-level capabilities plus allocation-sensitive and real outcome parity for project preparation, readiness/validation, execution, progress/status, inspection, and reporting with safe defaults. The task decides command partitioning and ordering, scheduler-selection behavior, and stable advanced inspection/override/debug routes. Exact command names and sequence remain open. |
...
| <code>RUN-03</code> | P1 | Open | Realize a coherent journey around public Run as the immutable binding of one admitted Analysis revision and one internal immutable Execution Plan while eliminating manual transfer of run roots or internal state between phases. | Scientific-intent changes create a new Analysis revision and Run; changes to identity-bearing effective toolchain/backend/profile/resource-policy semantics create a new Run; raw config, locator, placement, logging, and report-only changes do not. Re-executing the same plan creates an Attempt; actual realization may vary only inside the Run's declared permissible envelope. Attempt state cannot alter or reconstruct Run. Reporting is default-on for full runs, disable-able, independently regenerable, and creates neither Run nor Attempt. The current path constructs and commits the immutable successor Run before allocation/Attempt, admits it directly through workflow/task boundaries, binds reporting inputs to their origin Attempt, supports zero-Attempt inspection and compatible recovery, keeps historical Runs readable/resumable, and does not let reporting failure erase successful scientific work. Successor Attempts derive executor provenance from the immutable Execution Plan; direct-versus-Slurm allocation and exact Slurm job ID remain Attempt-local and Run-identity-neutral; controlled placement changes with equal effective resource resolution leave the Run and all non-placement materialization bytes unchanged. The coherent public one-command journey, allocation-sensitive and real placement/outcome parity, report opt-out/regeneration, role-aware operation signatures, generalized executor/policy realization, and remaining public migration remain Open. |
...
| <code>FILESYSTEM-01</code> | P2 | Open | Define a stable generated public storage model with one discoverable scientist-facing result surface and decide its relationship to the proposed Run Bundle. | Locations for provenance, work, results/artifacts/reports, and logs are predictable and created automatically; no manual directory assembly or competing hidden report root remains. Exact public directory labels, on-disk nesting, bundle layout, portability, archival, and sharing semantics remain Open. |
...
| <code>OBS-02</code> | P1 | Open | Provide a supported high-level progress, status, and recovery-guidance surface while hiding the internal execution/publication state machine. | Inspection reports Run integrity, scientific Attempt outcome, scientific Results, reporting, recovery, domain-specific blockers, and one deterministic safe next action independently; lifecycle/control recovery uses that read model rather than the retired aggregate Python projections. The remaining outcome must add preparation/alignment/QC/candidate/report milestones, elapsed time, and progressive disclosure while keeping engine jobs, transactions, publication states, and internals in evidence/debug views. |
...
| <code>ARCH-01</code> | P2 | Open | Establish formal architectural layers and dependency direction, introduce deliberate application/infrastructure abstractions, and aggressively reduce maintained surface and cross-module coupling while preserving every declared invariant. | Completed <code>ARCH-CONST-01</code>, <code>ARCH-LAYER-01</code>, <code>ARCH-MODEL-AUDIT-01</code>, <code>ARCH-MODEL-DECISION-01</code>, and <code>ARCH-MODEL-FIELDS-01</code> supply the invariant, responsibility, audit, model, and semantic prerequisites. Successor Run authority, Run-last persistence, current-path migration, historical read/resume, direct workflow/task Run admission, Attempt-owned reporting inputs, and temporary-projection retirement are implemented. Campaign governance still requires compression/mutation inventories, category-separated deltas, and separate approval for any evidence deletion. <code>ARCH-01</code> remains Open for the public application/operation surface; execution, policy, identity, and artifact APIs; Project and Results realization; generalized backend and storage realization; remaining authority/caller migrations; Artifact Store decision; package realization; and guarantee-preservation contracts. One final authority per artifact class or guarantee without a god object, review-relevant scientific visibility, bounded migration and parity, eventual retirement, and mapped equal-or-stronger defense replacement remain binding. |
...
| <code>RESULTS-01</code> | P1 | Open | Publish one discoverable scientist-facing result bundle beneath the canonical run-relative results surface and make it the ordinary entry point into the broader Run Bundle. | Inspection derives scientific Results independently of reporting and always prints independently verified report locations. The remaining outcome must collect primary reports, ranked results, complete tables, and evidence/operational links under <code>&lt;run-root&gt;/results</code>; <code>FILESYSTEM-01</code> leaves no competing hidden report root, duplicate authority, or scratch/intermediate content there. The result remains bound to its Run identity and provenance without requiring forensic navigation. |
...
|---|---|---|
| <code>CONTROL-01</code>, <code>REVIEW-UX-03</code> | The public surface must become substantially simpler and use progressive disclosure for scientist, advanced-scientist, operator, and developer roles. <code>ARCH-MODEL-DECISION-01</code> ratifies <code>Project -&gt; Analysis -&gt; Run -&gt; Results</code>, with Attempt progressively disclosed; Execution Plan internal/inspectable; Dataset, Reference, and ExperimentalDesign as scientific-definition sections; Runtime/profile operator-facing; Artifact advanced; Task internal; and Report downstream beneath Results. <code>ARCH-MODEL-FIELDS-01</code> fixes semantic fields and subordinate identity rules, and the first internal Run-authority records/persistence are implemented without exposing generated records as the ordinary interface. | Simplification preserves guarantees. Every effective operational value and source remains inspectable, while only owner-defined safe values are overrideable. Expert interface, safe override roster, role-aware APIs, command names, public Project/Results types, generalized storage, remaining compatibility migration, percentages, and concept-count targets remain Open. |
| <code>CONTROL-01</code>, <code>RUN-03</code>, <code>IDENTITY-01</code>, <code>ARCH-01</code> | The completed audit and decisions selected model C, the Run/Attempt boundary, exact semantic fields, domain-separated identities, symbolic resource envelope, logical authorities, recovery ownership, and separated status domains. Current request-to-Analysis intake freezes admitted bytes and authority without introducing a public Project representation. Successor canonical Analysis-revision, Execution-Plan, and Run-binding records are committed Run-last; workflow/task admit exact Run authority; reporting inputs are Attempt-owned and origin-bound; historical read/recovery remains; no successor execution projection remains; read-only inspection separates Run integrity, scientific Attempt outcome, Results, reporting, and recovery, derives one safe next action, and retires the aggregate Python projections while preserving receipt-v1; the grouped CLI is the only supported Run-control surface; and successor Attempts derive executor provenance from their immutable Execution Plan while allocation placement remains Attempt-local. | Public Project/Analysis/Results representation and control, generalized backend/policy boundaries, high-level milestones, elapsed time, role-aware presentation, and remaining public/campaign migration remain Open. Private planning helpers and current low-level names are not automatically public, historical evidence remains readable, and no evidence deletion is authorized. |
| <code>CONFIG-01</code>, <code>OPS-01</code>, <code>RUNTIME-01</code> | Scientific intent, execution/site policy, and EMRYS-owned evidence configuration have different authors. Named execution profiles and Managed/Site/Explicit are source-proposed ways to represent distinct concerns; the tasks must choose terminology and taxonomy without conflating runtime provisioning, execution backend, or profile. The current local-backend/outer-Slurm boundary separates backend semantics from Attempt-local placement without selecting that taxonomy. | Whatever taxonomy is accepted, generated configuration retains an inspectable precedence and provenance model, safe overrides remain owner-defined, and runtime/readiness qualification includes internal engine dependencies such as Snakemake without exposing engine invocation or configuration as an ordinary scientist task. |
