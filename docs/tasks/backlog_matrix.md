...
| <code>OBS-02</code> | P1 | Open | Provide a supported high-level progress, status, and recovery-guidance surface while hiding the internal execution/publication state machine. | Inspection reports Run integrity, scientific Attempt outcome, scientific Results, reporting, recovery, domain-specific blockers, and one deterministic safe next action independently; lifecycle/control recovery uses that read model rather than the retired aggregate Python projections. The remaining outcome derives preparation/alignment/QC/candidate/statistical-context milestones and downstream reporting from persisted EMRYS records, labels elapsed time for the current/latest Attempt without summing resumes or inferring ETA, and progressively discloses engine, scheduler, transaction, receipt, and task detail. The dashboard is stale and receives no campaign updates; retirement is reconsidered only after campaign completion and separate approval. |
