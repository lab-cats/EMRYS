...
|---|---|---|---|---|
| Package root `__main__.py` | Installed-command composition over owner validation, inspection, onboarding, and Run/report control | Source import and invocation | Transitional | `ARCH-CLOSE-01` completes the Project-root ordinary-command cutover. Low-level reporting builders are private; the installed CLI reaches only the Run-oriented reporting operation. |
...
| Semantic behavior under `stages/`, `analyses/`, and `evidence/` | Recognizable transformation, analysis, and evidence semantics with adjacent contracts and tests; the bounded analysis-module descriptor declares typed inputs/outputs, exact dependencies, minimum resources, and producer/validator plans | Import, invocation, and semantic artifact/evidence flow | Aligned | `ANALYSIS-02` implementation is present and awaits exact-head verification with `ARCH-CLOSE-01`; it adds no universal Stage hierarchy or workflow language. |
