...
| Local orchestration | The semantic all-pass command is implemented; the accepted design has Snakemake's local executor coordinate one fixed CMH profile over public owner commands and publish content-bound verified records. | The fixed profile, workflow assets, executor profile, task/attempt publication, inspection, and lifecycle adapter are not implemented. |
