...
| --- | --- | --- |
| Local YAML+TSV lifecycle | B2 implements closed request/profile/execution/lifecycle schemas plus read-only safe-YAML normalization, content identity, and deterministic reporting projection. | Run materialization, immutable publication, inspection, and public lifecycle commands are not implemented. There is no version 1 request queue or watcher. |
| Local orchestration | B3 implements the fixed local-CMH profile, thirteen-rule static Snakemake graph, local executor profile, closed dispatch task boundary, task-attempt publication, and content-bound verified records. Reference, one-sample, and cohort slices are proven with no-science test doubles. | Reporting-tail scheduling, aggregate attempt finalization, interruption/resume/inspection, the public lifecycle adapter, and real science-tool execution are not implemented. |
