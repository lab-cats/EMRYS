...
| Phase `03` sixth bounded package | Move `construct_canonical_BAM` through selected [`MIG-03F`](../tasks/IN_PROGRESS/MIG-03F-migrate-construct-canonical-bam-owner.md) | selected from published three-review-complete checkpoint `a8d760b`; task-specific planning precedes any executable/test mutation, and no downstream or later owner is preloaded |
...
All three `03F` reviews are complete, and `MIG-03F` alone is selected in the
commit containing this paragraph; later owners remain unselected.
