...
| Sixth JIT-tranche reviews | [`REVIEW-ARCH-03F`](../tasks/TODO/REVIEW-ARCH-03F-review-construct-canonical-bam-migration.md) → [`REVIEW-REL-03F`](../tasks/TODO/REVIEW-REL-03F-review-construct-canonical-bam-migration.md) → [`REVIEW-UX-03F`](../tasks/TODO/REVIEW-UX-03F-review-construct-canonical-bam-migration.md) | JIT-defined and unselected; the sequential passes must settle the two-function neutral-helper boundary, rollback-failure parity, scheduler continuity, and public/recovery findability before migration selection |
| Phase `03` sixth bounded package | Move `construct_canonical_BAM` through [`MIG-03F`](../tasks/TODO/MIG-03F-migrate-construct-canonical-bam-owner.md) | JIT-defined only from published parent `fa79883`; no review is selected, no executable/test path moved, and no downstream or later owner is preloaded |
...
documentation close `fa79883`. `MIG-03F` and its three review cards are
JIT-defined in the commit containing this paragraph; all four remain unselected,
and no downstream or later owner is preloaded.
