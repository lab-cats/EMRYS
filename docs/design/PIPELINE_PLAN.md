...
| Seventh JIT-tranche reviews | [`REVIEW-ARCH-03G`](../tasks/TODO/REVIEW-ARCH-03G-review-collect-canonical-bam-qc-evidence-migration.md) → [`REVIEW-REL-03G`](../tasks/TODO/REVIEW-REL-03G-review-collect-canonical-bam-qc-evidence-migration.md) → [`REVIEW-UX-03G`](../tasks/TODO/REVIEW-UX-03G-review-collect-canonical-bam-qc-evidence-migration.md) | defined and unselected; the reviews must challenge evidence-owner placement, direct-final mixed-attempt faults, final commands, and evidence ceilings sequentially before migration selection |
| Phase `03` seventh bounded package | Move `collect_canonical_BAM_QC_evidence` through [`MIG-03G`](../tasks/TODO/MIG-03G-migrate-collect-canonical-bam-qc-evidence-owner.md) | defined and unselected from clean MIG-03F close `543eb8f`; Step `02b` is the first canonical identity among three live-DAG-eligible owners, and no Step `03`, Step `04`, or later card is created |
...
live DAG then exposed three eligible owners. The commit containing this
paragraph defines only first-listed `collect_canonical_BAM_QC_evidence` as
`MIG-03G` plus its three sequential reviews. All four remain unselected in
`TODO`; Step `03`, Step `04`, and all later owner cards remain uncreated.
