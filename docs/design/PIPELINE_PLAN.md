...
strategy-card base. `TEST-01C` through `TEST-01Z` are implemented locally, and
TEST-01Z recorded an affirmative 88-row decision with no undefined row or
...
review at `fb21c9d` reproduced both blockers and returned `PUBLISHABLE`; live
Git owns publication and upstream-equality status. The decision releases only
`ARCH-02A`, `RPT-01`, `LOG-01`, and `DOC-IA-01` for later separate task
