...
Usability is complete from published/equal selection `3f71f87` with exact final
journeys, dry-run effects, two-directory recovery/preservation, ownership,
provenance, and evidence ceilings. Migration remains unselected, no executable/
test path changed, no computational check ran, and Step `07` and every later
owner/review card remain absent.
