...
| Phase `03` sixth bounded package | Move `construct_canonical_BAM` through selected [`MIG-03F`](../tasks/IN_PROGRESS/MIG-03F-migrate-construct-canonical-bam-owner.md) | old helper argv/result/exception/header parity is recorded from published plan `d8638a0`; native baseline is next and no executable/test mutation has begun |
...
baseline is recorded in the commit containing this paragraph; publish it before
the separate old native-owner baseline. Later owners remain unselected.
