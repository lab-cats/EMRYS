...
| `codex/mig-03a-extract-validation-report-library` | One-branch physical migration campaign beginning with [`MIG-03A`](../tasks/COMPLETED/MIG-03A-extract-validation-report-library.md) and continuing through [`MIG-03B`](../tasks/COMPLETED/MIG-03B-migrate-construct-star-index-owner.md) | neutral owner checkpoint `9d93694` and `construct_STAR_index` executable checkpoint `4f9c863` are complete and published; both use final owners without wrappers or package identity. The next unit must be created and reviewed just in time after this documentation-close commit is published and equal. |
