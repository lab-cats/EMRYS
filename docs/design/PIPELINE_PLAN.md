...
| `codex/mig-03a-extract-validation-report-library` | One-branch physical migration campaign through completed [`MIG-03A`](../tasks/COMPLETED/MIG-03A-extract-validation-report-library.md), [`MIG-03B`](../tasks/COMPLETED/MIG-03B-migrate-construct-star-index-owner.md), and [`MIG-03C`](../tasks/COMPLETED/MIG-03C-migrate-convert-gtf-to-bed12-owner.md) | neutral owner checkpoint `9d93694`, `construct_STAR_index` checkpoint `4f9c863`, and `convert_GTF_to_BED12` executable checkpoint `e19f281` are published; the third documentation/lifecycle close is the commit containing this row, and no later owner is preselected. |
...
campaign branch. `MIG-03C` executable/test checkpoint `e19f281` is published;
its documentation/lifecycle close is the commit containing this paragraph. No
later owner or review is selected. The user has separately authorized this
branch to continue autonomously through only one dependency-valid,
evidence-supported migration unit at a time: after the current close is
published and upstream-equal, create the next card and its three reviews just
in time, close and publish it, and only then inspect the following unit.
