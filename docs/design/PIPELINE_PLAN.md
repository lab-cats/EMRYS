...
| Phase `03` sixth bounded package | Move `construct_canonical_BAM` through [`MIG-03F`](../tasks/TODO/MIG-03F-migrate-construct-canonical-bam-owner.md) | JIT-defined only from published parent `fa79883`; all three reviews are complete, no executable/test path moved, and no downstream or later owner is preloaded |
...
All three `03F` reviews are complete in the commit containing this paragraph;
migration and later owners remain unselected.
