...
| Phase `03` sixth bounded package | Move `construct_canonical_BAM` through selected [`MIG-03F`](../tasks/IN_PROGRESS/MIG-03F-migrate-construct-canonical-bam-owner.md) | exact helper/owner write sets, two old-path baseline slices, minimal slice checks, one final executable card gate, and separate documentation close are frozen; no baseline or executable mutation has begun |
...
slices are frozen in the commit containing this paragraph; publish it before
capturing baseline evidence. Later owners remain unselected.
