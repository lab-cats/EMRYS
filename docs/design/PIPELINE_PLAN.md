...
| Phase `03` eighth bounded package | Move `collect_RSeQC_paired_orientation_evidence` through selected [`MIG-03H`](../tasks/IN_PROGRESS/MIG-03H-migrate-collect-rseqc-paired-orientation-evidence-owner.md) | exact three-test old-path baseline, atomic five-move/nine-update cutover, minimal slice checks, one complete executable card gate, and separate documentation close are frozen; no test mutation has begun |
...
`76923e1`. `MIG-03H` alone is selected at published checkpoint `13b8a7e`. The
exact baseline/cutover write sets, minimal checks, full card-boundary gate,
expected documentation ceiling, rollback order, and separate close are frozen
in the commit containing this paragraph. Step `04` and all later owner/review
cards remain unselected.
