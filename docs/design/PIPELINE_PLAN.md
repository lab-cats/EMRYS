...
| Phase `03` seventh bounded package | Move `collect_canonical_BAM_QC_evidence` through selected [`MIG-03G`](../tasks/IN_PROGRESS/MIG-03G-migrate-collect-canonical-bam-qc-evidence-owner.md) | selected only after all three dedicated reviews closed; task-specific planning precedes the three-test old-path baseline, and no executable/test mutation or later-owner preload has occurred |
...
selected in the commit containing this paragraph; task-specific planning,
baseline, cutover, and close remain later bounded slices, while Step `03`, Step
`04`, and all later owner cards remain uncreated and unselected.
