...
| Phase `03` sixth bounded package | Move `construct_canonical_BAM` through selected [`MIG-03F`](../tasks/IN_PROGRESS/MIG-03F-migrate-construct-canonical-bam-owner.md) | helper and native old-path parity are recorded through published helper checkpoint `9a2517b`; exact five-file helper preparation is next and no executable/test mutation has begun |
...
slices are frozen at published plan `d8638a0`. The old helper result/exception
baseline is published at `9a2517b`; the old native-owner baseline is recorded
in the commit containing this paragraph. Publish it before the exact five-file
helper preparation. Later owners remain unselected.
