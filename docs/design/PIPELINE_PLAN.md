...
| Phase `03` second bounded package | Move `construct_STAR_index` through [`MIG-03B`](../tasks/IN_PROGRESS/MIG-03B-migrate-construct-star-index-owner.md) | selected and task-specific execution baseline frozen after all three dedicated reviews; semantic-root selection follows the direct artifact graph and smallest bounded native-asset surface, no executable/test mutation has begun, and the atomic cutover waits for publication/equality of the baseline checkpoint |
