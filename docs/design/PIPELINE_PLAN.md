...
| Ninth JIT-tranche reviews | completed [`REVIEW-ARCH-03I`](../tasks/COMPLETED/REVIEW-ARCH-03I-review-mark-bam-duplicates-with-picard-migration.md) → completed [`REVIEW-REL-03I`](../tasks/COMPLETED/REVIEW-REL-03I-review-mark-bam-duplicates-with-picard-migration.md) → completed [`REVIEW-UX-03I`](../tasks/COMPLETED/REVIEW-UX-03I-review-mark-bam-duplicates-with-picard-migration.md) | all three read-only independent-in-time passes are complete with exact cutover, residue/parity/scheduler, command, diagnostics, preservation, provenance, evidence-ceiling, and rollback boundaries; migration remains unselected |
| Phase `03` ninth bounded package | Move `mark_BAM_duplicates_with_Picard` through selected [`MIG-03I`](../tasks/IN_PROGRESS/MIG-03I-migrate-mark-bam-duplicates-with-picard-owner.md) | selected after all three reviews from published usability completion; exact five-move/ten-update cutover, three bounded reliability-test owners, final journeys, artifact/coverage continuity, separate close, and rollback are frozen. Execution remains unstarted; Step `05` and all later owner/review cards remain uncreated. |
...
edge check leaves only `mark_BAM_duplicates_with_Picard` eligible because its
sole direct predecessor, `construct_canonical_BAM`, is migrated; Step `05`
still requires Step `04`. The commit containing this paragraph defines only
unselected `MIG-03I` and its architecture → reliability → usability reviews in
`TODO`. `REVIEW-ARCH-03I` is complete at published checkpoint `403fdf5`;
`REVIEW-REL-03I` is complete with an exact three-test-owner characterization
boundary, and `REVIEW-UX-03I` is complete with final-path, preservation,
provenance, and rollback journeys. `MIG-03I` alone is selected in the commit
containing this paragraph with execution unstarted. No executable/test file
changes or test runs, and Step `05` and every later owner/review card remain
