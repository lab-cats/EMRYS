...
| Phase `03` second bounded package | Move `construct_STAR_index` through [`MIG-03B`](../tasks/IN_PROGRESS/MIG-03B-migrate-construct-star-index-owner.md) | selected for task-specific execution planning after all three dedicated reviews; semantic-root selection is based on the direct artifact graph and smallest bounded native-asset surface, not historical numbering, and no executable or test mutation has begun |
