...
| `codex/mig-03l-generate-partitioned-cohort-mpileup-vcfs` | Fresh execution branch for completed [`MIG-03L`](../tasks/COMPLETED/MIG-03L-migrate-generate-partitioned-cohort-mpileup-vcfs-owner.md) and defined but unselected [`MIG-03M`](../tasks/TODO/MIG-03M-migrate-preprocess-and-annotate-cohort-candidates-owner.md) reviews | branched from clean/equal usability completion `5a501df`; five MIG-03L test baselines culminate in executable `60eb356` and documentation close `4562ec3`. The commit containing this row defines only MIG-03M and its three unselected sequential reviews; no card is selected and no Step `09` or later card is preloaded. |
...
| Thirteenth JIT-tranche reviews | [`REVIEW-ARCH-03M`](../tasks/TODO/REVIEW-ARCH-03M-review-preprocess-and-annotate-cohort-candidates-migration.md) → [`REVIEW-REL-03M`](../tasks/TODO/REVIEW-REL-03M-review-preprocess-and-annotate-cohort-candidates-migration.md) → [`REVIEW-UX-03M`](../tasks/TODO/REVIEW-UX-03M-review-preprocess-and-annotate-cohort-candidates-migration.md) | defined but unselected for sequential read-only review of the exact Bash/R/Python/SLURM cutover, private Step `09c`/report loading, transaction/R/validator/scheduler defects, final commands, split-root recovery, evidence ceiling, and rollback; no review finding or completion is claimed. |
...
ten integration owners. Architecture must prove the exact guarded-R test
ownership and private neutral-report/flat-Step-`09c` loader boundary before
reliability fixes any required old-path test-only slices. No card is selected;
no executable/test file changed or ran; and no Step `09` or later owner is
