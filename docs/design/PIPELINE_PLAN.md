...
| `codex/plan-02z-first-migration-readiness` | Rolling [`PLAN-02Z`](../tasks/IN_PROGRESS/PLAN-02Z-integrate-future-task-sequence.md) checkpoint, tranche-specific reviews, and first migration-card readiness rooted at the verified integrated canonical tip | documentation-only planning package in progress; [`MIG-03A`](../tasks/TODO/MIG-03A-extract-validation-report-library.md) is proposed but not selected or executing, all other work remains frozen, and the known nine unsupported-`UNREFINED` validator findings are the non-broadening ceiling |
...
| Phase `02` rolling checkpoint | Minimum shared architecture and one evidence-supported tranche in [`PLAN-02Z`](../tasks/IN_PROGRESS/PLAN-02Z-integrate-future-task-sequence.md) | selected documentation-only planning; later intake, report, logging, documentation, size, and broad-library inputs remain frozen rather than becoming false blockers |
| First-tranche reviews | [`REVIEW-ARCH-03A`](../tasks/TODO/REVIEW-ARCH-03A-review-validation-publication-migration.md) → [`REVIEW-REL-03A`](../tasks/TODO/REVIEW-REL-03A-review-validation-publication-migration.md) → [`REVIEW-UX-03A`](../tasks/TODO/REVIEW-UX-03A-review-validation-publication-migration.md) | separately scoped read-only review chain; not yet selected, and no broad `REVIEW-*` completion is claimed |
| Phase `03` first bounded package | Extract the neutral validation-report protocol through [`MIG-03A`](../tasks/TODO/MIG-03A-extract-validation-report-library.md) | proposed by the selected plan; blocked by its tranche-specific reviews and not selected or executing |
...
decision points without selecting them. `PLAN-02Z` is now selected only to
define `MIG-03A` and its tranche-specific review chain. No review or migration
is selected by this checkpoint. Every other package is preserved but currently
...
selected `PLAN-02Z` first tranche.
...
consolidation cards are not selected. `PLAN-02Z` is selected on the verified
