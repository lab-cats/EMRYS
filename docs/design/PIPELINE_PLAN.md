...
| Second JIT-tranche reviews | [`REVIEW-ARCH-03B`](../tasks/COMPLETED/REVIEW-ARCH-03B-review-construct-star-index-migration.md) → [`REVIEW-REL-03B`](../tasks/COMPLETED/REVIEW-REL-03B-review-construct-star-index-migration.md) → [`REVIEW-UX-03B`](../tasks/TODO/REVIEW-UX-03B-review-construct-star-index-migration.md) | architecture and reliability passes complete; reliability added missing producer-side-effect and arbitrary-CWD validator parity plus exact coverage/provenance obligations, while usability remains unselected |
| Phase `03` second bounded package | Move `construct_STAR_index` through [`MIG-03B`](../tasks/TODO/MIG-03B-migrate-construct-star-index-owner.md) | proposed only; semantic-root selection is based on the direct artifact graph and smallest bounded native-asset surface, not historical numbering, and executable work remains blocked by its dedicated reviews |
