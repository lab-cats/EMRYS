...
| Broader reporting policy | Outside this lifecycle change: independent reporting producers, transaction layout, and validation-check roster policy. The later reporting-memory retirement is defined by the [Run-coordinator contract](../../../src/emrys/orchestration/run_coordinator/CONTRACT.md#profiles-and-immutable-planning). The dashboard remains until a replacement dashboard is implemented and validated. |
